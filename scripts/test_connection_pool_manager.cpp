...
protected:
    ...
    void createPoolManager() {
        // Create null pointers for handler and wsManager since we're testing pool logic
        ...
        poolManager_ = std::make_unique<ConnectionPoolManager>(
            ...
            handler, wsManager, timeoutManager_
        );
    }
...
TEST_F(ConnectionPoolManagerTest, ConcurrentAcquisitionIsThreadSafe) {
    ...
    for (int i = 0; i < numThreads; ++i) {
        futures.push_back(std::async(std::launch::async, [this, connectionsPerThread]() {
            std::vector<std::shared_ptr<PooledSession>> sessions;
...
TEST_F(ConnectionPoolManagerTest, AcquisitionAfterShutdownThrows) {
    ...
    auto socket = createSocket();
    EXPECT_THROW(poolManager_->acquireConnection(std::move(socket)), std::runtime_error);
}
...
TEST_F(ConnectionPoolManagerTest, StatisticsAreAccurate) {
    ...
    auto socket2 = createSocket();
    auto session2 = poolManager_->acquireConnection(std::move(socket2)); // Should reuse
    
    auto socket3 = createSocket();
    auto session3 = poolManager_->acquireConnection(std::move(socket3)); // Should create new
