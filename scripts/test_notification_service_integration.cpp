...
int main() {
    ...
    try {
        ...
        auto notificationService = std::make_shared<NotificationServiceImpl>(&logger);
        ...
        notificationService->sendJobFailureAlert("demo_job_001", "Database connection timeout");
        ...
        std::cout << "Queue size: " << notificationService->getQueueSize() << std::endl;
        std::cout << "Processed count: " << notificationService->getProcessedCount() << std::endl;
        std::cout << "Failed count: " << notificationService->getFailedCount() << std::endl;
        ...
        for (size_t i = 0; i < recentNotifications.size(); ++i) {
            ...
            std::cout << (i + 1) << ". [" << notif.id << "] "
                      << notif.subject << " - " << notif.message << std::endl;
        }
        ...
        customNotif.message = "This is a custom notification created for demonstration purposes";
        ...
        std::cout << "Should retry: " << (retryTest.shouldRetry() ? "yes" : "no") << std::endl;
        
        retryTest.incrementRetry();
        std::cout << "After increment - retry count: " << retryTest.retryCount << std::endl;
        std::cout << "Retry delay: " << retryTest.getRetryDelay().count() << "ms" << std::endl;
        
        // 10. Integration with JobMonitorService simulation
        std::cout << "\n--- JobMonitorService Integration Simulation ---" << std::endl;
        std::cout << "In a real scenario, JobMonitorService would:" << std::endl;
        std::cout << "1. Be initialized with the NotificationService instance" << std::endl;
        std::cout << "2. Call notification methods when job events occur" << std::endl;
        std::cout << "3. Example: onJobStatusChanged() -> sendJobFailureAlert()" << std::endl;
        std::cout << "4. Example: timeout detection -> sendJobTimeoutWarning()" << std::endl;
        
        // Simulate what JobMonitorService would do
        std::cout << "\nSimulating JobMonitorService calling notification methods:" << std::endl;
        notificationService->sendJobFailureAlert("simulated_job_123", "Simulated failure from JobMonitorService");
        ...
        std::cout << "Total processed: " << notificationService->getProcessedCount() << std::endl;
        std::cout << "Total failed: " << notificationService->getFailedCount() << std::endl;
        std::cout << "Queue size: " << notificationService->getQueueSize() << std::endl;
...
1. [notif_66b6d2a0_abcd1234] ETL Job Failed: demo_job_001 - Job demo_job_001 has failed with error: Database connection timeout
2. [notif_66b6d2a1_abcd1235] ETL Job Timeout Warning: demo_job_002 - Job demo_job_002 has been running for 30 minutes, which exceeds the warning threshold of 25 minutes
3. [notif_66b6d2a2_abcd1236] Resource Alert: Memory usage exceeded threshold - Memory usage exceeded threshold. Current value: 0.92 percentage, threshold: 0.85 percentage
4. [notif_66b6d2a3_abcd1237] Resource Alert: CPU usage exceeded threshold - CPU usage exceeded threshold. Current value: 0.95 percentage, threshold: 0.90 percentage
5. [notif_66b6d2a4_abcd1238] System Error in DatabaseManager - A critical error occurred in component DatabaseManager: Connection pool exhausted
6. [notif_66b6d2a5_abcd1239] Resource Alert: Memory usage exceeded threshold - Memory usage exceeded threshold. Current value: 0.88 percentage, threshold: 0.85 percentage
7. [notif_66b6d2a6_abcd123a] Resource Alert: CPU usage exceeded threshold - CPU usage exceeded threshold. Current value: 0.95 percentage, threshold: 0.90 percentage
8. [notif_66b6d2a7_abcd123b] Resource Alert: Disk space usage is above threshold - Disk space usage is above threshold. Current value: 0.92 percentage, threshold: 0.90 percentage

--- Testing Custom Notification ---

--- Testing JSON Serialization ---
Notification JSON: {"id":"notif_66b6d2a8_abcd123c","type":3,"priority":1,"jobId":"","subject":"Demo Custom Notification"...}
