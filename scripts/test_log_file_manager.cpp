...
TEST_F(LogFileManagerTest, WriteToFileIncreasesFileSize) {
    ...
    if (manager.getCurrentFileSize() == 0) {
        ...
        std::string backupFile = getConfig().logDirectory + "/" + getConfig().defaultLogFile + ".1";
...
TEST_F(LogFileManagerTest, CleanupRemovesTempFiles) {
    ...
    for (int i = 1; i <= 3; ++i) {
        std::string tempFile = getConfig().logDirectory + "/temp" + std::to_string(i) + ".tmp";
...
TEST_F(LogFileManagerTest, MetricsTracking) {
    ...
    EXPECT_GT(finalMetrics.totalBytesWritten.load(), initialMetrics.totalBytesWritten.load());
}
