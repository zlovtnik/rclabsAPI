...
void printSecurityResult(const std::string& testName,
    ...
    std::cout << "Rate Limited: " << (result.rateLimitExceeded ? "❌ YES" : "✅ NO") << "\n";
...
int main() {
    ...
    std::cout << "  XSS Protection: " << (config.enableXssProtection ? "Enabled" : "Disabled") << "\n";
    std::cout << "  SQL Injection Protection: " << (config.enableSqlInjectionProtection ? "Enabled" : "Disabled") << "\n";
    ...
    {
        auto req = createTestRequest("GET", "/api/jobs?status=running&limit=10&offset=0", "", {
    ...
    {
        auto req = createTestRequest("GET", "/api/jobs?search=<script>alert('xss')</script>", "", {
            {"User-Agent", "RequestValidator-Demo/1.0"},
    ...
    {
        auto req = createTestRequest("GET", "/api/jobs?id=1'; DROP TABLE users; --", "", {
            {"User-Agent", "RequestValidator-Demo/1.0"},
    ...
    // Display final statistics
    {
        ...
        if (stats.totalRequests > 0) {
            double validPercent = (double)stats.validRequests / stats.totalRequests * 100.0;
    ...
    // Test utility methods
    {
        ...
        std::string jobId1 = validator.extractJobIdFromPath("/api/jobs/job-12345", "/api/jobs/", "");
        std::string jobId2 = validator.extractJobIdFromPath("/api/jobs/job-67890/status", "/api/jobs/", "/status");
        ...
        std::string connId1 = validator.extractConnectionIdFromPath("/api/websocket/conn-abc123/filters", "/api/websocket/");
        std::string connId2 = validator.extractConnectionIdFromPath("/api/websocket/conn-xyz789", "/api/websocket/");
        ...
        std::cout << "  /api/auth/login -> " << (validator.isKnownEndpoint("/api/auth/login") ? "✅ Known" : "❌ Unknown") << "\n";
        std::cout << "  /api/jobs/job-123 -> " << (validator.isKnownEndpoint("/api/jobs/job-123") ? "✅ Known" : "❌ Unknown") << "\n";
        std::cout << "  /api/unknown -> " << (validator.isKnownEndpoint("/api/unknown") ? "✅ Known" : "❌ Unknown") << "\n";
    }
