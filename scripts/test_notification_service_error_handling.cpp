...
public:
    void runTests() {
        std::cout << "=== Notification Service Error Handling Tests ===" << std::endl;
        ...
        std::cout << "✅ All Notification Service error handling tests completed!" << std::endl;
    }
...
private:
    ...
    void testNotificationCircuitBreaker() {
        ...
        notification_recovery::NotificationCircuitBreaker circuitBreaker(3, std::chrono::minutes(1), 2);
        
        // Test initial state (CLOSED)
        assert(circuitBreaker.getState() == notification_recovery::NotificationCircuitBreaker::State::CLOSED);
        assert(circuitBreaker.allowDelivery() == true);
        assert(circuitBreaker.isInDegradedMode() == false);
        
        std::cout << "✓ Notification circuit breaker starts in CLOSED state" << std::endl;
        ...
        circuitBreaker.onDeliveryFailure();
        assert(circuitBreaker.getState() == notification_recovery::NotificationCircuitBreaker::State::OPEN);
        assert(circuitBreaker.allowDelivery() == false);
        assert(circuitBreaker.isInDegradedMode() == true);
        
        std::cout << "✓ Notification circuit breaker opens after failure threshold" << std::endl;
        
        // For testing purposes, we can't easily wait for the full timeout,
        // but we can test the success recovery logic
        notification_recovery::NotificationCircuitBreaker testBreaker(2, std::chrono::seconds(1), 2);
        
        // Trigger failures to open
        testBreaker.onDeliveryFailure();
        testBreaker.onDeliveryFailure();
        assert(testBreaker.getState() == notification_recovery::NotificationCircuitBreaker::State::OPEN);
        ...
        assert(testBreaker.getState() == notification_recovery::NotificationCircuitBreaker::State::HALF_OPEN);
        
        // Test recovery with successes
        testBreaker.onDeliverySuccess();
        testBreaker.onDeliverySuccess();
        assert(testBreaker.getState() == notification_recovery::NotificationCircuitBreaker::State::CLOSED);
        
        std::cout << "✓ Notification circuit breaker recovery works correctly" << std::endl;
    }
    ...
    void testRetryQueueManager() {
        ...
        retryManager.addFailedNotification(testNotification, "Network timeout", NotificationMethod::EMAIL);
        
        assert(retryManager.empty() == false);
        assert(retryManager.size() == 1);
        
        std::cout << "✓ Failed notifications can be added to retry queue" << std::endl;
        ...
        std::cout << "✓ Failed notifications become ready for retry after delay" << std::endl;
        
        // Test multiple notifications with different retry counts
        testNotification.retryCount = 1;
        retryManager.addFailedNotification(testNotification, "Server error", NotificationMethod::WEBHOOK);
        
        testNotification.id = NotificationMessage::generateId();
        testNotification.retryCount = 2;
        retryManager.addFailedNotification(testNotification, "Connection refused", NotificationMethod::SLACK);
    ...
    void testFailedNotificationHandling() {
        ...
        auto nextRetryTime = std::chrono::system_clock::now() + std::chrono::milliseconds(500);
        
        notification_recovery::FailedNotification failedNotification(
            notification, nextRetryTime, "Connection timeout", NotificationMethod::EMAIL);
        ...
        std::cout << "✓ Failed notification becomes ready for retry after scheduled time" << std::endl;
        ...
        std::cout << "✓ Exponential backoff retry delay calculation works correctly" << std::endl;
    }
    ...
    void testDeliveryErrorScenarios() {
        ...
        for (const auto& errorScenario : errorScenarios) {
            retryManager.addFailedNotification(notification, errorScenario, NotificationMethod::WEBHOOK);
            std::cout << "  Added failed notification for: " << errorScenario << std::endl;
        }
        
        assert(retryManager.size() == errorScenarios.size());
        
        std::cout << "✓ Various delivery error scenarios can be handled and queued" << std::endl;
        ...
        std::cout << "✓ Error scenarios categorized for retry decision making" << std::endl;
    }
    ...
    void testBulkRetryMechanism() {
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            retryManager.addFailedNotification(notification, "Bulk test failure", NotificationMethod::EMAIL);
        }
        ...
        for (size_t i = 0; i < readyNotifications.size(); ++i) {
            ...
            assert(failedNotification.notification.jobId == "bulk_job_" + std::to_string(i));
            assert(failedNotification.failureReason == "Bulk test failure");
            assert(failedNotification.failedMethod == NotificationMethod::EMAIL);
            assert(failedNotification.isReadyForRetry() == true);
        }
        
        std::cout << "✓ All bulk retry notifications have correct properties" << std::endl;
    }
};
