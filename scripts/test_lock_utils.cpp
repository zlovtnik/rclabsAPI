...
void testBasicLocking() {
    ...
    {
        ScopedTimedLock lock(testMutex, std::chrono::milliseconds(1000), "test_mutex");
        assert(lock.owns_lock());
        std::cout << "✓ Successfully acquired lock: " << lock.getLockName() << std::endl;
...
void testLockTimeout() {
    ...
    std::thread holder([&testMutex]() {
        ScopedTimedLock lock(testMutex, std::chrono::milliseconds(5000), "holder_lock");
    ...
    try {
        ScopedTimedLock lock(testMutex, std::chrono::milliseconds(100), "timeout_test");
...
void testOrderedMutex() {
    std::cout << "\n=== Testing Ordered Mutex and Deadlock Prevention ===" << std::endl;
    ...
    {
        ScopedTimedLock configLock(configMutex, std::chrono::milliseconds(1000), "config");
        ScopedTimedLock containerLock(containerMutex, std::chrono::milliseconds(1000), "container");
        ScopedTimedLock resourceLock(resourceMutex, std::chrono::milliseconds(1000), "resource");
    ...
    try {
        ScopedTimedLock resourceLock(resourceMutex, std::chrono::milliseconds(1000), "resource_first");
...
void testSharedMutex() {
    ...
    std::thread writer([&sharedMutex, &readerCount, &writerActive]() {
        ScopedTimedLock lock(sharedMutex, std::chrono::milliseconds(2000), "writer");
    ...
    std::cout << "✓ Shared mutex reader-writer coordination working correctly" << std::endl;
}
...
void testLockMonitoring() {
    ...
    for (int i = 0; i < 5; ++i) {
        ScopedTimedLock lock(testMutex, std::chrono::milliseconds(1000), "monitored_lock");
    ...
    std::cout << "  - Average wait time: " << stats.getAverageWaitTime() << "μs" << std::endl;
    std::cout << "  - Max wait time: " << stats.maxWaitTime.load() << "μs" << std::endl;
...
void testConcurrentAccess() {
    ...
    for (int i = 0; i < 10; ++i) {
        workers.emplace_back([&containerMutex, &counter, &maxConcurrent, &currentConcurrent, i]() {
            for (int j = 0; j < 5; ++j) {
    ...
    assert(maxConcurrent.load() == 1); // Mutex should ensure only 1 concurrent access
...
void testPerformanceUnderLoad() {
    ...
    for (int i = 0; i < numWorkers; ++i) {
        workers.emplace_back([&resourceMutex, &operationCount, operationsPerWorker]() {
            for (int j = 0; j < operationsPerWorker; ++j) {
                ScopedTimedLock lock(resourceMutex, std::chrono::milliseconds(100), "perf_test");
...
int main() {
    ...
    try {
        ...
        for (const auto& [lockName, stats] : allStats) {
            ...
            std::cout << "  - Acquisitions: " << stats.acquisitions.load() << std::endl;
            std::cout << "  - Failures: " << stats.failures.load() << std::endl;
            std::cout << "  - Avg wait time: " << stats.getAverageWaitTime() << "μs" << std::endl;
            std::cout << "  - Failure rate: " << (stats.getFailureRate() * 100) << "%" << std::endl;
        }
