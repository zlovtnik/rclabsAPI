...
public:
    MOCK_METHOD(void, logInfo, (const std::string& component, const std::string& message), (override));
    MOCK_METHOD(void, logWarning, (const std::string& component, const std::string& message), (override));
    MOCK_METHOD(void, logError, (const std::string& component, const std::string& message), (override));
    MOCK_METHOD(void, logDebug, (const std::string& component, const std::string& message), (override));
};
...
TEST_F(NotificationServiceTest, Service_SystemErrorAlert) {
    ...
    service_->sendSystemErrorAlert("DatabaseManager", "Connection pool exhausted");
...
TEST_F(NotificationServiceTest, Service_QueueSizeLimit) {
    ...
    EXPECT_CALL(*mockLogger_, logWarning(_, HasSubstr("queue full"))).Times(AtLeast(1));
...
TEST_F(NotificationServiceTest, LogDelivery_BasicFunctionality) {
    ...
    EXPECT_CALL(*logger, logWarning("NotificationService", HasSubstr("Test Subject")));
...
TEST_F(NotificationServiceTest, Performance_HighVolumeNotifications) {
    ...
    for (int i = 0; i < numNotifications; ++i) {
        service_->sendJobFailureAlert("job_" + std::to_string(i), "Error " + std::to_string(i));
    }
