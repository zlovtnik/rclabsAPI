...
void testRealTimeStreaming() {
    ...
    for (const auto& msg : messages) {
        std::cout << "Message: jobId=" << msg.jobId << ", level=" << msg.logLevel << std::endl;
        if (msg.jobId == "job_123" && msg.logLevel.find("INFO") != std::string::npos) {
            foundInfoMessage = true;
        }
        if (msg.jobId == "job_456" && msg.logLevel.find("ERROR") != std::string::npos) {
...
void testStreamingWithFiltering() {
    ...
    logger.infoForJob("TestComponent", "Message for filtered job", "filtered_job");
    
    // Give time for messages to be processed
    std::this_thread::sleep_for(std::chrono::milliseconds(300));
    
    auto messages = mockWsManager->getReceivedMessages();
    std::cout << "Received " << messages.size() << " messages with filtering" << std::endl;
...
void testStreamingPerformance() {
    ...
    for (int i = 0; i < messageCount; i++) {
        logger.infoForJob("TestComponent", "Performance test message " + std::to_string(i), "perf_job");
    }
    
    auto endTime = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime);
    
    std::cout << "Generated " << messageCount << " log messages in " << duration.count() << "ms" << std::endl;
    
    // Give time for all messages to be processed
    std::this_thread::sleep_for(std::chrono::milliseconds(500));
    
    size_t receivedCount = mockWsManager->getMessageCount();
    std::cout << "Received " << receivedCount << " messages via WebSocket" << std::endl;
...
int main() {
    std::cout << "Starting Logger Real-time Streaming Integration Tests..." << std::endl;
    ...
    try {
        ...
        std::cout << "\n🎉 All log streaming integration tests passed!" << std::endl;
