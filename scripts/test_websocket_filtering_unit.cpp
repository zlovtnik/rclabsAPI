...
private:
    ...
    void testConnectionFiltersBasics() {
        ...
        assert(filters.shouldReceiveMessageType(MessageType::JOB_STATUS_UPDATE) == true);
        assert(filters.shouldReceiveMessageType(MessageType::JOB_LOG_MESSAGE) == false);
        ...
        assert(filters.shouldReceiveJob("job_123") == false);  // No longer has job_123, only job_456
        assert(filters.shouldReceiveJob("job_456") == true);   // Still has job_456
        assert(filters.shouldReceiveMessageType(MessageType::JOB_STATUS_UPDATE) == true);  // Empty = all
    ...
    void testConnectionFiltersEnhanced() {
        ...
        filters.addMessageType(MessageType::JOB_STATUS_UPDATE);
        filters.addMessageType(MessageType::JOB_STATUS_UPDATE);  // Should not add duplicate
        ...
        assert(parsedFilters.includeSystemNotifications == filters.includeSystemNotifications);
    ...
    void testWebSocketConnectionFilterMethods() {
        ...
        // Note: This test would require mock WebSocket connections in a real scenario
        // For now, we'll test the logic without actual network connections
    ...
    void testWebSocketManagerFilterMethods() {
        ...
        auto typeConnections = wsManager->getConnectionsForMessageType(MessageType::JOB_STATUS_UPDATE);
        assert(typeConnections.empty());
        std::cout << "  ✓ getConnectionsForMessageType returns empty for no connections\n";
        
        auto levelConnections = wsManager->getConnectionsForLogLevel("ERROR");
        assert(levelConnections.empty());
        std::cout << "  ✓ getConnectionsForLogLevel returns empty for no connections\n";
    ...
    void testWebSocketFilterManagerBasics() {
        ...
        bool loaded = filterManager->loadFilterTemplate("error-only", templateFilters);
        assert(loaded == true);
    ...
    void testWebSocketFilterManagerAdvanced() {
        ...
        bool loaded = filterManager->loadConnectionPreferences("test_connection", loadedFilters);
        assert(loaded == true);
        ...
        loaded = filterManager->loadConnectionPreferences("test_connection", loadedFilters);
        assert(loaded == false);
        ...
        loaded = filterManager->loadFilterTemplate("custom-template", loadedTemplate);
        assert(loaded == true);
    ...
    void testFilterTemplates() {
        ...
        for (const auto& expectedTemplate : expectedTemplates) {
            bool found = std::find(availableTemplates.begin(), availableTemplates.end(), 
        ...
        std::cout << "  ✓ All default templates load correctly with expected content\n";
    ...
    void testFilterStatistics() {
        ...
        http::response<http::string_body> response = filterManager->handleGetFilterStatistics();
    ...
    void testBatchOperations() {
        ...
        filterManager->applyFiltersToMultipleConnections(connectionIds, batchFilters);
        std::cout << "  ✓ Batch filter application handles non-existent connections\n";
    ...
    void testErrorHandling() {
        ...
        auto response = filterManager->handleGetConnectionFilters("non_existent_connection");
        assert(response.result() == http::status::not_found);
        std::cout << "  ✓ Non-existent connection returns 404\n";
        
        // Test invalid filter data
        response = filterManager->handleSetConnectionFilters("test_connection", "invalid_json");
        assert(response.result() == http::status::not_found || response.result() == http::status::bad_request);
        std::cout << "  ✓ Invalid JSON returns appropriate error\n";
        
        // Test loading non-existent template
        ConnectionFilters filters;
        bool loaded = filterManager->loadFilterTemplate("non_existent_template", filters);
        assert(loaded == false);
    ...
    void testPerformance() {
        ...
        for (int i = 0; i < 1000; ++i) {
            ...
            filters.addLogLevel("ERROR");
            filterManager->saveFilterTemplate("template_" + std::to_string(i), filters);
        }
        ...
        std::cout << "  ✓ Saving 1000 filter templates completed in " << duration.count() << "ms\n";
        ...
        for (int i = 0; i < 1000; ++i) {
            ConnectionFilters filters;
            filterManager->loadFilterTemplate("template_" + std::to_string(i), filters);
        }
        ...
        std::cout << "  ✓ Loading 1000 filter templates completed in " << duration.count() << "ms\n";
        ...
        std::cout << "  ✓ 10000 filter matches completed in " << duration.count() << "ms\n";
