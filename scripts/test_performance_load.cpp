...
struct PerformanceMetrics {
    ...
    void printSummary() const {
        ...
        std::cout << "  Job Success Rate: " << (jobsCompleted.load() * 100.0 / std::max(1UL, jobsCreated.load())) << "%" << std::endl;
        std::cout << "  Job Throughput: " << (jobsCompleted.load() / std::max(1.0, durationSec)) << " jobs/sec" << std::endl;
        
        std::cout << "\nWebSocket Metrics:" << std::endl;
        std::cout << "  Connections Created: " << wsConnectionsCreated.load() << std::endl;
        std::cout << "  Connections Dropped: " << wsConnectionsDropped.load() << std::endl;
        std::cout << "  Messages Sent: " << messagesSent.load() << std::endl;
        std::cout << "  Messages Received: " << messagesReceived.load() << std::endl;
        std::cout << "  Message Throughput: " << (messagesSent.load() / std::max(1.0, durationSec)) << " msg/sec" << std::endl;
        std::cout << "  Max Concurrent Connections: " << maxWsConnections << std::endl;
        
        std::cout << "\nNotification Metrics:" << std::endl;
        std::cout << "  Notifications Sent: " << notificationsSent.load() << std::endl;
        std::cout << "  Notifications Failed: " << notificationsFailed.load() << std::endl;
        std::cout << "  Notification Success Rate: " << (notificationsSent.load() * 100.0 / std::max(1UL, notificationsSent.load() + notificationsFailed.load())) << "%" << std::endl;
        std::cout << "  Max Notification Queue: " << maxNotificationQueue << std::endl;
        
        std::cout << "\nResource Metrics:" << std::endl;
        std::cout << "  Peak Memory Usage: " << peakMemoryUsageMB << " MB" << std::endl;
        std::cout << "  Peak CPU Usage: " << (peakCpuUsage * 100.0) << "%" << std::endl;
    ...
    void saveToFile(const std::string& filename) const {
        ...
        file << "job_throughput_per_sec," << (jobsCompleted.load() / std::max(1.0, durationSec)) << std::endl;
        file << "ws_connections_created," << wsConnectionsCreated.load() << std::endl;
        file << "ws_connections_dropped," << wsConnectionsDropped.load() << std::endl;
        file << "messages_sent," << messagesSent.load() << std::endl;
        file << "messages_received," << messagesReceived.load() << std::endl;
        file << "message_throughput_per_sec," << (messagesSent.load() / std::max(1.0, durationSec)) << std::endl;
...
public:
    ...
    bool runAllPerformanceTests() {
        std::cout << "\n=== Starting Comprehensive Performance Test Suite ===" << std::endl;
        
        if (!initializeSystem()) {
            std::cerr << "Failed to initialize system for performance testing" << std::endl;
            return false;
        }
        ...
        if (!runLoadTest("Light Load", config_.lightLoadJobs, config_.lightLoadConnections, config_.shortTestDuration)) {
        ...
        if (!runLoadTest("Medium Load", config_.mediumLoadJobs, config_.mediumLoadConnections, config_.mediumTestDuration)) {
        ...
        if (!runLoadTest("Heavy Load", config_.heavyLoadJobs, config_.heavyLoadConnections, config_.mediumTestDuration)) {
...
private:
    bool initializeSystem() {
        std::cout << "Initializing system components for performance testing..." << std::endl;
        ...
        std::cout << "System initialized successfully for performance testing" << std::endl;
        return true;
    }
    ...
    void cleanupSystem() {
        ...
        if (etlManager_) etlManager_->stop();
        if (jobMonitor_) jobMonitor_->stop();
        if (wsManager_) wsManager_->stop();
        if (notificationService_) notificationService_->stop();
        
        std::cout << "System cleanup completed" << std::endl;
    }
    
    bool runLoadTest(const std::string& testName, int numJobs, int numConnections, int durationSeconds) {
        std::cout << "Running " << testName << " test..." << std::endl;
        std::cout << "  Jobs: " << numJobs << ", Connections: " << numConnections << ", Duration: " << durationSeconds << "s" << std::endl;
        ...
        auto resourceMonitor = std::async(std::launch::async, [this]() {
            monitorResources();
        });
        
        // Start WebSocket activity simulation
        auto wsActivitySimulator = std::async(std::launch::async, [this, &clients]() {
        ...
        // Validate results
        bool testPassed = validateLoadTestResults(testName, numJobs, numConnections);
        
        std::cout << testName << " Test: " << (testPassed ? "PASSED" : "FAILED") << std::endl;
        return testPassed;
    }
    ...
    bool runBurstLoadTest() {
        ...
        auto burstCreator = std::async(std::launch::async, [this, BURST_SIZE, NUM_BURSTS, BURST_INTERVAL]() {
            for (int burst = 0; burst < NUM_BURSTS; ++burst) {
                std::cout << "Creating burst " << (burst + 1) << " of " << NUM_BURSTS << std::endl;
        ...
        double failureRate = static_cast<double>(metrics_.jobsFailed.load()) / metrics_.jobsCreated.load();
        bool testPassed = failureRate < 0.1; // Less than 10% failure rate
        
        std::cout << "Burst Load Test: " << (testPassed ? "PASSED" : "FAILED") << std::endl;
        return testPassed;
    }
    
    bool runSustainedLoadTest() {
        std::cout << "Running sustained load test (continuous operation)..." << std::endl;
        ...
        auto jobCreator = std::async(std::launch::async, [this, SUSTAINED_DURATION, JOBS_PER_MINUTE]() {
            auto endTime = std::chrono::steady_clock::now() + std::chrono::seconds(SUSTAINED_DURATION);
            int jobCounter = 0;
        ...
        auto healthMonitor = std::async(std::launch::async, [this, SUSTAINED_DURATION]() {
            auto endTime = std::chrono::steady_clock::now() + std::chrono::seconds(SUSTAINED_DURATION);
            ...
            while (std::chrono::steady_clock::now() < endTime && testRunning_) {
                ...
                metrics_.maxWsConnections = std::max(metrics_.maxWsConnections, wsConnections);
                metrics_.maxNotificationQueue = std::max(metrics_.maxNotificationQueue, notificationQueue);
        ...
        bool testPassed = metrics_.maxActiveJobs < 1000 && // Reasonable job queue size
                         metrics_.maxNotificationQueue < 10000; // Reasonable notification queue
        
        std::cout << "Sustained Load Test: " << (testPassed ? "PASSED" : "FAILED") << std::endl;
        return testPassed;
    }
    ...
    bool runMemoryStressTest() {
        ...
        auto memoryMonitor = std::async(std::launch::async, [this]() {
            while (testRunning_) {
                ...
                double currentMemory = 100.0 + (metrics_.jobsCreated.load() * 0.5); // MB
                metrics_.peakMemoryUsageMB = std::max(metrics_.peakMemoryUsageMB, currentMemory);
        ...
        std::cout << "Memory Stress Test: " << (testPassed ? "PASSED" : "FAILED") << std::endl;
        return testPassed;
    }
    ...
    bool runConnectionStressTest() {
        ...
        std::cout << "Created " << clients.size() << " WebSocket connections" << std::endl;
        ...
        bool testPassed = metrics_.wsConnectionsCreated.load() >= MAX_CONNECTIONS * 0.9; // 90% success rate
        
        std::cout << "Connection Stress Test: " << (testPassed ? "PASSED" : "FAILED") << std::endl;
        return testPassed;
    }
    ...
    void processLoadTestJob(int jobId) {
        try {
            ...
            jobMonitor_->onJobStatusChanged(job->jobId, JobStatus::PENDING, JobStatus::RUNNING);
            
            // Random processing time
            std::random_device rd;
            std::mt19937 gen(rd());
            std::uniform_int_distribution<> timeDis(config_.jobProcessingTimeMin, config_.jobProcessingTimeMax);
            ...
            if (failureDis(gen) < config_.failureRate) {
                jobMonitor_->onJobStatusChanged(job->jobId, JobStatus::RUNNING, JobStatus::FAILED);
                metrics_.jobsFailed++;
            } else {
                jobMonitor_->onJobStatusChanged(job->jobId, JobStatus::RUNNING, JobStatus::COMPLETED);
    ...
    void processMemoryIntensiveJob(int jobId) {
        try {
            ...
            for (int i = 0; i < NUM_BLOCKS; ++i) {
                ...
                jobMonitor_->onJobProgressUpdated(job->jobId, progress, "Allocating memory block " + std::to_string(i + 1));
    ...
    void simulateWebSocketActivity(const std::vector<std::unique_ptr<LoadTestWebSocketClient>>& clients) {
        std::random_device rd;
        std::mt19937 gen(rd());
    ...
    bool validateLoadTestResults(const std::string& testName, int expectedJobs, int expectedConnections) {
        // Basic validation criteria
        double jobSuccessRate = static_cast<double>(metrics_.jobsCompleted.load()) / 
                               std::max(1UL, metrics_.jobsCreated.load());
        
        double connectionSuccessRate = static_cast<double>(metrics_.wsConnectionsCreated.load()) / 
                                     std::max(1, expectedConnections);
        
        bool jobsValid = jobSuccessRate >= 0.8; // At least 80% job success rate
        bool connectionsValid = connectionSuccessRate >= 0.9; // At least 90% connection success rate
        bool noSystemFailures = metrics_.maxNotificationQueue < 10000; // Reasonable queue size
        
        std::cout << "Validation Results for " << testName << ":" << std::endl;
        std::cout << "  Job Success Rate: " << (jobSuccessRate * 100.0) << "% " << (jobsValid ? "✓" : "✗") << std::endl;
        std::cout << "  Connection Success Rate: " << (connectionSuccessRate * 100.0) << "% " << (connectionsValid ? "✓" : "✗") << std::endl;
        std::cout << "  System Stability: " << (noSystemFailures ? "✓" : "✗") << std::endl;
...
int main() {
    ...
    std::cout << "\nTotal test suite execution time: " << totalDuration.count() << " minutes" << std::endl;
    
    if (success) {
        std::cout << "\n🎉 ALL PERFORMANCE TESTS PASSED! 🎉" << std::endl;
        std::cout << "The system demonstrates excellent performance under various load conditions." << std::endl;
        return 0;
    } else {
        std::cout << "\n❌ SOME PERFORMANCE TESTS FAILED" << std::endl;
        std::cout << "Please review the test results and optimize system performance." << std::endl;
        return 1;
    }
}