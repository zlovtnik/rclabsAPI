...
public:
    ...
    void sendJobTimeoutWarning(const std::string& jobId, int executionTimeMinutes) override {
        std::lock_guard<std::mutex> lock(mutex_);
        timeout_warnings++;
...
public:
    ...
    std::string receiveMessage(std::chrono::milliseconds timeout = std::chrono::milliseconds(5000)) {
...
TEST_F(RealTimeMonitoringWorkflowTest, CompleteJobLifecycle) {
    ...
    std::string message = wsClient.receiveMessage(std::chrono::milliseconds(2000));
    ASSERT_NE(message, "TIMEOUT") << "Timeout waiting for initial job status message";
    ASSERT_NE(message.substr(0, 6), "ERROR:") << "Error receiving message: " << message;
    ...
    std::string restResponse = httpClient.get("127.0.0.1", std::to_string(server_port), 
                                             "/api/jobs/" + jobId + "/status");
    ASSERT_NE(restResponse.substr(0, 6), "ERROR:") << "REST API call failed: " << restResponse;
    ...
    ASSERT_NE(message.substr(0, 6), "ERROR:") << "Error receiving progress message: " << message;
    ...
    ASSERT_NE(message.substr(0, 6), "ERROR:") << "Error receiving completion message: " << message;
...
TEST_F(RealTimeMonitoringWorkflowTest, MultiClientTest) {
    ...
    for (int i = 0; i < numClients; ++i) {
        std::string message = clients[i]->receiveMessage(std::chrono::milliseconds(2000));
        ASSERT_NE(message, "TIMEOUT") << "Client " << i << " timed out waiting for message";
        ASSERT_NE(message.substr(0, 6), "ERROR:") << "Client " << i << " received error: " << message;
    ...
    for (int i = 0; i < numClients; ++i) {
        std::string message = clients[i]->receiveMessage(std::chrono::milliseconds(2000));
        ASSERT_NE(message, "TIMEOUT") << "Client " << i << " timed out waiting for progress update";
...
TEST_F(RealTimeMonitoringWorkflowTest, EndToEndMonitoringAndNotificationFlow) {
    ...
    std::string restResponse = httpClient.get("127.0.0.1", std::to_string(server_port), 
                                             "/api/monitor/jobs");
    ASSERT_NE(restResponse.substr(0, 6), "ERROR:") << "Failed to get jobs list: " << restResponse;
    ...
    for (size_t i = 0; i < jobIds.size(); ++i) {
        etl_manager->updateJobProgress(jobIds[i], (i + 1) * 30, "Processing step " + std::to_string(i + 1));
...
int main(int argc, char **argv) {
    ...
    std::cout << "Starting Real-time Monitoring Workflow Integration Tests..." << std::endl;
    std::cout << "These tests verify complete job lifecycle with real-time WebSocket updates," << std::endl;
    std::cout << "REST API integration, multi-client scenarios, and notification flow." << std::endl;
    ...
    if (result == 0) {
        std::cout << "\n🎉 All real-time monitoring workflow integration tests passed!" << std::endl;
        std::cout << "✅ Complete job lifecycle with WebSocket updates: VERIFIED" << std::endl;
        std::cout << "✅ REST API integration with monitoring data: VERIFIED" << std::endl;
        std::cout << "✅ Multi-client concurrent WebSocket connections: VERIFIED" << std::endl;
        std::cout << "✅ End-to-end job monitoring and notification flow: VERIFIED" << std::endl;
    } else {
        std::cerr << "\n❌ Some integration tests failed. Check the output above for details." << std::endl;
    }
