...
public:
    void runAllTests() {
        std::cout << "=== PooledSession Performance Integration Test Suite ===" << std::endl;
        ...
        std::cout << "=== All PooledSession Performance Integration Tests Passed ===" << std::endl;
    }

private:
    void testSessionWithPerformanceMonitor() {
        std::cout << "Testing PooledSession with PerformanceMonitor..." << std::endl;
        ...
        boost::asio::local::stream_protocol::socket::connect_pair(local_socket1, local_socket2);
        ...
        // Note: This test is simplified since we can't easily create a real TCP connection
        // In a real scenario, the session would be created with a proper TCP socket
        
        std::cout << "✓ PooledSession with PerformanceMonitor creation test passed" << std::endl;
    }
    
    void testSessionWithoutPerformanceMonitor() {
        std::cout << "Testing PooledSession without PerformanceMonitor..." << std::endl;
        ...
        // This test verifies that PooledSession works correctly without a performance monitor
        // The session should handle null performance monitor gracefully
        
        std::cout << "✓ PooledSession without PerformanceMonitor test passed" << std::endl;
    }
    ...
    void testTimeoutRecording() {
        ...
        performanceMonitor->recordTimeout(PerformanceMonitor::TimeoutType::CONNECTION);
    ...
    void testRequestTimingAccuracy() {
        ...
        assert(metrics.averageResponseTime.load() <= 100.0); // At most 100ms (allowing for overhead)
...
public:
    void runAllTests() {
        std::cout << "=== ConnectionPool Performance Integration Test Suite ===" << std::endl;
        
        testConnectionReuseTracking();
        testNewConnectionTracking();
        testMetricsIntegration();
        
        std::cout << "=== All ConnectionPool Performance Integration Tests Passed ===" << std::endl;
    }
...
private:
    void testConnectionReuseTracking() {
        ...
        assert(metrics.connectionReuseRate == 0.5); // 1 reuse out of 2 total connections
    ...
    void testMetricsIntegration() {
        ...
        for (int i = 0; i < 20; ++i) {
            ...
            for (int j = 0; j < 2; ++j) {
                performanceMonitor->recordRequestStart();
                performanceMonitor->recordRequestEnd(std::chrono::milliseconds(25 + j * 10));
            }
        ...
        assert(metrics.connectionReuses.load() > 0);
        assert(metrics.totalRequests.load() == 40); // 20 connections * 2 requests each
        assert(metrics.activeRequests.load() == 0);
        assert(metrics.averageResponseTime.load() > 0.0);
...
int main() {
    try {
        ...
        std::cout << "Demonstrating integrated performance monitoring..." << std::endl;
        ...
        for (int i = 0; i < 50; ++i) {
            ...
            performanceMonitor->recordRequestEnd(std::chrono::milliseconds(processingTime));
        ...
        std::cout << "P95 Response Time: " << performanceMonitor->getPercentileResponseTime(0.95).count() << " ms" << std::endl;
        std::cout << "P99 Response Time: " << performanceMonitor->getPercentileResponseTime(0.99).count() << " ms" << std::endl;
