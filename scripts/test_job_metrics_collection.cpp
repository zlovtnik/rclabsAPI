...
TEST_F(JobMetricsCollectorTest, RealTimeUpdates) {
    ...
    auto callback = [&](const std::string& jobId, const JobMetricsCollector::MetricsSnapshot& snapshot) {
        callbackCalled = true;
        callbackJobId = jobId;
    ...
    collector->setUpdateInterval(std::chrono::milliseconds(100)); // Fast updates for testing
