...
void testJobMetricsSerialization() {
    ...
    assertTrue(deserializedMetrics.recordsProcessed == 1000, "Deserialized recordsProcessed");
    assertTrue(deserializedMetrics.recordsSuccessful == 950, "Deserialized recordsSuccessful");
    assertTrue(deserializedMetrics.recordsFailed == 50, "Deserialized recordsFailed");
    assertTrue(std::abs(deserializedMetrics.processingRate - 125.75) < 0.01, "Deserialized processingRate");
    assertTrue(deserializedMetrics.memoryUsage == 1024000, "Deserialized memoryUsage");
    assertTrue(std::abs(deserializedMetrics.cpuUsage - 85.5) < 0.01, "Deserialized cpuUsage");
    assertTrue(deserializedMetrics.executionTime.count() == 30000, "Deserialized executionTime");
}
...
void testJobStatusUpdateSerialization() {
    ...
    assertTrue(json.find("\"currentStep\":\"Processing batch 3/4\"") != std::string::npos, 
    ...
    assertTrue(deserializedUpdate.status == JobStatus::RUNNING, "Deserialized status");
    assertTrue(deserializedUpdate.previousStatus == JobStatus::PENDING, "Deserialized previousStatus");
    assertTrue(deserializedUpdate.progressPercent == 75, "Deserialized progressPercent");
    assertEqual("Processing batch 3/4", deserializedUpdate.currentStep, "Deserialized currentStep");
    assertTrue(deserializedUpdate.metrics.recordsProcessed == 750, "Deserialized metrics");
    
    // Test helper methods
    assertTrue(update.isStatusChange(), "JobStatusUpdate isStatusChange");
    assertFalse(update.isProgressUpdate(), "JobStatusUpdate isProgressUpdate (status changed)");
    
    // Test progress update (same status)
    update.previousStatus = JobStatus::RUNNING;
    assertFalse(update.isStatusChange(), "JobStatusUpdate isStatusChange (same status)");
...
void testJobStatusUpdateWithError() {
    ...
    assertTrue(json.find("\"errorMessage\":\"Database connection timeout\"") != std::string::npos, 
               "JobStatusUpdate JSON contains errorMessage");
    
    JobStatusUpdate deserializedUpdate = JobStatusUpdate::fromJson(json);
    assertTrue(deserializedUpdate.errorMessage.has_value(), "Deserialized errorMessage exists");
    assertEqual("Database connection timeout", deserializedUpdate.errorMessage.value(), 
...
void testJobMonitoringDataSerialization() {
    ...
    assertTrue(json.find("\"currentStep\":\"Transform phase\"") != std::string::npos, 
    ...
    assertEqual("running", data.getStatusString(), "JobMonitoringData getStatusString");
    assertEqual("full_etl", data.getJobTypeString(), "JobMonitoringData getJobTypeString");
    
    // Test deserialization
    JobMonitoringData deserializedData = JobMonitoringData::fromJson(json);
    assertEqual("monitoring_job_789", deserializedData.jobId, "Deserialized jobId");
    assertTrue(deserializedData.jobType == JobType::FULL_ETL, "Deserialized jobType");
    assertTrue(deserializedData.status == JobStatus::RUNNING, "Deserialized status");
    assertTrue(deserializedData.progressPercent == 60, "Deserialized progressPercent");
}
...
void testLogMessageSerialization() {
    ...
    assertTrue(json.find("\"component\":\"DataTransformer\"") != std::string::npos, 
               "LogMessage JSON contains component");
    assertTrue(json.find("\"message\":\"Failed to parse record: invalid format\"") != std::string::npos, 
    ...
    assertTrue(logMsg.matchesFilter("log_test_job", "ERROR"), "LogMessage matches exact filters");
    assertTrue(logMsg.matchesFilter("", "ERROR"), "LogMessage matches level filter only");
    assertTrue(logMsg.matchesFilter("log_test_job", ""), "LogMessage matches job filter only");
    assertTrue(logMsg.matchesFilter("", ""), "LogMessage matches no filters");
    assertFalse(logMsg.matchesFilter("other_job", "ERROR"), "LogMessage doesn't match wrong job");
    assertFalse(logMsg.matchesFilter("log_test_job", "INFO"), "LogMessage doesn't match wrong level");
    ...
    assertEqual("DataTransformer", deserializedMsg.component, "Deserialized component");
    assertEqual("Failed to parse record: invalid format", deserializedMsg.message, "Deserialized message");
}
...
void testWebSocketMessageSerialization() {
    ...
    assertTrue(logWsMessage.type == MessageType::JOB_LOG_MESSAGE, "Log message type");
    ...
    WebSocketMessage metricsMessage = WebSocketMessage::createMetricsUpdate("metrics_job", metrics);
    assertTrue(metricsMessage.type == MessageType::JOB_METRICS_UPDATE, "Metrics message type");
    
    WebSocketMessage errorMessage = WebSocketMessage::createErrorMessage("Test error");
    assertTrue(errorMessage.type == MessageType::ERROR_MESSAGE, "Error message type");
    assertTrue(errorMessage.data.find("Test error") != std::string::npos, "Error message data");
    
    WebSocketMessage ackMessage = WebSocketMessage::createConnectionAck();
    assertTrue(ackMessage.type == MessageType::CONNECTION_ACK, "Connection ack type");
}
...
void testConnectionFiltersSerialization() {
    ...
    filters.messageTypes = {MessageType::JOB_STATUS_UPDATE, MessageType::JOB_LOG_MESSAGE};
    filters.includeSystemNotifications = false;
    
    std::string json = filters.toJson();
    std::cout << "ConnectionFilters JSON: " << json << std::endl;
    
    assertTrue(json.find("\"jobIds\":[\"job1\",\"job2\",\"job3\"]") != std::string::npos, 
               "ConnectionFilters JSON contains jobIds");
    assertTrue(json.find("\"logLevels\":[\"ERROR\",\"WARN\"]") != std::string::npos, 
               "ConnectionFilters JSON contains logLevels");
    assertTrue(json.find("\"messageTypes\":[\"job_status_update\",\"job_log_message\"]") != std::string::npos, 
               "ConnectionFilters JSON contains messageTypes");
    assertTrue(json.find("\"includeSystemNotifications\":false") != std::string::npos, 
    ...
    assertTrue(filters.shouldReceiveLogLevel("ERROR"), "Should receive ERROR level");
    assertFalse(filters.shouldReceiveLogLevel("INFO"), "Should not receive INFO level");
    ...
    assertTrue(filters.shouldReceiveMessage(testMessage), "Should receive matching message");
    
    testMessage.targetJobId = "job4";
    assertFalse(filters.shouldReceiveMessage(testMessage), "Should not receive non-matching job");
    
    testMessage.type = MessageType::SYSTEM_NOTIFICATION;
    assertFalse(filters.shouldReceiveMessage(testMessage), "Should not receive system notifications");
    
    // Test deserialization
    ConnectionFilters deserializedFilters = ConnectionFilters::fromJson(json);
    assertTrue(deserializedFilters.jobIds.size() == 3, "Deserialized jobIds count");
    assertTrue(deserializedFilters.logLevels.size() == 2, "Deserialized logLevels count");
    assertTrue(deserializedFilters.messageTypes.size() == 2, "Deserialized messageTypes count");
    assertFalse(deserializedFilters.includeSystemNotifications, "Deserialized includeSystemNotifications");
}
...
void testUtilityFunctions() {
    ...
    assertEqual("job_status_update", messageTypeToString(MessageType::JOB_STATUS_UPDATE), 
                "MessageType to string conversion");
    assertTrue(stringToMessageType("job_status_update") == MessageType::JOB_STATUS_UPDATE, 
               "String to MessageType conversion");
    
    // Test job status conversion
    assertEqual("running", jobStatusToString(JobStatus::RUNNING), "JobStatus to string conversion");
    assertTrue(stringToJobStatus("running") == JobStatus::RUNNING, "String to JobStatus conversion");
    
    // Test job type conversion
    assertEqual("full_etl", jobTypeToString(JobType::FULL_ETL), "JobType to string conversion");
    assertTrue(stringToJobType("full_etl") == JobType::FULL_ETL, "String to JobType conversion");
    
    // Test JSON escaping
    assertEqual("Hello \\\"World\\\"", escapeJsonString("Hello \"World\""), "JSON string escaping");
    assertEqual("Line 1\\nLine 2", escapeJsonString("Line 1\nLine 2"), "JSON newline escaping");
    assertEqual("Tab\\tSeparated", escapeJsonString("Tab\tSeparated"), "JSON tab escaping");
...
void testTimestampHandling() {
    ...
    assertTrue(formatted.find('T') != std::string::npos, "Timestamp contains T separator");
    assertTrue(formatted.find('Z') != std::string::npos, "Timestamp contains Z suffix");
    assertTrue(formatted.find('.') != std::string::npos, "Timestamp contains milliseconds");
    
    // Test parsing (note: parsing may lose some precision due to timezone handling)
    ...
    assertTrue(std::abs(diff) < 86400, "Parsed timestamp is within reasonable range");
}
...
int main() {
    ...
    try {
        ...
        std::cout << "Job monitoring models implementation is working correctly!" << std::endl;
