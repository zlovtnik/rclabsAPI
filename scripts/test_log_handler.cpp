...
private:
    ...
    void testLogEntryWithJobIdAndContext() {
        ...
        LogEntry entry(LogLevel::ERROR, "TestComponent", "Test message", "job123", context);
    ...
    void testFileLogHandlerTextFormat() {
        ...
        FileLogHandler handler("test-file", filename, FileLogHandler::Format::TEXT, LogLevel::DEBUG);
    ...
    void testFileLogHandlerJsonFormat() {
        ...
        FileLogHandler handler("test-json", filename, FileLogHandler::Format::JSON, LogLevel::DEBUG);
    ...
    void testFileLogHandlerLevelFiltering() {
        ...
        FileLogHandler handler("test-filter", filename, FileLogHandler::Format::TEXT, LogLevel::WARN);
    ...
    void testFileLogHandlerContextHandling() {
        ...
        FileLogHandler handler("test-context", filename, FileLogHandler::Format::TEXT, LogLevel::DEBUG);
        
        std::unordered_map<std::string, std::string, TransparentStringHash, std::equal_to<>> context;
        context["user"] = "john";
        context["ip"] = "192.168.1.1";
        
        LogEntry entry(LogLevel::INFO, "TestComponent", "User action", "job456", context);
        handler.handle(entry);
        handler.flush();
    ...
    void testConsoleLogHandlerLevelFiltering() {
        ...
        ConsoleLogHandler handler("test-console-filter", true, true, LogLevel::ERROR);
    ...
    void testStreamingLogHandlerBasic() {
        ...
        // StreamingLogHandler tests are temporarily disabled due to WebSocketManager dependency
        // We'll enable this after integrating with the existing WebSocketManager
    ...
    void testLogHandlerUtilityMethods() {
        ...
        FileLogHandler handler("test-utils", testDir_ / "utils.log", FileLogHandler::Format::TEXT, LogLevel::DEBUG);
    ...
    void testJsonEscaping() {
        ...
        FileLogHandler handler("test-escape", filename, FileLogHandler::Format::JSON, LogLevel::DEBUG);
        
        // Test message with special characters that need escaping
        LogEntry entry(LogLevel::INFO, "TestComponent", "Message with \"quotes\" and \n newlines");
        handler.handle(entry);
        handler.flush();
    ...
    void testFileLogHandlerFileSize() {
        ...
        FileLogHandler handler("test-size", filename, FileLogHandler::Format::TEXT, LogLevel::DEBUG);
    ...
    void testHandlerShutdown() {
        ...
        FileLogHandler handler("test-shutdown", filename, FileLogHandler::Format::TEXT, LogLevel::DEBUG);
        ...
        // Note: We can't directly test if file is closed, but we can test that shutdown doesn't throw
        try {
