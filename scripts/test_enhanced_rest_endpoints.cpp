...
private:
    void createMockJobs() {
        ...
        // if available, or we'll modify the tests to work with the existing interface
    }
};
        ...
        job1->startedAt = std::chrono::system_clock::now() - std::chrono::hours(2) + std::chrono::minutes(5);
        ...
        job3->startedAt = std::chrono::system_clock::now() - std::chrono::hours(3) + std::chrono::minutes(2);
        job3->completedAt = std::chrono::system_clock::now() - std::chrono::hours(2) - std::chrono::minutes(30);
...
void testJobStatusEndpoint() {
    ...
    // Test valid job ID
    http::request<http::string_body> req{http::verb::get, "/api/jobs/job_001/status", 11};
...
void testJobMetricsEndpoint() {
    ...
    // Test valid job ID
    http::request<http::string_body> req{http::verb::get, "/api/jobs/job_001/metrics", 11};
    req.set(http::field::host, "localhost");
    req.set(http::field::content_type, "application/json");
...
void testMonitorJobsEndpoint() {
    ...
    http::request<http::string_body> req1{http::verb::get, "/api/monitor/jobs", 11};
    ...
    http::request<http::string_body> req2{http::verb::get, "/api/monitor/jobs?status=completed", 11};
    ...
    http::request<http::string_body> req3{http::verb::get, "/api/monitor/jobs?type=extract", 11};
    ...
    // Test with limit
    http::request<http::string_body> req4{http::verb::get, "/api/monitor/jobs?limit=2", 11};
...
void testInvalidJobId() {
    ...
    http::request<http::string_body> req{http::verb::get, "/api/jobs/nonexistent/status", 11};
    req.set(http::field::host, "localhost");
    req.set(http::field::content_type, "application/json");
...
int main() {
    ...
    try {
        ...
        std::cout << "Enhanced REST API endpoints are working correctly." << std::endl;
