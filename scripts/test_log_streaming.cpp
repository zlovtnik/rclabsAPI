...
void testQueueOverflow() {
    ...
    for (int i = 0; i < 20; i++) {
        logger.infoForJob("TestComponent", "Flood message " + std::to_string(i), "flood_job");
    }
    ...
    std::cout << "Messages dropped due to queue overflow: " << metrics.droppedMessages.load() << std::endl;
...
void testLogMessageCreation() {
    ...
    // Create a log message using the private method (we'll test through public interface)
    logger.infoForJob("TestComponent", "Test message with context", "test_job_json", context);
