...
void testLogMessageCreation() {
    std::cout << "\n=== Testing LogMessage Creation and Serialization ===" << std::endl;
    ...
    // Test deserialization (note: context parsing not implemented in current fromJson)
    ...
    std::cout << "✓ LogMessage creation and serialization test passed" << std::endl;
}
...
void testJobSpecificLogging() {
    ...
    logger.debugForJob("TestComponent", "Debug message for job", "job_debug", context);
    logger.infoForJob("TestComponent", "Info message for job", "job_info", context);
    logger.warnForJob("TestComponent", "Warning message for job", "job_warn", context);
    logger.errorForJob("TestComponent", "Error message for job", "job_error", context);
...
void testStreamingQueueManagement() {
    ...
    for (int i = 0; i < 10; i++) {
        logger.infoForJob("TestComponent", "Queue test message " + std::to_string(i), "queue_test_job");
    }
    ...
    // Since streaming is disabled, no messages should be dropped from streaming queue
