...
explicit SimpleTestHandler(const std::string &id) : id_(id) {}
   ...
   * formatted message to std::cout in the form: "[<id>] <component>: <message>".
   *
   * @param entry Log entry to handle; this uses entry.component and entry.message
 ...
 * @return true Always returns true, indicating the handler will handle every entry.
 */
 ...
 *
 * @return int The cumulative count of messages handled by this SimpleTestHandler since construction.
 */
 ...
 * messages handled by the registered handler and a short summary of achievements.
 *
