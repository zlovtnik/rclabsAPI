...
void demonstrateLogStreaming() {
    std::cout << "\n=== Log Streaming Functionality Demonstration ===" << std::endl;
    
    Logger& logger = Logger::getInstance();
    
    // Configure logger for demonstration (disable streaming to avoid WebSocket dependency)
    LogConfig config;
    ...
    logger.debugForJob("ETLJobManager", "Starting job validation", "job_001", context);
    logger.infoForJob("ETLJobManager", "Processing batch 1 of 5", "job_001", context);
    logger.warnForJob("ETLJobManager", "Found 3 invalid records", "job_001", context);
    logger.errorForJob("ETLJobManager", "Failed to process record ID 12345", "job_001", context);
    ...
    std::cout << "Added job filters for: important_job, critical_job" << std::endl;
    ...
    std::cout << "\n4. Testing LogMessage creation and serialization:" << std::endl;
    ...
    std::cout << "Matches job filter 'demo_job': " << (matchesJob ? "Yes" : "No") << std::endl;
    std::cout << "Matches level filter 'INFO': " << (matchesLevel ? "Yes" : "No") << std::endl;
    std::cout << "Matches both filters: " << (matchesBoth ? "Yes" : "No") << std::endl;
    std::cout << "Matches wrong filters: " << (matchesNeither ? "Yes" : "No") << std::endl;
    
    std::cout << "\n6. Testing logger metrics:" << std::endl;
    
    LogMetrics metrics = logger.getMetrics();
    std::cout << "Total messages logged: " << metrics.totalMessages.load() << std::endl;
    ...
    std::cout << "\n✓ Log streaming functionality demonstration completed!" << std::endl;
}
