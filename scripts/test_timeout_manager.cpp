...
TEST_F(TimeoutManagerTest, ConnectionTimeoutBasic) {
    ...
    auto callback = [&timeoutCalled, &timeoutType](std::shared_ptr<PooledSession> s, TimeoutType type) {
        timeoutCalled = true;
        timeoutType = type;
    };
    
    timeoutManager_->startConnectionTimeout(session, callback, std::chrono::seconds(1));
...
TEST_F(TimeoutManagerTest, RequestTimeoutBasic) {
    ...
    timeoutManager_->startRequestTimeout(session, callback, std::chrono::seconds(1));
...
TEST_F(TimeoutManagerTest, TimeoutCancellation) {
    ...
    auto callback = [&timeoutCalled](std::shared_ptr<PooledSession> s, TimeoutType type) {
        timeoutCalled = true;
    };
    
    timeoutManager_->startConnectionTimeout(session, callback, std::chrono::seconds(2));
    timeoutManager_->startRequestTimeout(session, callback, std::chrono::seconds(2));
...
TEST_F(TimeoutManagerTest, IndividualTimeoutCancellation) {
    ...
    auto callback = [&timeoutCount](std::shared_ptr<PooledSession> s, TimeoutType type) {
        timeoutCount++;
    };
...
TEST_F(TimeoutManagerTest, MultipleSessions) {
    ...
    timeoutManager_->startConnectionTimeout(session1, callback, std::chrono::seconds(1));
    timeoutManager_->startConnectionTimeout(session2, callback, std::chrono::seconds(1));
    timeoutManager_->startRequestTimeout(session1, callback, std::chrono::seconds(1));
...
TEST_F(TimeoutManagerTest, DefaultTimeoutCallback) {
    ...
    timeoutManager_->startConnectionTimeout(session, nullptr, std::chrono::seconds(1));
    EXPECT_EQ(timeoutManager_->getActiveConnectionTimers(), 1);
...
TEST_F(TimeoutManagerTest, CustomDefaultCallback) {
    ...
    auto customCallback = [&customCallbackCalled](std::shared_ptr<PooledSession> s, TimeoutType type) {
...
TEST_F(TimeoutManagerTest, CancelAllTimers) {
    ...
    timeoutManager_->startConnectionTimeout(session1, callback, std::chrono::seconds(2));
    timeoutManager_->startRequestTimeout(session1, callback, std::chrono::seconds(2));
    timeoutManager_->startConnectionTimeout(session2, callback, std::chrono::seconds(2));
    timeoutManager_->startRequestTimeout(session2, callback, std::chrono::seconds(2));
...
TEST_F(TimeoutManagerTest, CallbackExceptionHandling) {
    ...
    auto throwingCallback = [](std::shared_ptr<PooledSession> s, TimeoutType type) {
        throw std::runtime_error("Test exception");
    };
    
    timeoutManager_->startConnectionTimeout(session, throwingCallback, std::chrono::seconds(1));
