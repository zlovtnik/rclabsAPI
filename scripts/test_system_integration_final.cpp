...
private:
    ...
    public:
        ...
        void disconnect() {
            connected_ = false;
            std::cout << "Mock WebSocket client " << id_ << " disconnected" << std::endl;
        }
...
public:
    ...
    bool runFullIntegrationTest() {
        std::cout << "\n=== Starting Comprehensive System Integration Test ===" << std::endl;
...
private:
    bool initializeComponents() {
        ...
        requestHandler_ = std::make_shared<RequestHandler>(dbManager_, authManager_, etlManager_);
    ...
    bool runLoadTests() {
        ...
        std::cout << "✓ Created " << NUM_WEBSOCKET_CONNECTIONS << " mock WebSocket connections" << std::endl;
        ...
        std::cout << "✓ Launched " << NUM_CONCURRENT_JOBS << " concurrent ETL jobs" << std::endl;
        ...
        while (!allJobsCompleted && 
               std::chrono::steady_clock::now() - startTime < std::chrono::seconds(TEST_DURATION_SECONDS)) {
        ...
        return (jobsCompleted_.load() + jobsFailed_.load()) >= NUM_CONCURRENT_JOBS * 0.8; // 80% success rate
    }
    
    bool testErrorHandling() {
        std::cout << "\n--- Phase 5: Testing Error Handling and Recovery ---" << std::endl;
        
        // Test job failure handling
        std::cout << "Testing job failure scenarios..." << std::endl;
        
        // Create a job that will fail
        auto failingJob = etlManager_->createJob(JobType::DATA_IMPORT, "test_failing_job");
        if (failingJob) {
        ...
        notificationService_->sendSystemErrorAlert("TestComponent", "Test error for recovery testing");
    ...
    bool testResourceMonitoring() {
        ...
        notificationService_->checkConnectionLimit(95, 100); // Trigger connection limit alert
    ...
    bool testPerformanceAndStability() {
        std::cout << "\n--- Phase 7: Testing Performance and Stability ---" << std::endl;
        ...
        auto sustainedJobCreator = std::async(std::launch::async, [&]() {
            ...
            while (sustainedTestRunning) {
                auto job = etlManager_->createJob(JobType::DATA_EXPORT, "sustained_test_" + std::to_string(jobCounter++));
                if (job) {
        ...
        std::cout << "✓ Sustained load test completed. Jobs processed: " << sustainedJobsProcessed.load() << std::endl;
    ...
    bool cleanupAndValidate() {
        ...
        if (messagesReceived_.load() == 0) {
            std::cerr << "✗ No WebSocket messages were received during testing" << std::endl;
            validationPassed = false;
        }
    ...
    void simulateETLJob(int jobId) {
        try {
            ...
            for (int progress = 0; progress <= 100; progress += progressDis(gen)) {
                ...
                std::string step = "Processing batch " + std::to_string(progress / 10 + 1);
            ...
            std::cerr << "Job " << jobId << " failed with exception: " << e.what() << std::endl;
            jobsFailed_++;
        }
    }
    ...
    void printTestSummary() {
        ...
        std::cout << "Jobs completed: " << jobsCompleted_.load() << std::endl;
        std::cout << "Jobs failed: " << jobsFailed_.load() << std::endl;
        std::cout << "WebSocket messages: " << messagesReceived_.load() << std::endl;
        std::cout << "Notifications sent: " << notificationsSent_.load() << std::endl;
        std::cout << "Test duration: " << TEST_DURATION_SECONDS << " seconds" << std::endl;
        std::cout << "Concurrent jobs: " << NUM_CONCURRENT_JOBS << std::endl;
        std::cout << "WebSocket connections: " << NUM_WEBSOCKET_CONNECTIONS << std::endl;
        
        // Calculate performance metrics
        double jobThroughput = static_cast<double>(jobsCompleted_.load()) / TEST_DURATION_SECONDS;
        double messageThroughput = static_cast<double>(messagesReceived_.load()) / TEST_DURATION_SECONDS;
        
        std::cout << "Job throughput: " << jobThroughput << " jobs/second" << std::endl;
        std::cout << "Message throughput: " << messageThroughput << " messages/second" << std::endl;
...
int main() {
    ...
    std::cout << "\nTotal test execution time: " << duration.count() << " seconds" << std::endl;
