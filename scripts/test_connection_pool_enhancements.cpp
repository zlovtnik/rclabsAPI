...
public:
    ...
    void testThreadSafeAccess() {
        std::cout << "Testing thread-safe access to pool statistics..." << std::endl;
        ...
        for (int i = 0; i < 10; ++i) {
            futures.push_back(std::async(std::launch::async, [this, &successfulAccesses]() {
                try {
                    for (int j = 0; j < 100; ++j) {
    ...
    void testConfigurationValidation() {
        ...
        try {
            ...
            // Expected exception
            std::cout << "✓ Invalid min/max configuration correctly rejected" << std::endl;
        }
        ...
        try {
            ...
            // Expected exception
            std::cout << "✓ Invalid timeout configuration correctly rejected" << std::endl;
        }
    ...
    void runAllTests() {
        ...
        try {
            ...
            std::cout << "✗ Connection pool enhancement test failed: " << e.what() << std::endl;
            cleanup();
            throw;
        } catch (...) {
            std::cout << "✗ Connection pool enhancement test failed with unknown exception" << std::endl;
            cleanup();
            throw;
        }
    }
...
int main() {
    try {
        ...
        std::cerr << "Connection pool enhancement test suite failed: " << e.what() << std::endl;
        return 1;
    } catch (...) {
        std::cerr << "Connection pool enhancement test suite failed with unknown exception" << std::endl;
        return 1;
    }
}