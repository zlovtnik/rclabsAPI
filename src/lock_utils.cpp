...
void LockMonitor::recordAcquisition(const std::string &lockName,
  ...
  // Phase 1: Try to find existing entry with shared lock
  {
    ...
    if (it != lockStats_.end()) {
      ...
      if (detailedLogging_.load()) {
        std::cout << "[LockMonitor] Acquired lock '" << lockName << "' after "
                  << waitTime.count() << "μs" << std::endl;
      }
      return;
    }
  }
...
void LockMonitor::recordFailure(const std::string &lockName,
  ...
  if (detailedLogging_.load()) {
    std::cout << "[LockMonitor] Failed to acquire lock '" << lockName
              << "' after " << waitTime.count()
              << "μs (timeout: " << timeout.count() << "ms)" << std::endl;
  }
}

void LockMonitor::recordRelease(const std::string &lockName) {
  if (detailedLogging_.load()) {
    std::cout << "[LockMonitor] Released lock '" << lockName << "'"
              << std::endl;
  }
}

LockMonitor::LockStats
LockMonitor::getLockStats(const std::string &lockName) const {
  std::shared_lock<std::shared_mutex> lock(statsMutex_);
...
std::vector<std::pair<std::string, LockMonitor::LockStats>>
LockMonitor::getAllStats() const {
  std::shared_lock<std::shared_mutex> lock(statsMutex_);
...
void DeadlockDetector::registerSharedLockAcquisition(
    std::thread::id threadId, LockLevel level, const std::string &mutexId) {
  if (!enabled_.load())
    return;
...
void DeadlockDetector::unregisterSharedLockAcquisition(
    std::thread::id threadId, LockLevel level, const std::string &mutexId) {
  if (!enabled_.load())
    return;
...
void DeadlockDetector::validateLockOrdering(std::thread::id threadId,
                                            LockLevel newLevel) {
  auto it = threadLocks_.find(threadId);
  if (it == threadLocks_.end()) {
  ...
  for (const auto &[level, mutexId] : threadInfo.exclusiveLocks) {
    if (static_cast<int>(level) > static_cast<int>(newLevel)) {
      ...
      oss << "Lock ordering violation detected! Thread " << threadId
          ...
          << " (mutex: " << mutexId
          << ") and is trying to acquire lock at level "
  ...
  for (const auto &[threadId1, info1] : threadLocks_) {
    for (const auto &[threadId2, info2] : threadLocks_) {
      ...
      if (!info1.exclusiveLocks.empty() && !info2.exclusiveLocks.empty()) {
        ...
        auto max1 = std::max_element(info1.exclusiveLocks.begin(),
                                     info1.exclusiveLocks.end());
        auto min2 = std::min_element(info2.exclusiveLocks.begin(),
                                     info2.exclusiveLocks.end());

        if (max1 != info1.exclusiveLocks.end() &&
            min2 != info2.exclusiveLocks.end()) {
