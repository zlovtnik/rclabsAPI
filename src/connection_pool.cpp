...
ConnectionPool::ConnectionPool(const ConnectionPoolConfig &config)
    : config_(config) {
  WS_LOG_DEBUG("Connection pool created with max connections: " +
...
void ConnectionPool::addConnection(
  ...
  if (!connection) {
    ...
    throw etl::ETLException(etl::ErrorCode::INVALID_CONNECTION,
                            "Cannot add null connection to pool");
  }

  if (!running_.load()) {
    WS_LOG_WARN("Connection pool not running, cannot add connection");
    throw etl::ETLException(etl::ErrorCode::POOL_NOT_RUNNING,
  ...
  if (connections_.size() >= config_.maxConnections) {
    WS_LOG_WARN("Connection pool at maximum capacity: " +
                std::to_string(config_.maxConnections));
    throw etl::ETLException(etl::ErrorCode::POOL_CAPACITY_EXCEEDED,
...
ConnectionPool::getConnection(const std::string &connectionId) const {
...
ConnectionPool::getActiveConnections() const {
  SCOPED_SHARED_LOCK_TIMEOUT(connectionsMutex_, 500);
...
void ConnectionPool::removeInactiveConnections() {
  ...
  if (removedCount > 0) {
    WS_LOG_INFO("Removed " + std::to_string(removedCount) +
                " inactive connections from pool");
  }
}
...
void ConnectionPool::performHealthCheck() {
  ...
  WS_LOG_DEBUG(
      "Health check completed - Healthy: " + std::to_string(healthyCount) +
...
void ConnectionPool::scheduleHealthCheck() {
  ...
  if (!healthCheckTimer_) {
    // We need an io_context - this is a limitation, but for now we'll skip
    // automatic scheduling In a real implementation, this would be passed in or
    // managed externally
    WS_LOG_DEBUG(
        "Health check timer not available - manual health checks only");
    return;
  }

  healthCheckTimer_->expires_after(config_.healthCheckInterval);
  healthCheckTimer_->async_wait(
...
std::vector<std::shared_ptr<WebSocketConnection>>
ConnectionPool::getConnectionsByFilter(
...
void ConnectionPool::updateConfig(const ConnectionPoolConfig &newConfig) {
  ...
  if (connections_.size() > config_.maxConnections) {
    WS_LOG_WARN("Configuration update would exceed max connections. Current: " +
                ...
                ", New max: " + std::to_string(config_.maxConnections));
  }
...
void ConnectionPool::cleanupStaleConnections() {
  ...
  for (auto it = connections_.begin();
       it != connections_.end() && batchSize < config_.cleanupBatchSize;) {
    auto &connection = it->second;
  ...
  if (cleanedCount > 0) {
    WS_LOG_INFO("Cleaned up " + std::to_string(cleanedCount) +
                " stale connections");
  }
}
...
void ConnectionPool::forceCleanup(size_t maxToRemove) {
  ...
  for (auto it = connections_.begin();
       it != connections_.end() && removedCount < targetRemovals;) {
  ...
  if (removedCount > 0) {
    WS_LOG_INFO("Force cleaned up " + std::to_string(removedCount) +
                " connections");
  }
}

bool ConnectionPool::isConnectionStale(
