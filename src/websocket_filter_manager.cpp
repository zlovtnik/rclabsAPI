...
#define WS_FILTER_LOG_DEBUG(msg)                                               \
  Logger::getInstance().log(LogLevel::DEBUG, "WebSocketFilterManager", msg)
#define WS_FILTER_LOG_INFO(msg)                                                \
  Logger::getInstance().log(LogLevel::INFO, "WebSocketFilterManager", msg)
#define WS_FILTER_LOG_WARN(msg)                                                \
  Logger::getInstance().log(LogLevel::WARN, "WebSocketFilterManager", msg)
#define WS_FILTER_LOG_ERROR(msg)                                               \
  Logger::getInstance().log(LogLevel::ERROR, "WebSocketFilterManager", msg)

WebSocketFilterManager::WebSocketFilterManager(
...
WebSocketFilterManager::handleGetConnectionFilters(
  ...
  try {
    ...
    WS_FILTER_LOG_ERROR("Failed to get connection filters: " +
                        std::string(e.what()));
...
http::response<http::string_body>
WebSocketFilterManager::handleSetConnectionFilters(
    const std::string &connectionId, const std::string &requestBody) {
  ...
  try {
    ...
    return createSuccessResponse(
        R"({"status":"success","message":"Filters updated successfully"})");

  } catch (const std::exception &e) {
    WS_FILTER_LOG_ERROR("Failed to set connection filters: " +
                        std::string(e.what()));
    return createErrorResponse(http::status::bad_request,
...
http::response<http::string_body>
WebSocketFilterManager::handleUpdateConnectionFilters(
    const std::string &connectionId, const std::string &requestBody) {
  ...
  try {
    ...
    WS_FILTER_LOG_ERROR("Failed to update connection filters: " +
                        std::string(e.what()));
    return createErrorResponse(http::status::bad_request,
                               "Failed to parse filter data: " +
                                   std::string(e.what()));
...
http::response<http::string_body>
WebSocketFilterManager::handleAddJobFilter(const std::string &connectionId,
                                           const std::string &jobId) {
  WS_FILTER_LOG_DEBUG("Adding job filter '" + jobId +
  ...
  try {
    ...
    ConnectionFilters currentFilters =
        wsManager_->getConnectionFilters(connectionId);
    saveConnectionPreferences(connectionId, currentFilters);

    WS_FILTER_LOG_INFO("Job filter '" + jobId +
                       "' added to connection: " + connectionId);
    return createSuccessResponse(
        R"({"status":"success","message":"Job filter added successfully"})");
...
http::response<http::string_body>
WebSocketFilterManager::handleRemoveJobFilter(const std::string &connectionId,
                                              const std::string &jobId) {
  WS_FILTER_LOG_DEBUG("Removing job filter '" + jobId +
  ...
  try {
    ...
    WS_FILTER_LOG_INFO("Job filter '" + jobId +
                       "' removed from connection: " + connectionId);
    return createSuccessResponse(
        R"({"status":"success","message":"Job filter removed successfully"})");

  } catch (const std::exception &e) {
    WS_FILTER_LOG_ERROR("Failed to remove job filter: " +
                        std::string(e.what()));
    return createErrorResponse(http::status::internal_server_error,
...
http::response<http::string_body>
WebSocketFilterManager::handleAddMessageTypeFilter(
    const std::string &connectionId, const std::string &messageType) {
  WS_FILTER_LOG_DEBUG("Adding message type filter '" + messageType +
                      "' to connection: " + connectionId);
  ...
  try {
    ...
    WS_FILTER_LOG_INFO("Message type filter '" + messageType +
                       "' added to connection: " + connectionId);
    return createSuccessResponse(
        R"({"status":"success","message":"Message type filter added successfully"})");

  } catch (const std::exception &e) {
    WS_FILTER_LOG_ERROR("Failed to add message type filter: " +
                        std::string(e.what()));
    return createErrorResponse(http::status::internal_server_error,
...
http::response<http::string_body>
WebSocketFilterManager::handleRemoveMessageTypeFilter(
    const std::string &connectionId, const std::string &messageType) {
  WS_FILTER_LOG_DEBUG("Removing message type filter '" + messageType +
                      "' from connection: " + connectionId);
  ...
  try {
    ...
    WS_FILTER_LOG_INFO("Message type filter '" + messageType +
                       "' removed from connection: " + connectionId);
    return createSuccessResponse(
        R"({"status":"success","message":"Message type filter removed successfully"})");

  } catch (const std::exception &e) {
    WS_FILTER_LOG_ERROR("Failed to remove message type filter: " +
                        std::string(e.what()));
    return createErrorResponse(http::status::internal_server_error,
...
WebSocketFilterManager::handleAddLogLevelFilter(const std::string &connectionId,
  ...
  WS_FILTER_LOG_DEBUG("Adding log level filter '" + logLevel +
                      "' to connection: " + connectionId);
  ...
  try {
    ...
    WS_FILTER_LOG_INFO("Log level filter '" + logLevel +
                       "' added to connection: " + connectionId);
    return createSuccessResponse(
        R"({"status":"success","message":"Log level filter added successfully"})");

  } catch (const std::exception &e) {
    WS_FILTER_LOG_ERROR("Failed to add log level filter: " +
                        std::string(e.what()));
    return createErrorResponse(http::status::internal_server_error,
...
http::response<http::string_body>
WebSocketFilterManager::handleRemoveLogLevelFilter(
    const std::string &connectionId, const std::string &logLevel) {
  WS_FILTER_LOG_DEBUG("Removing log level filter '" + logLevel +
                      "' from connection: " + connectionId);
  ...
  try {
    ...
    WS_FILTER_LOG_INFO("Log level filter '" + logLevel +
                       "' removed from connection: " + connectionId);
    return createSuccessResponse(
        R"({"status":"success","message":"Log level filter removed successfully"})");

  } catch (const std::exception &e) {
    WS_FILTER_LOG_ERROR("Failed to remove log level filter: " +
                        std::string(e.what()));
    return createErrorResponse(http::status::internal_server_error,
...
http::response<http::string_body>
WebSocketFilterManager::handleClearConnectionFilters(
    const std::string &connectionId) {
  ...
  try {
    ...
    return createSuccessResponse(
        R"({"status":"success","message":"All filters cleared successfully"})");

  } catch (const std::exception &e) {
    WS_FILTER_LOG_ERROR("Failed to clear connection filters: " +
                        std::string(e.what()));
    return createErrorResponse(http::status::internal_server_error,
...
WebSocketFilterManager::handleGetConnectionStats() {
  ...
  try {
    ...
    WS_FILTER_LOG_ERROR("Failed to get connection statistics: " +
                        std::string(e.what()));
    return createErrorResponse(http::status::internal_server_error,
...
http::response<http::string_body>
WebSocketFilterManager::handleTestConnectionFilter(
    const std::string &connectionId, const std::string &requestBody) {
  ...
  try {
    ...
    bool shouldReceive =
        wsManager_->testConnectionFilter(connectionId, testMessage);

    std::ostringstream json;
    json << R"({"connectionId":")" << connectionId << R"(",)"
         << R"("shouldReceive":)" << (shouldReceive ? "true" : "false")
         << R"(,)"
         << R"("messageType":")" << messageTypeToString(testMessage.type)
         << R"(")"
         << "}";
    ...
    WS_FILTER_LOG_ERROR("Failed to test connection filter: " +
                        std::string(e.what()));
    return createErrorResponse(http::status::bad_request,
                               "Failed to parse test message: " +
                                   std::string(e.what()));
  }
}
...
WebSocketFilterManager::handleGetFilterStatistics() {
  ...
  try {
    ...
    WS_FILTER_LOG_ERROR("Failed to get filter statistics: " +
                        std::string(e.what()));
    return createErrorResponse(http::status::internal_server_error,
...
void WebSocketFilterManager::saveConnectionPreferences(
...
bool WebSocketFilterManager::loadConnectionPreferences(
    const std::string &connectionId, ConnectionFilters &filters) {
  std::lock_guard<std::mutex> lock(preferencesLock_);
...
void WebSocketFilterManager::clearStoredPreferences(
    const std::string &connectionId) {
  std::lock_guard<std::mutex> lock(preferencesLock_);
...
void WebSocketFilterManager::applyFiltersToMultipleConnections(
  ...
  WS_FILTER_LOG_INFO("Applying filters to " +
                     std::to_string(connectionIds.size()) + " connections");
...
void WebSocketFilterManager::clearFiltersFromMultipleConnections(
  ...
  WS_FILTER_LOG_INFO("Clearing filters from " +
                     std::to_string(connectionIds.size()) + " connections");
...
void WebSocketFilterManager::broadcastToFilteredConnections(
  ...
  wsManager_->sendToMatchingConnections(
      message,
      [customFilter](const ConnectionFilters &filters,
                     const WebSocketMessage &msg) -> bool {
        return customFilter(filters) && filters.shouldReceiveMessage(msg);
      });
}

void WebSocketFilterManager::saveFilterTemplate(
    const std::string &templateName, const ConnectionFilters &filters) {
...
void WebSocketFilterManager::applyFilterTemplate(
    const std::string &connectionId, const std::string &templateName) {
  ConnectionFilters filters;
  if (loadFilterTemplate(templateName, filters)) {
    if (validateConnectionExists(connectionId)) {
      wsManager_->setConnectionFilters(connectionId, filters);
      saveConnectionPreferences(connectionId, filters);
      WS_FILTER_LOG_INFO("Filter template '" + templateName +
                         "' applied to connection: " + connectionId);
    }
  } else {
...
WebSocketFilterManager::getAvailableFilterTemplates() const {
  std::lock_guard<std::mutex> lock(templatesLock_);
...
WebSocketFilterManager::FilterStatistics
WebSocketFilterManager::getFilterStatistics() const {
  ...
  stats.averageFiltersPerConnection =
      stats.totalConnections > 0
          ? static_cast<double>(totalFilters) / stats.totalConnections
          : 0.0;
...
http::response<http::string_body>
WebSocketFilterManager::createErrorResponse(http::status status,
  ...
  std::ostringstream json;
  json << R"({"error":")"
       << ETLPlus::ExceptionHandling::escapeJsonString(message) << R"("})";
  res.body() = json.str();
  res.prepare_payload();
  return res;
}

ConnectionFilters WebSocketFilterManager::parseConnectionFiltersFromJson(
...
WebSocketMessage
WebSocketFilterManager::parseWebSocketMessageFromJson(const std::string &json) {
  // This is a simplified JSON parser for the WebSocketMessage
  // In a production system, you'd use a proper JSON library
  return WebSocketMessage::fromJson(json);
}

std::string WebSocketFilterManager::connectionFiltersToJson(
    const ConnectionFilters &filters) {
  return filters.toJson();
}

std::string WebSocketFilterManager::filterStatisticsToJson(
    const FilterStatistics &stats) const {
  std::ostringstream json;
  json << "{"
       ...
       << R"("averageFiltersPerConnection":)" << std::fixed
       << std::setprecision(2) << stats.averageFiltersPerConnection << ","
       << R"("jobFilterCounts":{)";
  ...
  for (const auto &[jobId, count] : stats.jobFilterCounts) {
    if (!first)
      json << ",";
    json << R"(")" << ETLPlus::ExceptionHandling::escapeJsonString(jobId)
         << R"(":)" << count;
    first = false;
  }
  ...
  for (const auto &[logLevel, count] : stats.logLevelFilterCounts) {
    if (!first)
      json << ",";
    json << R"(")" << ETLPlus::ExceptionHandling::escapeJsonString(logLevel)
         << R"(":)" << count;
    first = false;
  }
...
bool WebSocketFilterManager::validateConnectionExists(
    const std::string &connectionId) {
  std::vector<std::string> connectionIds = wsManager_->getConnectionIds();
  return std::find(connectionIds.begin(), connectionIds.end(), connectionId) !=
         connectionIds.end();
}

bool WebSocketFilterManager::validateFilterData(
    const ConnectionFilters &filters, std::string &errorMessage) {
  if (!filters.isValid()) {
...
void WebSocketFilterManager::initializeDefaultTemplates() {
  ...
  systemNotificationsTemplate.messageTypes.push_back(
