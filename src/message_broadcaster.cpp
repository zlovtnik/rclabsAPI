...
MessageBroadcaster::MessageBroadcaster(std::shared_ptr<ConnectionPool> connectionPool)
...
MessageBroadcaster::MessageBroadcaster(std::shared_ptr<ConnectionPool> connectionPool,
    ...
    if (!connectionPool_) {
        throw etl::ETLException(etl::ErrorCode::INVALID_CONNECTION, "Connection pool cannot be null");
    }
...
void MessageBroadcaster::broadcastMessage(const std::string& message) {
    ...
    if (config_.enableAsyncProcessing && !isQueueFull()) {
        ...
        QueuedMessage queuedMsg{message, MessageType::SYSTEM_NOTIFICATION, "", "", std::chrono::system_clock::now()};
        {
...
void MessageBroadcaster::sendToConnection(const std::string& connectionId, const std::string& message) {
    if (!running_.load()) {
...
void MessageBroadcaster::broadcastJobUpdate(const std::string& message, const std::string& jobId) {
    if (!running_.load()) {
...
void MessageBroadcaster::broadcastLogMessage(const std::string& message, const std::string& jobId, const std::string& logLevel) {
    if (!running_.load()) {
        WS_LOG_WARN("Message broadcaster not running, cannot broadcast log message");
        return;
    }

    if (config_.enableAsyncProcessing && !isQueueFull()) {
        // Queue for async processing
        QueuedMessage queuedMsg{message, MessageType::JOB_LOG_MESSAGE, jobId, logLevel, std::chrono::system_clock::now()};
        {
            std::lock_guard<std::mutex> lock(queueMutex_);
            messageQueue_.push(queuedMsg);
            stats_.totalMessagesQueued++;
        ...
        for (auto& connection : connections) {
            if (shouldProcessMessage(connection, MessageType::JOB_LOG_MESSAGE, jobId, logLevel)) {
                sendMessageToConnection(connection, message);
                sentCount++;
            }
        }
...
void MessageBroadcaster::broadcastByMessageType(const std::string& message, MessageType messageType, const std::string& jobId) {
    if (!running_.load()) {
        WS_LOG_WARN("Message broadcaster not running, cannot broadcast by message type");
        return;
    }

    if (config_.enableAsyncProcessing && !isQueueFull()) {
        // Queue for async processing
        QueuedMessage queuedMsg{message, messageType, jobId, "", std::chrono::system_clock::now()};
        {
            std::lock_guard<std::mutex> lock(queueMutex_);
            messageQueue_.push(queuedMsg);
            stats_.totalMessagesQueued++;
...
void MessageBroadcaster::broadcastToFilteredConnections(const std::string& message,
                                                      std::function<bool(const ConnectionFilters&)> filterPredicate) {
    if (!running_.load()) {
        WS_LOG_WARN("Message broadcaster not running, cannot broadcast to filtered connections");
        return;
    }
    ...
    broadcastToConnections(message, connections);
    updateStats(connections.size());
    WS_LOG_DEBUG("Message broadcasted to " + std::to_string(connections.size()) + " filtered connections");
}

void MessageBroadcaster::broadcastWithAdvancedRouting(const WebSocketMessage& message) {
    if (!running_.load()) {
        WS_LOG_WARN("Message broadcaster not running, cannot broadcast with advanced routing");
        return;
    }
...
void MessageBroadcaster::sendToMatchingConnections(const WebSocketMessage& message,
                                                 std::function<bool(const ConnectionFilters&, const WebSocketMessage&)> customMatcher) {
    if (!running_.load()) {
        WS_LOG_WARN("Message broadcaster not running, cannot send to matching connections");
        return;
    }
...
bool MessageBroadcaster::testConnectionFilter(const std::string& connectionId, const WebSocketMessage& testMessage) const {
    auto connection = connectionPool_->getConnection(connectionId);
    if (connection && connection->isOpen()) {
...
void MessageBroadcaster::setConnectionFilters(const std::string& connectionId, const ConnectionFilters& filters) {
    auto connection = connectionPool_->getConnection(connectionId);
    if (connection && connection->isOpen()) {
...
ConnectionFilters MessageBroadcaster::getConnectionFilters(const std::string& connectionId) const {
    auto connection = connectionPool_->getConnection(connectionId);
    if (connection && connection->isOpen()) {
...
void MessageBroadcaster::updateConnectionFilters(const std::string& connectionId, const ConnectionFilters& filters) {
    auto connection = connectionPool_->getConnection(connectionId);
    if (connection && connection->isOpen()) {
...
void MessageBroadcaster::addJobFilterToConnection(const std::string& connectionId, const std::string& jobId) {
    auto connection = connectionPool_->getConnection(connectionId);
    if (connection && connection->isOpen()) {
...
void MessageBroadcaster::removeJobFilterFromConnection(const std::string& connectionId, const std::string& jobId) {
    auto connection = connectionPool_->getConnection(connectionId);
    if (connection && connection->isOpen()) {
...
void MessageBroadcaster::addMessageTypeFilterToConnection(const std::string& connectionId, MessageType messageType) {
    auto connection = connectionPool_->getConnection(connectionId);
    if (connection && connection->isOpen()) {
...
void MessageBroadcaster::removeMessageTypeFilterFromConnection(const std::string& connectionId, MessageType messageType) {
    auto connection = connectionPool_->getConnection(connectionId);
    if (connection && connection->isOpen()) {
...
void MessageBroadcaster::addLogLevelFilterToConnection(const std::string& connectionId, const std::string& logLevel) {
    auto connection = connectionPool_->getConnection(connectionId);
    if (connection && connection->isOpen()) {
...
void MessageBroadcaster::removeLogLevelFilterFromConnection(const std::string& connectionId, const std::string& logLevel) {
    auto connection = connectionPool_->getConnection(connectionId);
    if (connection && connection->isOpen()) {
...
void MessageBroadcaster::clearConnectionFilters(const std::string& connectionId) {
    auto connection = connectionPool_->getConnection(connectionId);
    if (connection && connection->isOpen()) {
...
std::vector<std::string> MessageBroadcaster::getConnectionsForJob(const std::string& jobId) const {
...
std::vector<std::string> MessageBroadcaster::getConnectionsForMessageType(MessageType messageType) const {
    return connectionPool_->getConnectionIdsByFilter(
...
std::vector<std::string> MessageBroadcaster::getConnectionsForLogLevel(const std::string& logLevel) const {
    return connectionPool_->getConnectionIdsByFilter(
...
void MessageBroadcaster::flushQueue() {
    ...
    while (!messageQueue_.empty()) {
        ...
        size_t batchSize = std::min(static_cast<size_t>(config_.batchSize), messageQueue_.size());
...
void MessageBroadcaster::clearQueue() {
    ...
    if (clearedCount > 0) {
        WS_LOG_INFO("Cleared " + std::to_string(clearedCount) + " messages from queue");
    }
}
...
void MessageBroadcaster::broadcastToConnections(const std::string& message,
...
void MessageBroadcaster::sendMessageToConnection(const std::shared_ptr<WebSocketConnection>& connection,
                                                const std::string& message) {
    try {
        connection->send(message);
    } catch (const std::exception& e) {
        WS_LOG_ERROR("Failed to send message to connection " + connection->getId() + ": " + e.what());
    }
}

void MessageBroadcaster::updateStats(size_t messagesSent, size_t messagesDropped) {
    std::lock_guard<std::mutex> lock(statsMutex_);
...
bool MessageBroadcaster::shouldProcessMessage(const std::shared_ptr<WebSocketConnection>& connection,
...
void MessageBroadcaster::startAsyncProcessing() {
    size_t threadCount = std::min(config_.maxConcurrentBroadcasts, static_cast<size_t>(4)); // Max 4 threads

    for (size_t i = 0; i < threadCount; ++i) {
        processingThreads_.emplace_back(&MessageBroadcaster::processMessageQueue, this);
    }

    WS_LOG_INFO("Started " + std::to_string(threadCount) + " async processing threads");
}
...
std::string MessageBroadcasterStats::toJson() const {
    return "{"
           "\"totalMessagesSent\":" + std::to_string(totalMessagesSent) + ","
           "\"totalMessagesQueued\":" + std::to_string(totalMessagesQueued) + ","
           "\"totalMessagesDropped\":" + std::to_string(totalMessagesDropped) + ","
           "\"currentQueueSize\":" + std::to_string(currentQueueSize) + ","
           "\"activeBroadcasts\":" + std::to_string(activeBroadcasts) + ","
           "\"lastMessageSent\":\"" + formatTimestamp(lastMessageSent) + "\","
           "\"messagesPerSecond\":" + std::to_string(messagesPerSecond) +
           "}";
}
