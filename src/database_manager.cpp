...
bool DatabaseManager::connect(const ConnectionConfig &config) {
  ...
  DB_LOG_INFO("Attempting to connect to PostgreSQL database: " + config.host +
              ":" + std::to_string(config.port) + "/" + config.database);
  DB_LOG_DEBUG("Using username: " + config.username);

  try {
    // Create connection pool
    pImpl->connectionPool =
        std::make_unique<DatabaseConnectionPool>(pImpl->poolConfig);
...
void DatabaseManager::disconnect() {
  if (pImpl->connected && pImpl->connectionPool) {
    ...
    bool gracefulSuccess =
        pImpl->connectionPool->gracefulShutdown(GRACEFUL_SHUTDOWN_TIMEOUT);

    if (!gracefulSuccess) {
      DB_LOG_WARN("Graceful shutdown failed, some connections may have been "
                  "forcibly closed");
    }
...
bool DatabaseManager::executeQuery(const std::string &query) {
  ...
  DB_LOG_DEBUG("Executing query: " + query.substr(0, 100) +
               (query.length() > 100 ? "..." : ""));
#else
...
DatabaseManager::selectQuery(const std::string &query) {
  ...
  DB_LOG_DEBUG("Executing select query: " + query.substr(0, 100) +
               (query.length() > 100 ? "..." : ""));
  ...
  try {
    ...
    DB_LOG_DEBUG("Select query completed, returning " +
...
void DatabaseManager::setMaxConnections(int maxConn) {
  ...
  DB_LOG_INFO("Set maximum PostgreSQL database connections to " +
              std::to_string(maxConn));
}
