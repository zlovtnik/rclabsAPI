...
const notification_recovery::RetryConfig
    notification_recovery::RetryQueueManager::defaultConfig_{
        true,                              // enableRetry
        3,                                 // maxRetryAttempts
...
ResourceAlert ResourceAlert::fromJson(const std::string &jsonStr) {
  try {
    ...
    alert.timestamp =
        std::chrono::system_clock::now(); // Use current time for simplicity

    return alert;
  } catch (const nlohmann::json::exception &e) {
    throw etl::SystemException(etl::ErrorCode::PROCESSING_FAILED,
                               "Failed to parse resource alert JSON: " +
                                   std::string(e.what()),
                               "NotificationService");
  }
}
...
NotificationConfig NotificationConfig::fromConfig(const ConfigManager &config) {
  ...
  notifConfig.enabled =
      config.getBool("monitoring.notifications.enabled", true);
  notifConfig.jobFailureAlerts =
      config.getBool("monitoring.notifications.job_failure_alerts", true);
  notifConfig.timeoutWarnings =
      config.getBool("monitoring.notifications.timeout_warnings", true);
  notifConfig.resourceAlerts =
      config.getBool("monitoring.notifications.resource_alerts", true);
  notifConfig.maxRetryAttempts =
      config.getInt("monitoring.notifications.retry_attempts", 3);
  notifConfig.baseRetryDelayMs =
      config.getInt("monitoring.notifications.retry_delay", 5000);
  notifConfig.timeoutWarningThresholdMinutes =
      config.getInt("monitoring.job_tracking.timeout_warning_threshold", 25);

  // Resource thresholds
  notifConfig.memoryUsageThreshold =
      config.getDouble("monitoring.notifications.memory_threshold", 0.85);
  notifConfig.cpuUsageThreshold =
      config.getDouble("monitoring.notifications.cpu_threshold", 0.90);
  notifConfig.diskSpaceThreshold =
      config.getDouble("monitoring.notifications.disk_threshold", 0.90);

  // Email configuration
  notifConfig.emailSmtpServer =
      config.getString("monitoring.notifications.email.smtp_server");
  notifConfig.emailSmtpPort =
      config.getInt("monitoring.notifications.email.smtp_port", 587);
  notifConfig.emailUsername =
      config.getString("monitoring.notifications.email.username");
  notifConfig.emailPassword =
      config.getString("monitoring.notifications.email.password");

  // Webhook configuration
  notifConfig.webhookUrl =
      config.getString("monitoring.notifications.webhook.url");
  notifConfig.webhookSecret =
      config.getString("monitoring.notifications.webhook.secret");
  notifConfig.webhookTimeoutMs =
      config.getInt("monitoring.notifications.webhook.timeout", 30000);

  // Default methods
  notifConfig.defaultMethods = {NotificationMethod::LOG_ONLY};

  // Set up priority-based methods
  notifConfig.priorityMethods[NotificationPriority::LOW] = {
      NotificationMethod::LOG_ONLY};
  notifConfig.priorityMethods[NotificationPriority::MEDIUM] = {
      NotificationMethod::LOG_ONLY, NotificationMethod::WEBHOOK};
  notifConfig.priorityMethods[NotificationPriority::HIGH] = {
      NotificationMethod::LOG_ONLY, NotificationMethod::EMAIL,
      NotificationMethod::WEBHOOK};
  notifConfig.priorityMethods[NotificationPriority::CRITICAL] = {
      NotificationMethod::LOG_ONLY, NotificationMethod::EMAIL,
      NotificationMethod::WEBHOOK};
...
bool NotificationConfig::isValid() const {
  ...
  // Email validation
  if (!emailSmtpServer.empty() && !emailUsername.empty() &&
      !emailPassword.empty()) {
    hasValidMethod = true;
  }
...
bool EmailNotificationDelivery::isConfigured() const {
  return !config_.emailSmtpServer.empty() && !config_.emailUsername.empty() &&
         !config_.emailPassword.empty() && !config_.emailRecipients.empty();
}
...
bool EmailNotificationDelivery::sendEmail(const std::string &to,
...
WebhookNotificationDelivery::WebhookNotificationDelivery(
    const NotificationConfig &config)
    : config_(config) {}
...
bool WebhookNotificationDelivery::sendWebhook(const std::string &payload) {
  ...
  std::cout << "[WEBHOOK MOCK] Would send to " << config_.webhookUrl << ": "
            << payload << std::endl;
  return true;
#endif
}
...
NotificationServiceImpl::NotificationServiceImpl(Logger *logger)
    : logger_(logger), running_(false), testMode_(false), processedCount_(0),
      failedCount_(0) {
...
NotificationServiceImpl::~NotificationServiceImpl() { stop(); }
...
void NotificationServiceImpl::start() {
  ...
  running_.store(true);
  processingThread_ =
      std::thread(&NotificationServiceImpl::processNotifications, this);
...
bool NotificationServiceImpl::isRunning() const { return running_.load(); }

void NotificationServiceImpl::sendJobFailureAlert(const std::string &jobId,
                                                  const std::string &error) {
  if (!config_.enabled || !config_.jobFailureAlerts)
    return;
...
void NotificationServiceImpl::sendJobTimeoutWarning(const std::string &jobId,
                                                    int executionTimeMinutes) {
  if (!config_.enabled || !config_.timeoutWarnings)
    return;

  auto notification =
      createTimeoutWarningNotification(jobId, executionTimeMinutes);
  queueNotification(notification);
}

void NotificationServiceImpl::sendResourceAlert(const ResourceAlert &alert) {
  if (!config_.enabled || !config_.resourceAlerts)
    return;
...
void NotificationServiceImpl::sendSystemErrorAlert(const std::string &component,
                                                   const std::string &error) {
  if (!config_.enabled)
    return;
...
void NotificationServiceImpl::sendCustomNotification(
...
void NotificationServiceImpl::queueNotification(
    const NotificationMessage &message) {
  std::lock_guard<std::mutex> lock(queueMutex_);
  ...
  if (notificationQueue_.size() >= config_.queueMaxSize) {
    if (logger_) {
      logger_->warn("NotificationService",
                    "Notification queue full, dropping message: " + message.id);
    }
    return;
  }
  ...
  if (logger_) {
    logger_->debug("NotificationService",
                   "Queued notification: " + message.id + " (queue size: " +
...
void NotificationServiceImpl::checkConnectionLimit(int currentConnections,
                                                   int maxConnections) {
  ...
  if (usage > (config_.connectionLimitThreshold / 100.0)) {
    ...
    alert.thresholdValue =
        maxConnections * (config_.connectionLimitThreshold / 100.0);
...
NotificationServiceImpl::getRecentNotifications(size_t limit) const {
  ...
  return std::vector<NotificationMessage>(recentNotifications_.begin() + start,
...
void NotificationServiceImpl::setTestMode(bool enabled) {
  ...
  if (logger_) {
    logger_->info("NotificationService",
                  "Test mode " + std::string(enabled ? "enabled" : "disabled"));
  }
}
...
void NotificationServiceImpl::processNotifications() {
  while (running_.load()) {
    ...
    queueCondition_.wait(lock, [this] {
      return !running_.load() || !notificationQueue_.empty() ||
             (retryManager_.getQueueSize() > 0);
    });
    ...
    for (const auto &failedNotif : readyForRetry) {
      ...
      if (deliverNotification(retryMessage)) {
        ...
        failedCount_++;
        scheduleRetry(
            retryMessage, failedNotif.failureReason,
            static_cast<NotificationMethod>(failedNotif.failedMethodIndex));
      }
    }
...
bool NotificationServiceImpl::deliverNotification(
    const NotificationMessage &message) {
  if (testMode_.load()) {
    // In test mode, just log and return success
    if (logger_) {
      logger_->info("NotificationService",
                    "[TEST MODE] Would deliver: " + message.subject);
    }
    return true;
  }
...
void NotificationServiceImpl::scheduleRetry(const NotificationMessage &message,
  ...
  retryManager_.addFailedNotification(
      ...
      message.message, reason, static_cast<int>(failedMethod));

  if (logger_) {
    logger_->info("NotificationService",
                  "Scheduled retry for notification " + message.id +
                      " (attempt " + std::to_string(message.retryCount + 1) +
                      "/" + std::to_string(message.maxRetries) + ")");
  }
}

void NotificationServiceImpl::addToRecentNotifications(
    const NotificationMessage &message) {
  std::lock_guard<std::mutex> lock(recentMutex_);
...
NotificationMessage NotificationServiceImpl::createJobFailureNotification(
    const std::string &jobId, const std::string &error) {
  NotificationMessage msg;
...
NotificationMessage NotificationServiceImpl::createTimeoutWarningNotification(
    const std::string &jobId, int executionTimeMinutes) {
  NotificationMessage msg;
  msg.id = NotificationMessage::generateId();
  ...
  msg.message = "Job " + jobId + " has been running for " +
                formatDuration(executionTimeMinutes) +
                ", which exceeds the warning threshold of " +
                formatDuration(config_.timeoutWarningThresholdMinutes);
  msg.timestamp = std::chrono::system_clock::now();
  msg.scheduledFor = msg.timestamp;
  msg.retryCount = 0;
  msg.maxRetries = config_.maxRetryAttempts;
  ...
  msg.metadata["threshold_minutes"] =
      std::to_string(config_.timeoutWarningThresholdMinutes);
  msg.metadata["alert_type"] = "timeout_warning";

  return msg;
}

NotificationMessage NotificationServiceImpl::createResourceAlertNotification(
  ...
  msg.message = alert.description +
                ". Current value: " + std::to_string(alert.currentValue) + " " +
                alert.unit +
                ", threshold: " + std::to_string(alert.thresholdValue) + " " +
                alert.unit;
...
NotificationMessage NotificationServiceImpl::createSystemErrorNotification(
    const std::string &component, const std::string &error) {
  NotificationMessage msg;
  msg.id = NotificationMessage::generateId();
  ...
  msg.message =
      "A critical error occurred in component " + component + ": " + error;
  msg.timestamp = std::chrono::system_clock::now();
  msg.scheduledFor = msg.timestamp;
  msg.retryCount = 0;
  msg.maxRetries = config_.maxRetryAttempts;
...
void NotificationServiceImpl::setupDeliveryMethods() {
  ...
  deliveryMethods_.push_back(
      std::make_unique<LogNotificationDelivery>(logger_));

  // Add email delivery if configured
  if (!config_.emailSmtpServer.empty()) {
    deliveryMethods_.push_back(
        std::make_unique<EmailNotificationDelivery>(config_));
  }

  // Add webhook delivery if configured
  if (!config_.webhookUrl.empty()) {
    deliveryMethods_.push_back(
        std::make_unique<WebhookNotificationDelivery>(config_));
  }
}

std::vector<NotificationMethod>
NotificationServiceImpl::getMethodsForPriority(NotificationPriority priority) {
...
std::string NotificationServiceImpl::formatDuration(int minutes) {
  if (minutes < 60) {
    ...
    return std::to_string(hours) + " hours " +
           std::to_string(remainingMinutes) + " minutes";
  }
}

std::string
NotificationServiceImpl::getNotificationTypeString(NotificationType type) {
  switch (type) {
  case NotificationType::JOB_FAILURE:
    return "Job Failure";
  case NotificationType::JOB_TIMEOUT_WARNING:
    return "Job Timeout Warning";
  case NotificationType::RESOURCE_ALERT:
...
std::string
NotificationServiceImpl::getPriorityString(NotificationPriority priority) {
  switch (priority) {
  case NotificationPriority::LOW:
    return "Low";
  case NotificationPriority::MEDIUM:
