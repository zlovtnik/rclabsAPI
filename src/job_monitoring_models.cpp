...
std::string JobMetrics::toJson() const {
  ...
  json << "{"
       ...
       << "\"processingRate\":" << std::fixed << std::setprecision(2)
       << processingRate << ","
       ...
       << "\"peakCpuUsage\":" << std::fixed << std::setprecision(2)
       << peakCpuUsage << ","
       << "\"averageProcessingRate\":" << std::fixed << std::setprecision(2)
       << averageProcessingRate << ","
       << "\"totalBytesProcessed\":" << totalBytesProcessed << ","
       << "\"totalBytesWritten\":" << totalBytesWritten << ","
       << "\"totalBatches\":" << totalBatches << ","
       << "\"averageBatchSize\":" << std::fixed << std::setprecision(2)
       << averageBatchSize
       << ","
       ...
       << "\"throughputMBps\":" << std::fixed << std::setprecision(2)
       << throughputMBps << ","
       << "\"memoryEfficiency\":" << std::fixed << std::setprecision(2)
       << memoryEfficiency << ","
       << "\"cpuEfficiency\":" << std::fixed << std::setprecision(2)
       << cpuEfficiency << "}";
  return json.str();
}
...
JobMetrics JobMetrics::fromJson(const std::string &json) {
  ...
  std::regex averageProcessingRateRegex(
      "\"averageProcessingRate\"\\s*:\\s*([0-9.]+)");
  ...
  if (std::regex_search(json, match, executionTimeRegex)) {
    metrics.executionTime =
        std::chrono::milliseconds(std::stoll(match[1].str()));
  }
...
void JobMetrics::updateProcessingRate(std::chrono::milliseconds elapsed) {
  if (elapsed.count() > 0) {
    processingRate =
        static_cast<double>(recordsProcessed) / (elapsed.count() / 1000.0);
  }
}
...
void JobMetrics::updatePerformanceIndicators() {
  ...
  if (executionTime.count() > 0) {
    processingRate = static_cast<double>(recordsProcessed) /
  ...
  // Calculate memory efficiency (records per MB) - protect against
  // divide-by-zero
  ...
  // Calculate CPU efficiency (records per CPU percentage) - protect against
  // divide-by-zero
...
void JobMetrics::recordBatch(int batchSize, int successful, int failed,
...
void JobMetrics::calculateAverages() {
  ...
  // Calculate average processing rate over job lifetime - protect against
  // divide-by-zero
  if (executionTime.count() > 0) {
    averageProcessingRate = static_cast<double>(recordsProcessed) /
                            (executionTime.count() / 1000.0);
  } else {
...
double JobMetrics::getOverallEfficiency() const {
  ...
  if (averageProcessingRate > 0) {
    efficiency +=
        std::min(1.0, averageProcessingRate /
                          1000.0); // Assume 1000 records/sec is excellent
    factors++;
  }
  ...
  if (memoryEfficiency > 0) {
    efficiency += std::min(
        1.0, memoryEfficiency / 1000.0); // Assume 1000 records/MB is excellent
    factors++;
  }
  ...
  if (cpuEfficiency > 0) {
    efficiency += std::min(
        1.0, cpuEfficiency / 100.0); // Assume 100 records per CPU% is excellent
    factors++;
  }
...
bool JobMetrics::isPerformingWell(const JobMetrics &baseline) const {
  ...
  if (baseline.averageProcessingRate > 0) {
    performanceGood &= (averageProcessingRate >=
                        baseline.averageProcessingRate * PERFORMANCE_THRESHOLD);
  }

  // Check error rate (should be lower than baseline)
  if (baseline.recordsProcessed > 0) {
    performanceGood &=
        (errorRate <= baseline.errorRate * 1.2); // Allow 20% more errors
  }

  // Check memory efficiency
  if (baseline.memoryEfficiency > 0) {
    performanceGood &=
        (memoryEfficiency >= baseline.memoryEfficiency * PERFORMANCE_THRESHOLD);
  }

  // Check CPU efficiency
  if (baseline.cpuEfficiency > 0) {
    performanceGood &=
        (cpuEfficiency >= baseline.cpuEfficiency * PERFORMANCE_THRESHOLD);
  }
...
std::string JobMetrics::getPerformanceSummary() const {
  ...
  summary << " (" << std::fixed << std::setprecision(1) << processingRate
          << " rec/sec)";
  summary << ", " << std::fixed << std::setprecision(1) << errorRate
          << "% error rate";

  if (throughputMBps > 0) {
    summary << ", " << std::fixed << std::setprecision(2) << throughputMBps
            << " MB/s throughput";
  }

  if (memoryEfficiency > 0) {
    summary << ", " << std::fixed << std::setprecision(1) << memoryEfficiency
            << " rec/MB memory efficiency";
  }

  double efficiency = getOverallEfficiency();
  summary << ", Overall efficiency: " << std::fixed << std::setprecision(1)
          << (efficiency * 100) << "%";
...
bool LogMessage::matchesFilter(const std::string &jobIdFilter,
...
WebSocketMessage
WebSocketMessage::createJobStatusUpdate(const JobStatusUpdate &update) {
  WebSocketMessage message;
...
WebSocketMessage
WebSocketMessage::createMetricsUpdate(const std::string &jobId,
  ...
  data << "{\"jobId\":\"" << escapeJsonString(jobId)
       << "\",\"metrics\":" << metrics.toJson() << "}";
...
WebSocketMessage
WebSocketMessage::createErrorMessage(const std::string &error) {
  WebSocketMessage message;
...
std::string ConnectionFilters::toJson() const {
  ...
  json << "],\"includeSystemNotifications\":"
       << (includeSystemNotifications ? "true" : "false") << "}";
...
ConnectionFilters ConnectionFilters::fromJson(const std::string &json) {
  ...
  if (std::regex_search(json, match, logLevelsRegex)) {
    ...
    std::sregex_iterator iter(logLevelsStr.begin(), logLevelsStr.end(),
                              logLevelRegex);
    std::sregex_iterator end;
    for (; iter != end; ++iter) {
  ...
  if (std::regex_search(json, match, messageTypesRegex)) {
    ...
    std::sregex_iterator iter(messageTypesStr.begin(), messageTypesStr.end(),
                              messageTypeRegex);
    std::sregex_iterator end;
    for (; iter != end; ++iter) {
  ...
  std::regex systemNotificationsRegex(
      "\"includeSystemNotifications\"\\s*:\\s*(true|false)");
...
bool ConnectionFilters::shouldReceiveMessage(
  ...
  // Check job ID filter
  if (message.targetJobId.has_value() &&
      !shouldReceiveJob(message.targetJobId.value())) {
    return false;
  }

  // Check log level filter
  if (message.targetLevel.has_value() &&
      !shouldReceiveLogLevel(message.targetLevel.value())) {
    return false;
  }
...
void ConnectionFilters::addMessageType(MessageType messageType) {
  if (std::find(messageTypes.begin(), messageTypes.end(), messageType) ==
      messageTypes.end()) {
...
void ConnectionFilters::addLogLevel(const std::string &logLevel) {
  if (std::find(logLevels.begin(), logLevels.end(), logLevel) ==
      logLevels.end()) {
...
bool ConnectionFilters::hasJobFilters() const { return !jobIds.empty(); }
...
std::string messageTypeToString(MessageType type) {
  switch (type) {
  case MessageType::JOB_STATUS_UPDATE:
    return "job_status_update";
  case MessageType::JOB_PROGRESS_UPDATE:
    return "job_progress_update";
  case MessageType::JOB_LOG_MESSAGE:
    return "job_log_message";
  case MessageType::JOB_METRICS_UPDATE:
    return "job_metrics_update";
...
MessageType stringToMessageType(const std::string &typeStr) {
  if (typeStr == "job_status_update")
    return MessageType::JOB_STATUS_UPDATE;
  if (typeStr == "job_progress_update")
    return MessageType::JOB_PROGRESS_UPDATE;
  if (typeStr == "job_log_message")
    return MessageType::JOB_LOG_MESSAGE;
  if (typeStr == "job_metrics_update")
    return MessageType::JOB_METRICS_UPDATE;
  if (typeStr == "system_notification")
    return MessageType::SYSTEM_NOTIFICATION;
  if (typeStr == "connection_ack")
...
bool validateJobId(const std::string &jobId) {
  ...
  // Job ID should contain only alphanumeric characters, underscores, and
  // hyphens
