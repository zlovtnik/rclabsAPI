...
PooledSession::PooledSession(
    ...
    : stream_(std::move(socket)), handler_(handler), wsManager_(wsManager),
      timeoutManager_(timeoutManager), performanceMonitor_(performanceMonitor),
      lastActivity_(std::chrono::steady_clock::now()),
      requestStartTime_(std::chrono::steady_clock::now()), isIdle_(false),
      processingRequest_(false) {

  HTTP_LOG_DEBUG(
      "PooledSession created with handler: " +
      std::string(handler ? "valid" : "null") +
      ", WebSocket manager: " + std::string(wsManager ? "valid" : "null") +
      ", Timeout manager: " + std::string(timeoutManager ? "valid" : "null") +
      ", Performance monitor: " +
      std::string(performanceMonitor ? "valid" : "null"));
}
...
bool PooledSession::isIdle() const { return isIdle_ && !processingRequest_; }
...
tcp::socket &PooledSession::getSocket() { return stream_.socket(); }

bool PooledSession::isProcessingRequest() const { return processingRequest_; }

void PooledSession::handleTimeout(const std::string &timeoutType) {
  HTTP_LOG_WARN("PooledSession::handleTimeout() - Timeout occurred: " +
                timeoutType);

  // Record timeout for performance monitoring
  if (performanceMonitor_) {
    if (timeoutType == "CONNECTION") {
      performanceMonitor_->recordTimeout(
  ...
  if (timeoutType == "CONNECTION") {
    HTTP_LOG_INFO(
        "PooledSession::handleTimeout() - Connection timeout, closing session");
  } else if (timeoutType == "REQUEST") {
    HTTP_LOG_INFO("PooledSession::handleTimeout() - Request timeout, sending "
                  "timeout response");

    // Send HTTP 408 Request Timeout response
    http::response<http::string_body> timeout_res{http::status::request_timeout,
                                                  req_.version()};
    ...
    try {
      ...
      HTTP_LOG_ERROR(
          "PooledSession::handleTimeout() - Error sending timeout response: " +
          std::string(e.what()));
      doClose();
    }
    return;
...
void PooledSession::doRead() {
  ...
  // Optimize memory allocation: reuse existing request object instead of
  // creating new one
  req_.clear();
...
void PooledSession::onRead(beast::error_code ec,
                           std::size_t bytes_transferred) {
  HTTP_LOG_DEBUG("PooledSession::onRead() - Read completed, bytes: " +
  ...
  HTTP_LOG_INFO("PooledSession::onRead() - Processing request: " +
  ...
  if (beast::websocket::is_upgrade(req_)) {
    HTTP_LOG_INFO(
        "PooledSession::onRead() - WebSocket upgrade request detected");

    if (!wsManager_) {
      HTTP_LOG_ERROR("PooledSession::onRead() - WebSocket manager not "
  ...
  try {
    HTTP_LOG_DEBUG(
        "PooledSession::onRead() - Calling handler->handleRequest()");
    auto response = handler_->handleRequest(std::move(req_));
    HTTP_LOG_DEBUG(
        "PooledSession::onRead() - Handler completed, sending response");
    sendResponse(std::move(response));
  } catch (const std::exception &e) {
    HTTP_LOG_ERROR("PooledSession::onRead() - Exception in handler: " +
                   std::string(e.what()));
...
void PooledSession::sendResponse(http::response<http::string_body> &&msg) {
  HTTP_LOG_DEBUG(
      "PooledSession::sendResponse() - Sending response with status: " +
      std::to_string(msg.result_int()));
  HTTP_LOG_DEBUG("PooledSession::sendResponse() - Response body size: " +
                 std::to_string(msg.body().size()));

  updateLastActivity();

  try {
    // Optimize memory allocation: use move semantics and avoid unnecessary
    // shared_ptr allocation for small responses that can be sent immediately
    const bool needsSharedPtr = msg.body().size() > 4096; // 4KB threshold

    if (needsSharedPtr) {
      HTTP_LOG_DEBUG("PooledSession::sendResponse() - Creating shared response "
                     "object for large response");
      auto response =
      ...
      http::async_write(stream_, *response,
                        [self, response](beast::error_code ec,
                                         std::size_t bytes_transferred) {
                          self->onWrite(response->need_eof(), ec,
                                        bytes_transferred);
                        });
    } else {
      HTTP_LOG_DEBUG("PooledSession::sendResponse() - Using direct write for "
                     "small response");
      bool close = msg.need_eof();
      auto self = shared_from_this();

      // Create a shared pointer to ensure the response lives for the duration
      // of the async operation
      auto response =
          std::make_shared<http::response<http::string_body>>(std::move(msg));

      http::async_write(stream_, *response,
                        [self, response, close](beast::error_code ec,
                                                std::size_t bytes_transferred) {
                          self->onWrite(close, ec, bytes_transferred);
                        });
    }

    HTTP_LOG_DEBUG("PooledSession::sendResponse() - http::async_write called "
                   "successfully");
  } catch (const std::exception &e) {
    HTTP_LOG_ERROR("PooledSession::sendResponse() - Exception: " +
                   std::string(e.what()));
    processingRequest_ = false;
    ...
    HTTP_LOG_ERROR(
        "PooledSession::sendResponse() - Unknown exception occurred");
    processingRequest_ = false;
...
void PooledSession::onWrite(bool close, beast::error_code ec,
                            std::size_t bytes_transferred) {
  HTTP_LOG_DEBUG("PooledSession::onWrite() - Write completed, bytes: " +
                 std::to_string(bytes_transferred) +
  ...
  HTTP_LOG_DEBUG(
      "PooledSession::onWrite() - Request completed, marking as idle");
  setIdle(true);
...
void PooledSession::startConnectionTimeout() {
  if (timeoutManager_) {
    HTTP_LOG_DEBUG("PooledSession::startConnectionTimeout() - Starting "
                   "connection timeout");
...
void PooledSession::startRequestTimeout() {
  if (timeoutManager_) {
    HTTP_LOG_DEBUG(
        "PooledSession::startRequestTimeout() - Starting request timeout");
...
void PooledSession::clearBuffers() {
  // Optimize memory management: don't shrink buffer capacity unless it's too
  // large This avoids frequent reallocations
