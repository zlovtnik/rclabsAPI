...
CacheManager::CacheManager(const CacheConfig &config)
  ...
  // Validate warmup configuration to prevent resource exhaustion and invalid
  // states
  if (config_.enableWarmup) {
    if (config_.warmupBatchSize == 0) {
      throw std::invalid_argument(
          "CacheManager: warmupBatchSize must be greater than 0");
    }
    ...
    if (config_.warmupBatchTimeout <= std::chrono::seconds(0)) {
      throw std::invalid_argument(
          "CacheManager: warmupBatchTimeout must be positive");
    }

    if (config_.warmupTotalTimeout <= std::chrono::seconds(0)) {
      throw std::invalid_argument(
          "CacheManager: warmupTotalTimeout must be positive");
    }

    if (config_.warmupTotalTimeout < config_.warmupBatchTimeout) {
      throw std::invalid_argument(
          "CacheManager: warmupTotalTimeout (" +
  ...
  WS_LOG_INFO("Cache manager initialized with TTL=" +
              std::to_string(config_.defaultTTL.count()) +
              "s, health check TTL=" +
              std::to_string(config_.healthCheckTTL.count()) + "s");
}
...
bool CacheManager::cacheSessionData(const std::string &sessionId,
  ...
  bool success = redisCache_->setWithTags(key, sessionData.dump(), tags,
                                          config_.sessionDataTTL);
  if (success) {
    updateStats(false, false);
    stats_.sets++;
  } else {
...
bool CacheManager::cacheData(const std::string &key, const nlohmann::json &data,
  ...
  std::chrono::seconds actualTTL =
      ttl.has_value() ? ttl.value() : getTTLForTags(tags);
...
CacheManager::CacheStats CacheManager::getCacheStats() const {
  ...
  stats.hitRate =
      (totalRequests > 0)
          ? (static_cast<double>(stats.hits) / totalRequests) * 100.0
          : 0.0;
...
void CacheManager::warmupCache(DatabaseManager *dbManager) {
  ...
  WS_LOG_INFO("Starting cache warmup with batch size: " +
              std::to_string(config_.warmupBatchSize) +
  ...
  if (config_.warmupMaxKeys > 0 && config_.warmupMaxKeys <= 50000) {
    ...
    WS_LOG_WARN("Invalid warmupMaxKeys value " +
                std::to_string(config_.warmupMaxKeys) +
  ...
  try {
    ...
    while (batchStart != results.end()) {
      auto batchEnd =
          std::min(batchStart + config_.warmupBatchSize, results.end());
      std::vector<std::vector<std::string>> batch(batchStart, batchEnd);

      // Launch async batch processing
      batchFutures.push_back(std::async(
          std::launch::async, [this, batch, &totalLoaded, &totalErrors]() {
      ...
      if (elapsed >= config_.warmupTotalTimeout) {
        WS_LOG_WARN("Cache warmup timed out after " +
    ...
    WS_LOG_INFO(
        "Cache warmup completed: loaded=" + std::to_string(totalLoaded.load()) +
...
std::chrono::seconds
CacheManager::getTTLForTags(const std::vector<std::string> &tags) const {
...
bool CacheManager::processWarmupBatch(
    const std::vector<std::vector<std::string>> &batch,
    std::atomic<size_t> &totalLoaded, std::atomic<size_t> &totalErrors) {
  bool batchSuccess = true;

  for (const auto &row : batch) {
    if (row.size() < 2) {
      WS_LOG_WARN(
          "Invalid row format in warmup batch, expected at least 2 columns");
      totalErrors++;
      continue;
    }
    ...
    try {
      ...
      WS_LOG_ERROR("Error processing warmup key '" + row[0] +
                   "': " + std::string(e.what()));
      totalErrors++;
      batchSuccess = false;
    }
