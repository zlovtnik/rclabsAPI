...
public:
  Listener(net::io_context &ioc, tcp::endpoint endpoint,
      ...
      : ioc_(ioc), acceptor_(net::make_strand(ioc)), poolManager_(poolManager) {
    beast::error_code ec;
  ...
  void onAccept(beast::error_code ec, tcp::socket socket) {
    if (ec) {
      ...
      if (stopped_ || ec == net::error::operation_aborted ||
          ...
          ec == net::error::invalid_argument || !acceptor_.is_open()) {
        HTTP_LOG_DEBUG(
            "Listener::onAccept() - Accept aborted/closed during shutdown: " +
            ec.message());
      ...
      fail(ec, "accept");
      HTTP_LOG_WARN("Listener::onAccept() - Non-fatal error, continuing to "
                    "accept connections");
    } else {
      ...
      if (!poolManager_) {
        ...
        try {
          ...
          if (session) {
            ...
            HTTP_LOG_ERROR("Listener::onAccept() - Failed to acquire "
                           "connection from pool");
          }
        } catch (const std::exception &e) {
          HTTP_LOG_ERROR(
              "Listener::onAccept() - Exception acquiring connection: " +
              std::string(e.what()));
        }
      }
    }
...
struct HttpServer::Impl {
  ...
  std::shared_ptr<Listener>
      listener; // keep handle to listener for proper shutdown
  bool running = false;
};
...
HttpServer::HttpServer(const std::string &address, unsigned short port,
                       int threads, const ServerConfig &config)
    : pImpl(std::make_unique<Impl>()) {
  pImpl->address = address;
  pImpl->port = port;
  pImpl->threads = std::max<int>(1, threads);
  ...
  if (!validation.isValid) {
    ...
    HTTP_LOG_INFO("HttpServer constructor - Applied default values for invalid "
                  "configuration");
  }
...
void HttpServer::start() {
  HTTP_LOG_INFO("HttpServer::start() - Starting HTTP server on " +
                pImpl->address + ":" + std::to_string(pImpl->port));
  ...
  if (!pImpl->handler) {
    ...
    throw etl::SystemException(etl::ErrorCode::CONFIGURATION_ERROR,
                               "No request handler set", "HttpServer");
  }

  try {
    HTTP_LOG_DEBUG("HttpServer::start() - Creating IO context with " +
                   std::to_string(pImpl->threads) + " threads");
    ...
    pImpl->timeoutManager = std::make_shared<TimeoutManager>(
        *pImpl->ioc, pImpl->config.connectionTimeout,
    ...
    pImpl->poolManager = std::make_shared<ConnectionPoolManager>(
        *pImpl->ioc, pImpl->config.minConnections, pImpl->config.maxConnections,
        pImpl->config.idleTimeout, pImpl->handler, pImpl->wsManager,
        pImpl->timeoutManager,
        ConnectionPoolManager::MonitorConfig{nullptr}, // performanceMonitor
        ConnectionPoolManager::QueueConfig{pImpl->config.maxQueueSize,
    ...
    HTTP_LOG_DEBUG("HttpServer::start() - Address parsed: " +
                   address.to_string());

    HTTP_LOG_DEBUG(
        "HttpServer::start() - Creating listener with connection pool");
    pImpl->listener = std::make_shared<Listener>(
        *pImpl->ioc, tcp::endpoint{address, pImpl->port}, pImpl->poolManager);
    pImpl->listener->run();
    ...
    for (int i = 0; i < pImpl->threads; ++i) {
      pImpl->threadPool.emplace_back([this, i]() {
        ...
        try {
          pImpl->ioc->run();
          HTTP_LOG_DEBUG("HttpServer thread " + std::to_string(i) +
                         " finished");
        } catch (const std::exception &e) {
          HTTP_LOG_ERROR("HttpServer thread " + std::to_string(i) +
    ...
    pImpl->running = true;
    HTTP_LOG_INFO("HttpServer::start() - HTTP server started successfully with "
                  "connection pooling");
    HTTP_LOG_INFO("HttpServer::start() - Pool config: min=" +
                  std::to_string(pImpl->config.minConnections) +
                  ", max=" + std::to_string(pImpl->config.maxConnections) +
                  ", idleTimeout=" +
                  std::to_string(pImpl->config.idleTimeout.count()) + "s");
  } catch (const std::exception &e) {
    HTTP_LOG_ERROR("HttpServer::start() - Error starting server: " +
                   std::string(e.what()));
...
void HttpServer::stop() {
  ...
  if (pImpl->poolManager) {
    HTTP_LOG_DEBUG(
        "HttpServer::stop() - Stopping connection pool cleanup timer");
...
bool HttpServer::isRunning() const { return pImpl->running; }

void HttpServer::setRequestHandler(std::shared_ptr<RequestHandler> handler) {
  HTTP_LOG_INFO("HttpServer::setRequestHandler() - Setting request handler: " +
                std::string(handler ? "valid" : "null"));
  pImpl->handler = handler;

  // If the pool manager already exists, we need to recreate it with the new
  // handler
  if (pImpl->poolManager && pImpl->ioc && pImpl->timeoutManager) {
    HTTP_LOG_DEBUG("HttpServer::setRequestHandler() - Recreating connection "
                   "pool with new handler");
    pImpl->poolManager->shutdown();
    pImpl->poolManager = std::make_shared<ConnectionPoolManager>(
        *pImpl->ioc, pImpl->config.minConnections, pImpl->config.maxConnections,
        pImpl->config.idleTimeout, pImpl->handler, pImpl->wsManager,
...
void HttpServer::setWebSocketManager(
    std::shared_ptr<WebSocketManager> wsManager) {
  ...
  // WebSocket manager
  if (pImpl->poolManager && pImpl->ioc && pImpl->timeoutManager) {
    HTTP_LOG_DEBUG("HttpServer::setWebSocketManager() - Recreating connection "
                   "pool with new WebSocket manager");
    pImpl->poolManager->shutdown();
    pImpl->poolManager = std::make_shared<ConnectionPoolManager>(
        *pImpl->ioc, pImpl->config.minConnections, pImpl->config.maxConnections,
        pImpl->config.idleTimeout, pImpl->handler, pImpl->wsManager,
...
void HttpServer::setServerConfig(const ServerConfig &config) {
  ...
  if (!validation.isValid) {
    ...
    pImpl->config.applyDefaults();
    HTTP_LOG_INFO("HttpServer::setServerConfig() - Applied default values for "
  ...
  if (pImpl->timeoutManager) {
    pImpl->timeoutManager->setConnectionTimeout(
...
ServerConfig HttpServer::getServerConfig() const { return pImpl->config; }
