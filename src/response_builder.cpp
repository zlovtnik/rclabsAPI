...
ResponseBuilder& ResponseBuilder::setHeaders(const std::unordered_map<std::string, std::string>& headers) {
...
ResponseBuilder& ResponseBuilder::setMetadata(const ResponseMetadata& metadata) {
    currentMetadata_ = metadata;
    return *this;
}

http::response<http::string_body> ResponseBuilder::success(const std::string& data) {
    currentStatus_ = http::status::ok;
    return buildResponse(data);
}

http::response<http::string_body> ResponseBuilder::success(const std::string& data, ContentType type) {
    currentStatus_ = http::status::ok;
    currentContentType_ = type;
    return buildResponse(data);
}

http::response<http::string_body> ResponseBuilder::successJson(const std::string& jsonData) {
    currentStatus_ = http::status::ok;
...
http::response<http::string_body> ResponseBuilder::successWithMessage(const std::string& message, const std::string& data) {
    currentStatus_ = http::status::ok;
    currentContentType_ = ContentType::JSON;
    
    std::ostringstream json;
    json << R"({"status":"success","message":")" << escapeJsonString(message) << R"(")";
    ...
    if (config_.includeTimestamp) {
        json << R"(,"timestamp":")" << formatTimestamp(std::chrono::system_clock::now()) << R"(")";
    }
...
http::response<http::string_body> ResponseBuilder::error(http::status status, const std::string& message) {
    currentStatus_ = status;
    currentContentType_ = ContentType::JSON;
    
    std::string errorJson = createErrorJson(message, std::to_string(static_cast<int>(status)));
    return buildResponse(errorJson);
}

http::response<http::string_body> ResponseBuilder::badRequest(const std::string& message) {
    return error(http::status::bad_request, message);
}

http::response<http::string_body> ResponseBuilder::unauthorized(const std::string& message) {
    return error(http::status::unauthorized, message);
}

http::response<http::string_body> ResponseBuilder::forbidden(const std::string& message) {
    return error(http::status::forbidden, message);
}

http::response<http::string_body> ResponseBuilder::notFound(const std::string& resource) {
    return error(http::status::not_found, resource + " not found");
}

http::response<http::string_body> ResponseBuilder::methodNotAllowed(const std::string& method, const std::string& endpoint) {
...
http::response<http::string_body> ResponseBuilder::conflict(const std::string& message) {
    return error(http::status::conflict, message);
}

http::response<http::string_body> ResponseBuilder::tooManyRequests(const std::string& message) {
    setHeader("retry-after", "60");
    return error(http::status::too_many_requests, message);
}

http::response<http::string_body> ResponseBuilder::internalServerError(const std::string& message) {
    return error(http::status::internal_server_error, message);
}

http::response<http::string_body> ResponseBuilder::serviceUnavailable(const std::string& message) {
    setHeader("retry-after", "300");
    return error(http::status::service_unavailable, message);
}

http::response<http::string_body> ResponseBuilder::fromException(const etl::ETLException& ex) {
...
http::response<http::string_body> ResponseBuilder::fromValidationResult(const InputValidator::ValidationResult& result) {
    return validationError(result);
}

http::response<http::string_body> ResponseBuilder::fromStandardException(const std::exception& ex, const std::string& context) {
...
http::response<http::string_body> ResponseBuilder::validationError(const InputValidator::ValidationResult& result) {
...
http::response<http::string_body> ResponseBuilder::authenticationRequired(const std::string& realm) {
...
http::response<http::string_body> ResponseBuilder::healthCheck(bool healthy, const std::string& details) {
    currentStatus_ = healthy ? http::status::ok : http::status::service_unavailable;
    currentContentType_ = ContentType::JSON;
...
http::response<http::string_body> ResponseBuilder::redirect(const std::string& location, http::status status) {
    currentStatus_ = status;
    setHeader("location", location);
    return buildResponse("");
}

http::response<http::string_body> ResponseBuilder::permanentRedirect(const std::string& location) {
    return redirect(location, http::status::moved_permanently);
}

http::response<http::string_body> ResponseBuilder::temporaryRedirect(const std::string& location) {
    return redirect(location, http::status::temporary_redirect);
}

http::response<http::string_body> ResponseBuilder::cached(const std::string& data, std::chrono::seconds maxAge) {
    setHeader("cache-control", "public, max-age=" + std::to_string(maxAge.count()));
    setHeader("expires", formatTimestamp(std::chrono::system_clock::now() + maxAge));
    return success(data);
}

http::response<http::string_body> ResponseBuilder::noCache(const std::string& data) {
...
http::response<http::string_body> ResponseBuilder::buildResponse(const std::string& body) {
...
void ResponseBuilder::applyDefaultHeaders(http::response<http::string_body>& response) {
    ...
    if (config_.includeTimestamp) {
        response.set("x-timestamp", formatTimestamp(std::chrono::system_clock::now()));
    }
}

void ResponseBuilder::applyCorsHeaders(http::response<http::string_body>& response, const CorsConfig& cors) {
...
void ResponseBuilder::applySecurityHeaders(http::response<http::string_body>& response) {
...
void ResponseBuilder::updateStats(const http::response<http::string_body>& response) {
...
std::string ResponseBuilder::createErrorJson(const std::string& message, const std::string& code) {
    std::ostringstream json;
    json << R"({"status":"error","error":")" << escapeJsonString(message) << R"(")";
...
std::string ResponseBuilder::createSuccessJson(const std::string& data, const std::string& message) {
    std::ostringstream json;
...
std::string ResponseBuilder::createValidationErrorJson(const InputValidator::ValidationResult& result) {
    std::ostringstream json;
...
std::string ResponseBuilder::createExceptionJson(const etl::ETLException& ex) {
    ...
    if (config_.includeTimestamp || (config_.includeRequestId && !currentRequestId_.empty())) {
...
std::string ResponseBuilder::sanitizeHeaderValue(const std::string& value) {
    ...
    sanitized.erase(std::remove_if(sanitized.begin(), sanitized.end(),
        [](char c) { return c < 32 && c != 9 && c != 10 && c != 13; }), 
        sanitized.end());
...
ResponseBuilder::ContentType ResponseBuilder::stringToContentType(const std::string& mimeType) {
...
std::string ResponseBuilder::statusToReasonPhrase(http::status status) {
    ...
    case http::status::too_many_requests: return "Too Many Requests";
    case http::status::internal_server_error: return "Internal Server Error";
    case http::status::service_unavailable: return "Service Unavailable";
...
std::string ResponseBuilder::formatTimestamp(const std::chrono::system_clock::time_point& time) {
