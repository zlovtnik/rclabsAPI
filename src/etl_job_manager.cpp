...
std::string ETLJobManager::scheduleJob(const ETLJobConfig &config) {
  ...
  ETL_LOG_INFO("Scheduled job: " + job->jobId +
...
std::vector<std::shared_ptr<ETLJob>>
ETLJobManager::getJobsByStatus(JobStatus status) const {
  SCOPED_LOCK_TIMEOUT(jobMutex_, kLockTO_Read.count());
...
bool ETLJobManager::isRunning() const { return running_; }

void ETLJobManager::setJobMonitorService(
    std::shared_ptr<JobMonitorServiceInterface> monitor) {
  monitorService_ = monitor;
  ETL_LOG_INFO("Job monitor service attached to ETL Job Manager");
}

void ETLJobManager::publishJobStatusUpdate(const std::string &jobId,
                                           JobStatus status) {
  if (monitorService_) {
...
void ETLJobManager::publishJobProgress(const std::string &jobId, int progress,
...
void ETLJobManager::publishJobMetrics(const std::string &jobId,
...
void ETLJobManager::enableMetricsCollection(bool enabled) {
  ...
  ETL_LOG_INFO("Metrics collection " +
...
void ETLJobManager::setMetricsUpdateInterval(
    std::chrono::milliseconds interval) {
  metricsUpdateInterval_ = interval;
  ETL_LOG_INFO("Metrics update interval set to " +
...
void ETLJobManager::workerLoop() {
  while (running_) {
    ...
    jobCondition_.wait(lock,
                       [this] { return !jobQueue_.empty() || !running_; });
...
void ETLJobManager::executeJob(std::shared_ptr<ETLJob> job) {
  ...
  try {
    ...
    ETL_LOG_ERROR("Job failed with ETL exception: " + job->jobId + " - " +
                  ex.toLogString());
    ...
    auto etlEx =
        etl::BusinessException(etl::ErrorCode::PROCESSING_FAILED,
                               "Job execution failed: " + std::string(e.what()),
                               "executeJob", context);

    ETL_LOG_ERROR("Job failed with standard exception: " + job->jobId + " - " +
                  etlEx.toLogString());
    throw etlEx;
    ...
    auto unknownEx = etl::BusinessException(
        ...
        "Job execution failed with unknown error", "executeJob", context);

    ETL_LOG_ERROR("Job failed with unknown exception: " + job->jobId + " - " +
                  unknownEx.toLogString());
    throw unknownEx;
  }
...
void ETLJobManager::executeExtractJob(std::shared_ptr<ETLJob> job) {
  ...
  for (int processed = 0; processed < totalRecords; processed += batchSize) {
    ...
    if (job->metricsCollector && job->metricsCollector->isCollecting()) {
      job->metricsCollector->recordBatchProcessed(currentBatch, successful,
                                                  failed);
...
void ETLJobManager::executeTransformJob(std::shared_ptr<ETLJob> job) {
  ...
  // Record metrics if collector is available
  if (job->metricsCollector && job->metricsCollector->isCollecting()) {
    job->metricsCollector->recordBatchProcessed(totalRecords, successful,
                                                failed);
...
void ETLJobManager::executeLoadJob(std::shared_ptr<ETLJob> job) {
  ...
  try {
    ...
    for (int processed = 0; processed < totalRecords; processed += batchSize) {
      ...
      // Simulate success/failure rates (94% success rate for database
      // operations)
      ...
      if (job->jobId.find("fail") != std::string::npos) {
        throw etl::SystemException(etl::ErrorCode::CONSTRAINT_VIOLATION,
...
void ETLJobManager::executeFullETLJob(std::shared_ptr<ETLJob> job) {
  ...
  std::cout << "Full ETL pipeline completed for job: " << job->jobId
            << std::endl;
}
...
void ETLJobManager::updateJobProgress(std::shared_ptr<ETLJob> job, int progress,
                                      const std::string &step) {
  ETL_LOG_DEBUG("Job progress update: " + job->jobId + " - " +
                std::to_string(progress) + "% - " + step);
...
void ETLJobManager::updateJobStatus(std::shared_ptr<ETLJob> job,
                                    JobStatus newStatus) {
  JobStatus oldStatus = job->status;
  ...
  if (newStatus == JobStatus::RUNNING &&
      job->startedAt.time_since_epoch().count() == 0) {
    job->startedAt = std::chrono::system_clock::now();
...
void ETLJobManager::startJobMetricsCollection(std::shared_ptr<ETLJob> job) {
  ...
  job->metricsCollector =
      std::make_shared<ETLPlus::Metrics::JobMetricsCollector>(job->jobId);
...
void ETLJobManager::setupMetricsCallback(std::shared_ptr<ETLJob> job) {
  ...
  auto metricsCallback =
      [this](const std::string &callbackJobId,
             const ETLPlus::Metrics::JobMetricsCollector::MetricsSnapshot
                 &snapshot) {
