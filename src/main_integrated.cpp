...
int main() {
  try {
    ...
    dbConfig.port =
        dbPortEnv ? std::stoi(dbPortEnv) : config.getInt("database.port", 5432);
    ...
    LOG_INFO("Main", "✓ WebSocket manager is running (connections: " +
                         std::to_string(wsConnections) + ")");

    // Check ETL job manager
    size_t activeJobs = jobMonitor->getActiveJobCount();
    LOG_INFO("Main", "✓ ETL job manager is running (active jobs: " +
                         std::to_string(activeJobs) + ")");

    if (!allServicesHealthy) {
      LOG_FATAL("Main",
                "Some services failed to start properly. Shutting down...");
      return 1;
    }
    ...
    LOG_INFO(
        "Main",
        "ETL Plus Backend with Real-time Monitoring is fully operational!");
    LOG_INFO("Main", "Available endpoints:");
    LOG_INFO("Main", "  - HTTP API: http://" + address + ":" +
                         std::to_string(port) + "/api/");
    LOG_INFO("Main", "  - WebSocket: ws://" + address + ":" +
                         std::to_string(port) + "/ws");
    LOG_INFO("Main", "  - Health Check: http://" + address + ":" +
                         std::to_string(port) + "/health");
    LOG_INFO("Main", "  - Monitoring: http://" + address + ":" +
                         std::to_string(port) + "/api/monitor/");
    LOG_INFO("Main", "");
    ...
    std::thread healthMonitor([&]() {
      while (server->isRunning()) {
        ...
        size_t processedNotifications =
            notificationService->getProcessedCount();

        LOG_INFO("Main",
                 "System Status - WS Connections: " +
                     std::to_string(currentConnections) +
                     ", Active Jobs: " + std::to_string(currentActiveJobs) +
                     ", Notification Queue: " +
                     std::to_string(notificationQueueSize) +
                     ", Processed Notifications: " +
                     std::to_string(processedNotifications));
        ...
        if (currentActiveJobs > 50) {
          LOG_WARN("Main", "High number of active jobs detected: " +
                               std::to_string(currentActiveJobs));
        }

        if (currentConnections > 100) {
          LOG_WARN("Main", "High number of WebSocket connections: " +
                               std::to_string(currentConnections));
        }
      }
    });
  ...
  LOG_INFO("Main",
           "ETL Plus Backend with Real-time Monitoring shutdown complete");
  return 0;
}