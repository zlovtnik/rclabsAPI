...
RequestValidator::RequestValidator(ValidationConfig config)
  ...
  etl::ComponentLogger<RequestValidator>::info(
      "RequestValidator initialized with config");
}

RequestValidator::ValidationResult
RequestValidator::validateRequest(const http::request<http::string_body> &req) {

  stats_.totalRequests++;

  etl::ComponentLogger<RequestValidator>::debug(
      "Validating request: " + std::string(req.method_string()) + " " +
  ...
  if (!basicResult.isValid) {
    result.errors.insert(result.errors.end(), basicResult.errors.begin(),
                         basicResult.errors.end());
  ...
  if (target.rfind("/api/auth", 0) == 0) {
    ...
    if (!jobsResult.isValid) {
      result.errors.insert(result.errors.end(), jobsResult.errors.begin(),
                           jobsResult.errors.end());
      result.isValid = false;
    }
  } else if (target.rfind("/api/logs", 0) == 0) {
    auto logsResult = validateLogsEndpoint(req);
    if (!logsResult.isValid) {
      result.errors.insert(result.errors.end(), logsResult.errors.begin(),
                           logsResult.errors.end());
      result.isValid = false;
    }
  } else if (target.rfind("/api/monitor", 0) == 0) {
    auto monitorResult = validateMonitoringEndpoint(req);
    if (!monitorResult.isValid) {
      result.errors.insert(result.errors.end(), monitorResult.errors.begin(),
                           monitorResult.errors.end());
      result.isValid = false;
    }
    ...
    if (!healthResult.isValid) {
      result.errors.insert(result.errors.end(), healthResult.errors.begin(),
                           healthResult.errors.end());
      result.isValid = false;
    }
  } else if (!isKnownEndpoint(target)) {
    result.addError("endpoint", "Unknown endpoint: " + target,
                    "UNKNOWN_ENDPOINT");
  }
  ...
  if (result.isValid) {
    ...
    stats_.invalidRequests++;
    etl::ComponentLogger<RequestValidator>::warn(
...
RequestValidator::ValidationResult RequestValidator::validateRequestBasics(
  ...
  result.extractedPath =
      (queryPos != std::string::npos) ? target.substr(0, queryPos) : target;

  // Validate path format
  auto pathResult = validatePath(result.extractedPath);
  if (!pathResult.isValid) {
    result.errors.insert(result.errors.end(), pathResult.errors.begin(),
  ...
  if (!headerResult.isValid) {
    result.errors.insert(result.errors.end(), headerResult.errors.begin(),
  ...
  if (queryPos != std::string::npos) {
    ...
    if (!queryResult.isValid) {
      result.errors.insert(result.errors.end(), queryResult.errors.begin(),
  ...
  if (!contentLengthResult.isValid) {
    result.errors.insert(result.errors.end(),
                         contentLengthResult.errors.begin(),
                         contentLengthResult.errors.end());
    result.isValid = false;
  }
...
RequestValidator::SecurityValidationResult RequestValidator::validateSecurity(
  ...
  if (config_.maxRequestsPerMinute > 0) {
    ...
    if (result.rateLimitExceeded) {
      etl::ComponentLogger<RequestValidator>::warn(
          "Rate limit exceeded for IP: " + sanitizeLogString(result.clientIp));
    }
  }
  ...
  if (config_.enableSqlInjectionProtection) {
    ...
    if (checkForSqlInjection(target) || checkForSqlInjection(req.body())) {
      ...
      etl::ComponentLogger<RequestValidator>::error(
  ...
  if (config_.enableXssProtection) {
    ...
    if (checkForXssAttempts(target) || checkForXssAttempts(req.body())) {
      ...
      etl::ComponentLogger<RequestValidator>::error(
          "XSS attempt from IP: " + sanitizeLogString(result.clientIp));
    }
  }
...
RequestValidator::extractHeaders(const http::request<http::string_body> &req) {
  ...
  for (auto const &field : req) {
    try {
      ...
      etl::ComponentLogger<RequestValidator>::warn(
          "Failed to extract header: " + std::string(e.what()));
    }
  }
...
RequestValidator::ValidationResult
RequestValidator::validatePath(std::string_view path) {
  ValidationResult result;
  ...
  if (path.find('\0') != std::string::npos) {
    result.addError("path", "Null bytes not allowed in path",
                    "NULL_BYTE_IN_PATH");
  }
  ...
  if (!std::regex_match(std::string(path), validPathRegex)) {
    result.addError("path", "Path contains invalid characters",
                    "INVALID_PATH_CHARS");
  }
...
RequestValidator::ValidationResult
RequestValidator::validateHeaders(const http::request<http::string_body> &req) {
  ...
  for (auto const &field : req) {
    ...
    if (name.length() + value.length() > config_.maxHeaderSize) {
      result.addError("headers", "Header too large: " + name,
                      "HEADER_TOO_LARGE");
    }

    if (!isValidHeaderName(name)) {
      result.addError("headers", "Invalid header name: " + name,
                      "INVALID_HEADER_NAME");
    }

    if (!isValidHeaderValue(value)) {
      result.addError("headers", "Invalid header value for: " + name,
...
RequestValidator::ValidationResult
RequestValidator::validateQueryParameters(std::string_view target) {
  ValidationResult result;
  ...
  while (std::getline(iss, pair, '&')) {
    ...
    size_t equalPos = pair.find('=');
    if (equalPos == std::string::npos) {
      result.addError("query", "Invalid query parameter format: " + pair,
    ...
    if (config_.enableSqlInjectionProtection &&
      ...
      result.addError("query", "Potential SQL injection in parameter: " + key,
                      "SQL_INJECTION");
    }

    if (config_.enableXssProtection &&
        (checkForXssAttempts(key) || checkForXssAttempts(value))) {
      result.addError("query", "Potential XSS in parameter: " + key,
                      "XSS_ATTEMPT");
    }
  }
...
RequestValidator::ValidationResult
RequestValidator::validateContentLength(size_t contentLength) {
  ValidationResult result;

  if (contentLength > config_.maxRequestSize) {
    result.addError("content_length", "Request body too large",
                    "REQUEST_TOO_LARGE");
  }
...
RequestValidator::ValidationResult RequestValidator::validateAuthEndpoint(
  ...
  if (target == "/api/auth/login") {
    if (method != "POST") {
      result.addError("method", "Login endpoint requires POST method",
                      "INVALID_METHOD");
    }
    ...
    if (method != "POST") {
      result.addError("method", "Logout endpoint requires POST method",
                      "INVALID_METHOD");
    }
    ...
    if (auto authIt = headers.find("authorization"); authIt != headers.end()) {
      ...
      result.addError("authorization",
                      "Authorization header required for logout",
                      "MISSING_AUTH");
    }
  } else if (target == "/api/auth/profile") {
    if (method != "GET") {
      result.addError("method", "Profile endpoint requires GET method",
                      "INVALID_METHOD");
    }
    ...
    if (auto authIt = headers.find("authorization"); authIt != headers.end()) {
      ...
      result.addError("authorization",
                      "Authorization header required for profile",
                      "MISSING_AUTH");
    }
  }
...
RequestValidator::ValidationResult RequestValidator::validateJobsEndpoint(
  ...
  if (path == "/api/jobs") {
    if (method == "GET") {
      ...
      if (!req.body().empty()) {
        auto bodyResult =
            InputValidator::validateJobCreationRequest(req.body());
        if (!bodyResult.isValid) {
          for (const auto &error : bodyResult.errors) {
            result.addError(error.field, error.message, error.code);
          }
      ...
      result.addError("method",
                      "Jobs endpoint supports GET and POST methods only",
                      "INVALID_METHOD");
    }
    ...
    if (jobId.empty()) {
      ...
      result.addError("job_id", "Job ID format is invalid",
    ...
    if (method == "GET") {
      ...
      result.addError("method", "Invalid method for job operations",
                      "INVALID_METHOD");
    }
  }
...
RequestValidator::ValidationResult RequestValidator::validateLogsEndpoint(
  ...
  if (method != "GET") {
    result.addError("method", "Logs endpoint only supports GET method",
                    "INVALID_METHOD");
    return result;
  }
  ...
  if (auto levelIt = queryParams.find("level"); levelIt != queryParams.end()) {
    ...
    if (validLevels.find(levelIt->second) == validLevels.end()) {
      result.addError("level", "Invalid log level: " + levelIt->second,
                      "INVALID_LOG_LEVEL");
    }
  }
  ...
  if (auto limitIt = queryParams.find("limit"); limitIt != queryParams.end()) {
    try {
      ...
      if (limit < 1 || limit > 10000) {
        result.addError("limit", "Limit must be between 1 and 10000",
                        "INVALID_LIMIT");
      }
    } catch (const std::exception &) {
      result.addError("limit", "Limit must be a valid number",
...
RequestValidator::ValidationResult RequestValidator::validateMonitoringEndpoint(
  ...
  if (method != "GET") {
    result.addError("method", "Monitoring endpoints only support GET method",
                    "INVALID_METHOD");
    return result;
  }
...
RequestValidator::ValidationResult RequestValidator::validateHealthEndpoint(
  ...
  if (method != "GET") {
    result.addError("method", "Health endpoint only supports GET method",
                    "INVALID_METHOD");
    return result;
  }
...
bool RequestValidator::checkForSqlInjection(const std::string &input) {
  static const std::vector<std::string> sqlKeywords = {
      ...
      "expression", "url",         "link",     "import", "include"};
  ...
  static const std::vector<std::regex> sqlPatterns = {
      std::regex(
          R"(\b(union|select|insert|update|delete)\b.*\b(from|where|into)\b)",
          std::regex::icase),
      std::regex(R"((\%27)|(\')|(\\x27)|(\\u0027))", std::regex::icase),
      std::regex(R"((\%3D)|(=)[^\n]*((\%27)|(\')|(\\x27)|(\\u0027)))",
                 std::regex::icase),
      std::regex(R"(\w*((\%27)|(\'))((\%6F)|o|(\%4F))((\%72)|r|(\%52)))",
                 std::regex::icase)};
...
bool RequestValidator::checkForXssAttempts(const std::string &input) {
  static const std::vector<std::string> xssPatterns = {"<script",
                                                       "</script>",
                                                       "javascript:",
                                                       "vbscript:",
                                                       "onload=",
...
std::string
RequestValidator::extractClientIp(const http::request<http::string_body> &req) {
  auto headers = extractHeaders(req);
...
std::string RequestValidator::extractUserAgent(
    const http::request<http::string_body> &req) {
  auto headers = extractHeaders(req);
...
void RequestValidator::initializeKnownEndpoints() {
  knownEndpoints_ = {"/api/auth/login",     "/api/auth/logout",
                     "/api/auth/profile",   "/api/jobs",
                     "/api/logs",           "/api/monitor/jobs",
                     "/api/monitor/status", "/api/monitor/metrics",
...
std::string RequestValidator::ValidationResult::toJsonString() const {
  ...
  if (!errors.empty()) {
    ...
    for (size_t i = 0; i < errors.size(); ++i) {
      if (i > 0)
        oss << ",";
      oss << "{\"field\":\"" << errors[i].field << "\",\"message\":\""
          << errors[i].message << "\",\"code\":\"" << errors[i].code << "\"}";
    }
    oss << "]";
  }
...
RequestValidator::ValidationResult
RequestValidator::validateAuthenticationHeader(const std::string &authHeader) {

  ValidationResult result;

  if (authHeader.empty()) {
    result.addError("authorization", "Authorization header is empty",
                    "EMPTY_AUTH_HEADER");
    return result;
  }
  ...
  if (authHeader.find("Bearer ") == 0) {
    ...
    if (!tokenResult.isValid) {
      result.errors.insert(result.errors.end(), tokenResult.errors.begin(),
                           tokenResult.errors.end());
      result.isValid = false;
    }
  } else {
    result.addError("authorization", "Invalid authorization format",
                    "INVALID_AUTH_FORMAT");
  }
...
RequestValidator::ValidationResult
RequestValidator::validateBearerToken(const std::string &token) {
  ValidationResult result;
...
std::string RequestValidator::extractJobIdFromPath(std::string_view target,
...
std::string
RequestValidator::extractConnectionIdFromPath(const std::string &target,
...
bool RequestValidator::validateHttpsRequirement(
    const http::request<http::string_body> &req) {
  auto headers = extractHeaders(req);
...
void RequestValidator::updateConfig(const ValidationConfig &newConfig) {
  config_ = newConfig;
  etl::ComponentLogger<RequestValidator>::info(
      "RequestValidator configuration updated");
}

void RequestValidator::resetStats() {
  stats_ = ValidationStats{};
  etl::ComponentLogger<RequestValidator>::info(
      "RequestValidator statistics reset");
}
...
RequestValidator::ValidationResult
RequestValidator::validateMethodForEndpoint(const std::string &method,
  ...
  if (!isValidMethod(method, path)) {
    result.addError("method",
                    "Method " + method + " not allowed for endpoint " + path,
                    "METHOD_NOT_ALLOWED");
  }
...
RequestValidator::ValidationResult
RequestValidator::validateEndpoint(const std::string &method,
                                   const std::string &path) {
  ...
  if (!methodResult.isValid) {
    result.errors.insert(result.errors.end(), methodResult.errors.begin(),
                         methodResult.errors.end());
    result.isValid = false;
  }

  // Check if endpoint is known
  if (!isKnownEndpoint(path)) {
    result.addError("endpoint", "Unknown endpoint: " + path,
                    "UNKNOWN_ENDPOINT");
  }
...
RequestValidator::ValidationResult
RequestValidator::validateJsonBody(const std::string &body) {
  ValidationResult result;
...
RequestValidator::ValidationResult
RequestValidator::validateBody(const std::string &body,
...
RequestValidator::ValidationResult
RequestValidator::validateContentType(const std::string &contentType,
                                      const std::string &endpoint) {
  ...
  if (it != allowedContentTypes.end()) {
    ...
    if (!found) {
      result.addError("content_type", "Invalid content type for endpoint",
...
bool RequestValidator::validateCsrfToken(const std::string &token,
