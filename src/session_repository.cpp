...
bool SessionRepository::createSession(const Session& session) {
    ...
    try {
        ...
        std::string query = "INSERT INTO sessions (session_id, user_id, created_at, expires_at, is_valid) "
                           "VALUES ('" + session.sessionId + "', '" + session.userId + "', '" + 
...
std::optional<Session> SessionRepository::getSessionById(const std::string& sessionId) {
    if (!dbManager_ || !dbManager_->isConnected()) {
        AUTH_LOG_ERROR("Database not connected");
        return std::nullopt;
    }
    ...
    try {
        std::string query = "SELECT session_id, user_id, created_at, expires_at, is_valid "
...
std::vector<Session> SessionRepository::getSessionsByUserId(const std::string& userId) {
    ...
    try {
        ...
        AUTH_LOG_ERROR("Failed to get sessions by user ID: " + std::string(e.what()));
    }
...
bool SessionRepository::updateSession(const Session& session) {
    ...
    try {
        ...
        std::string query = "UPDATE sessions SET expires_at = '" + expiresAtStr + 
...
bool SessionRepository::deleteSession(const std::string& sessionId) {
    ...
    try {
        std::string query = "DELETE FROM sessions WHERE session_id = '" + sessionId + "'";
        return dbManager_->executeQuery(query);
    } catch (const std::exception& e) {
...
bool SessionRepository::deleteExpiredSessions() {
    ...
    try {
        ...
        std::string query = "DELETE FROM sessions WHERE expires_at < '" + nowStr + "'";
        return dbManager_->executeQuery(query);
    } catch (const std::exception& e) {
        AUTH_LOG_ERROR("Failed to delete expired sessions: " + std::string(e.what()));
        return false;
    }
}
...
std::vector<Session> SessionRepository::getValidSessions() {
    ...
    try {
        std::string nowStr = timePointToString(std::chrono::system_clock::now());
        std::string query = "SELECT session_id, user_id, created_at, expires_at, is_valid "
                           "FROM sessions WHERE is_valid = true AND expires_at > '" + nowStr + 
...
std::string SessionRepository::timePointToString(const std::chrono::system_clock::time_point& tp) {
...
std::chrono::system_clock::time_point SessionRepository::stringToTimePoint(const std::string& str) {
    std::tm tm = {};
