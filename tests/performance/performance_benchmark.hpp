...
public:
  struct BenchmarkResult {
    ...
    std::string toString() const {
      std::stringstream ss;
      ss << std::left << std::setw(30) << name << std::right << std::setw(10)
         << operations << std::setw(8) << duration.count() << "ms"
         << std::setw(12) << std::fixed << std::setprecision(0) << throughput
         << " ops/sec" << std::setw(10) << (memoryUsage / 1024) << "KB"
  ...
  static void printHeader() {
    ...
    std::cout << std::left << std::setw(30) << "Benchmark" << std::right
              << std::setw(10) << "Operations" << std::setw(8) << "Time"
              << std::setw(12) << "Throughput" << std::setw(10) << "Memory"
              << std::setw(8) << "CPU" << "\n";
  ...
  static void printSummary(const std::vector<BenchmarkResult> &results) {
    ...
    if (maxMemory > 100 * 1024 * 1024) { // 100MB
      std::cout
          << "- High memory usage detected, consider memory optimizations\n";
    }
    if (maxCpu > 80.0) {
      std::cout
          << "- High CPU usage, consider load distribution or optimization\n";
    }
  }
};
...
protected:
  ...
  createResult(const std::string &subName, size_t operations,
               std::chrono::milliseconds duration,
