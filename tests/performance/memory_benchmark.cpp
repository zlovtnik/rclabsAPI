...
private:
  ...
  void benchmarkMemoryLeakDetection() {
    ...
    for (size_t i = 0; i < numCycles; ++i) {
      ...
      auto tempObject = std::make_shared<std::string>("Temporary object " +
                                                      std::to_string(i));
    ...
    addResult(
        createResult("Memory Leak Detection", numCycles, duration,
  ...
  void benchmarkObjectPooling() {
    ...
    for (size_t i = 0; i < poolSize; ++i) {
      pool.emplace_back(
          std::make_unique<std::string>("Pooled object " + std::to_string(i)));
    ...
    addResult(createResult(
        "Object Pooling", numOperations, duration,
  ...
  void benchmarkCacheEfficiency() {
    ...
    for (size_t i = 0; i < numLookups; ++i) {
      std::string key =
          "key_" + std::to_string(i % (cacheSize * 2)); // Some misses
    ...
    addResult(createResult("Cache Efficiency", numLookups, duration,
                           "Hit rate: " + std::to_string(hitRate) + "%, " +
                               "Memory: " + std::to_string(memoryDelta) +
                               " KB"));
  }
};
