...
private:
    void benchmarkMessageThroughput() {
        ...
        const std::string testMessage = R"({"type":"status","data":{"progress":50}})";
    ...
    void benchmarkConcurrentClients() {
        ...
        addResult(createResult("Concurrent Clients", totalMessages, duration,
    ...
    void benchmarkMessageLatency() {
        ...
        addResult(createResult("Message Latency", numLatencyTests, totalDuration,
                              "Average latency: " + std::to_string(avgLatency) + " microseconds"));
    }
    ...
    void benchmarkConnectionHandling() {
        ...
        for (size_t i = 0; i < numConnections; ++i) {
            connectionThreads.emplace_back([&, i]() {
                ...
                std::this_thread::sleep_for(std::chrono::milliseconds(connectionDurationMs));
