...
private:
  void generateReport(
      const std::vector<std::unique_ptr<BenchmarkBase>> &benchmarks) {
    std::cout << "========================================\n";
  ...
  void displayResultsByCategory(
      const std::vector<PerformanceBenchmark::BenchmarkResult> &results) {
    std::map<std::string, std::vector<PerformanceBenchmark::BenchmarkResult>>
        categorizedResults;
    ...
    for (const auto &[category, categoryResults] : categorizedResults) {
      ...
      for (const auto &result : categoryResults) {
        std::cout << std::left << std::setw(30) << result.name << std::right
                  << std::setw(10) << result.operations << " ops" << std::right
                  << std::setw(10) << result.duration.count() << " ms"
                  << std::right << std::setw(12) << std::fixed
                  << std::setprecision(2) << result.throughput << " ops/sec"
                  << "  " << result.notes << "\n";
      }
      std::cout << "\n";
    }
  ...
  void generatePerformanceSummary(
    ...
    double overallOpsPerSecond =
        static_cast<double>(totalOperations) / (totalDuration.count() / 1000.0);
  ...
  void analyzePerformanceThresholds(
      const std::vector<PerformanceBenchmark::BenchmarkResult> &results) {
    ...
    for (const auto &[category, throughputs] : categoryThroughputs) {
      ...
      if (it != thresholds.end()) {
        ...
        std::cout << std::left << std::setw(15) << category << std::right
                  << std::setw(10) << std::fixed << std::setprecision(2)
                  << avgThroughput << " ops/sec" << std::right << std::setw(10)
                  << threshold << " ops/sec" << std::right << std::setw(8)
                  << status << "\n";
      }
    }
    std::cout << "\n";
...
private:
  ...
  void displayResultsByCategory(const std::vector<BenchmarkResult> &results) {
    ...
    for (const auto &[category, categoryResults] : categorizedResults) {
      ...
      for (const auto &result : categoryResults) {
        std::cout << std::left << std::setw(30) << result.testName << std::right
                  << std::setw(10) << result.operations << " ops" << std::right
                  << std::setw(10) << result.durationMs.count() << " ms"
                  << std::right << std::setw(12) << std::fixed
                  << std::setprecision(2)
  ...
  void
  analyzePerformanceThresholds(const std::vector<BenchmarkResult> &results) {
    std::cout << "PERFORMANCE THRESHOLDS ANALYSIS\n";
    std::cout << "================================\n\n";
