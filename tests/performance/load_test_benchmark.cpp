...
private:
    void benchmarkConcurrentRequests() {
            ...
            clientThreads.emplace_back([&, i]() {
                for (size_t j = 0; j < requestsPerClient; ++j) {
                    try {
                        ...
                        std::string request = "GET /api/data/" + std::to_string(j) + " HTTP/1.1";
        ...
        double successRate = static_cast<double>(completedRequests) / totalRequests * 100.0;
                    ...
                    switch (op) {
                        ...
                        case 2: { // WebSocket message
                            std::string message = R"({"type":"update","thread":)" +
                                               std::to_string(i) + R"(,"op":)" + std::to_string(j) + "}";
    ...
    void benchmarkSpikeLoad() {
        ...
        for (size_t i = 0; i < baseLoad; ++i) {
            baseThreads.emplace_back([&, i]() {
                while (std::chrono::duration_cast<std::chrono::milliseconds>(
                       std::chrono::high_resolution_clock::now() - start).count() < spikeDurationMs / 2) {
                    auto connection = poolManager.acquireConnection();
                    std::this_thread::sleep_for(std::chrono::microseconds(200));
                    poolManager.releaseConnection(connection);
                    totalRequests++;
        ...
        for (size_t i = 0; i < spikeLoad; ++i) {
            spikeThreads.emplace_back([&, i]() {
                while (std::chrono::duration_cast<std::chrono::milliseconds>(
                       std::chrono::high_resolution_clock::now() - start).count() < spikeDurationMs) {
                    auto connection = poolManager.acquireConnection();
                    std::this_thread::sleep_for(std::chrono::microseconds(100));
                    poolManager.releaseConnection(connection);
                    totalRequests++;
    ...
    void benchmarkSustainedLoad() {
        ...
        for (size_t i = 0; i < numWorkerThreads; ++i) {
            workerThreads.emplace_back([&, i]() {
                while (std::chrono::duration_cast<std::chrono::milliseconds>(
                       std::chrono::high_resolution_clock::now() - start).count() < testDurationMs) {
        ...
        double successRate = static_cast<double>(successfulOperations) / totalOperations * 100.0;
        double opsPerSecond = static_cast<double>(totalOperations) / (duration.count() / 1000.0);
