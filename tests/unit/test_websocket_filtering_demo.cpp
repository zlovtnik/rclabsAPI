...
private:
  void demonstrateMessageTypeFiltering() {
    ...
    wsManager->broadcastByMessageType(
        jobStatusMsg, MessageType::JOB_STATUS_UPDATE, "job_123");
    ...
    wsManager->broadcastByMessageType(logMsg, MessageType::JOB_LOG_MESSAGE,
                                      "job_123");
    ...
    wsManager->broadcastByMessageType(notificationMsg,
  ...
  void demonstrateJobIdFiltering() {
    ...
    for (const auto &jobId : jobIds) {
      std::string jobMsg = R"({"type":"job_update","jobId":")" + jobId +
                           R"(","status":"PROCESSING"})";
...
void demonstrateFilterConfiguration() {
  ...
  std::cout << "✓ Job-specific filter configured for "
            << jobSpecificFilter.jobIds.size() << " jobs\n";
  ...
  std::cout << "✓ Error-only filter configured for "
            << errorOnlyFilter.logLevels.size() << " log levels\n";
  ...
  std::cout << "✓ Status-only filter configured for "
            << statusOnlyFilter.messageTypes.size() << " message types\n";
  ...
  std::cout << "✓ Combined filter configured with job, message type, and log "
               "level restrictions\n";
}
...
int main() {
  try {
    ...
    std::cerr << "Filtering demo failed with exception: " << e.what()
              << std::endl;
    return 1;
  }
}