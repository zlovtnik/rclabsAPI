...
TEST_F(ResponseBuilderTest, CreateSuccessResponseWithContentType) {
    auto response = builder_->success("Hello World", ResponseBuilder::ContentType::TEXT);
...
TEST_F(ResponseBuilderTest, CreateSuccessWithMessage) {
    auto response = builder_->successWithMessage("Operation completed", R"({"id":123})");
    ...
    EXPECT_TRUE(body.find(R"("message":"Operation completed")") != std::string::npos);
...
TEST_F(ResponseBuilderTest, CreateBadRequestResponse) {
    ...
    std::string body = response.body();
    EXPECT_TRUE(body.find(R"("error":"Missing required field")") != std::string::npos);
}
...
TEST_F(ResponseBuilderTest, CreateTooManyRequestsResponse) {
    ...
    std::string body = response.body();
    EXPECT_TRUE(body.find(R"("error":"Rate limit exceeded")") != std::string::npos);
}

// Fluent Interface Tests
TEST_F(ResponseBuilderTest, FluentInterfaceSetStatus) {
    auto response = builder_->setStatus(http::status::created)
                           .success(R"({"id":123})");
...
TEST_F(ResponseBuilderTest, FluentInterfaceSetKeepAlive) {
    auto response = builder_->setKeepAlive(true)
                           .success("test");
...
TEST_F(ResponseBuilderTest, CreateResponseFromETLException) {
    etl::ValidationException ex(etl::ErrorCode::INVALID_INPUT, "Invalid field value", "field1", "invalid");
...
TEST_F(ResponseBuilderTest, CreateResponseFromStandardException) {
    ...
    std::string body = response.body();
    EXPECT_TRUE(body.find("Database operation: Database connection failed") != std::string::npos);
}
...
TEST_F(ResponseBuilderTest, CreateAuthenticationRequiredResponse) {
    ...
    EXPECT_EQ(response.result(), http::status::unauthorized);
    EXPECT_EQ(response[http::field::www_authenticate], "Bearer realm=\"ETL API\"");
...
TEST_F(ResponseBuilderTest, CreateRedirectResponse) {
    ...
    EXPECT_EQ(response[http::field::location], "https://example.com/new-location");
}
...
TEST_F(ResponseBuilderTest, CreateCachedResponse) {
    auto response = builder_->cached("cached content", std::chrono::seconds(3600));
...
TEST_F(ResponseBuilderTest, CreateNoCacheResponse) {
    ...
    EXPECT_EQ(response.result(), http::status::ok);
    EXPECT_EQ(response[http::field::cache_control], "no-cache, no-store, must-revalidate");
...
TEST_F(ResponseBuilderTest, CorsHeadersApplied) {
    ...
    auto response = builder_->setCors(corsConfig)
                           .success("test");
...
TEST_F(ResponseBuilderTest, ContentTypeToString) {
    EXPECT_EQ(ResponseBuilder::contentTypeToString(ResponseBuilder::ContentType::JSON), 
              "application/json");
    EXPECT_EQ(ResponseBuilder::contentTypeToString(ResponseBuilder::ContentType::XML), 
              "application/xml");
    EXPECT_EQ(ResponseBuilder::contentTypeToString(ResponseBuilder::ContentType::HTML), 
              "text/html; charset=utf-8");
    EXPECT_EQ(ResponseBuilder::contentTypeToString(ResponseBuilder::ContentType::TEXT), 
...
TEST_F(ResponseBuilderTest, EscapeJsonString) {
    EXPECT_EQ(ResponseBuilder::escapeJsonString("Hello \"World\""), "Hello \\\"World\\\"");
    EXPECT_EQ(ResponseBuilder::escapeJsonString("Line 1\nLine 2"), "Line 1\\nLine 2");
    EXPECT_EQ(ResponseBuilder::escapeJsonString("Tab\tSeparated"), "Tab\\tSeparated");
}

TEST_F(ResponseBuilderTest, StatusToReasonPhrase) {
    EXPECT_EQ(ResponseBuilder::statusToReasonPhrase(http::status::ok), "OK");
    EXPECT_EQ(ResponseBuilder::statusToReasonPhrase(http::status::bad_request), "Bad Request");
    EXPECT_EQ(ResponseBuilder::statusToReasonPhrase(http::status::not_found), "Not Found");
    EXPECT_EQ(ResponseBuilder::statusToReasonPhrase(http::status::internal_server_error), 
