...
TEST_F(RequestValidatorTest, ValidateBasicPostRequest) {
    auto req = createRequest("POST", "/api/auth/login", R"({"username":"test","password":"pass"})");
...
TEST_F(RequestValidatorTest, ValidateHeaderCount) {
    ...
    for (int i = 0; i < 15; ++i) {
        req.set("x-custom-header-" + std::to_string(i), "value" + std::to_string(i));
    }
...
TEST_F(RequestValidatorTest, ValidateQueryParametersFormat) {
    auto req = createRequest("GET", "/api/jobs?invalid_param_without_value&valid=value");
...
TEST_F(RequestValidatorTest, DetectSqlInjection) {
    ...
    EXPECT_TRUE(validator_->checkForSqlInjection("UNION SELECT * FROM passwords"));
...
TEST_F(RequestValidatorTest, ValidateSecurityInQueryParams) {
    auto req = createRequest("GET", "/api/jobs?search=<script>alert('xss')</script>");
...
TEST_F(RequestValidatorTest, ValidateAuthLoginEndpoint) {
    auto req = createRequest("POST", "/api/auth/login", R"({"username":"test","password":"password123"})");
    req.set(http::field::content_type, "application/json");
...
TEST_F(RequestValidatorTest, ValidateBearerToken) {
    auto result = validator_->validateAuthenticationHeader("Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9");
...
TEST_F(RequestValidatorTest, ExtractJobIdFromPath) {
    EXPECT_EQ(validator_->extractJobIdFromPath("/api/jobs/job-123", "/api/jobs/", ""), "job-123");
    EXPECT_EQ(validator_->extractJobIdFromPath("/api/jobs/job-123/status", "/api/jobs/", "/status"), "job-123");
    EXPECT_EQ(validator_->extractJobIdFromPath("/api/jobs/", "/api/jobs/", ""), "");
    EXPECT_EQ(validator_->extractJobIdFromPath("/api/other/job-123", "/api/jobs/", ""), "");
}

TEST_F(RequestValidatorTest, ExtractConnectionIdFromPath) {
    EXPECT_EQ(validator_->extractConnectionIdFromPath("/api/websocket/conn-123/filters", "/api/websocket/"), "conn-123");
    EXPECT_EQ(validator_->extractConnectionIdFromPath("/api/websocket/conn-456", "/api/websocket/"), "conn-456");
    EXPECT_EQ(validator_->extractConnectionIdFromPath("/api/websocket/", "/api/websocket/"), "");
}
...
TEST_F(RequestValidatorTest, RecognizeKnownEndpoints) {
    ...
    EXPECT_TRUE(validator_->isKnownEndpoint("/api/jobs/job-123")); // Parameterized
