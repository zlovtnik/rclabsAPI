...
int main() {
  try {
    ...
    if (!testUsername || !testPassword) {
      std::cerr << "Error: TEST_USERNAME and TEST_PASSWORD environment "
    ...
    nlohmann::json loginBody = {{"username", std::string(testUsername)},
    ...
    if (loginResponse.result() != http::status::ok) {
      std::cerr << "Login failed with status: " << loginResponse.result_int()
                << std::endl;
      return 1;
    }
    ...
    try {
      ...
      if (ct == loginResponse.base().end() ||
          ct->value().find("application/json") ==
              boost::beast::string_view::npos) {
        std::cerr << "Login response missing/invalid Content-Type" << std::endl;
        return 1;
      }
      if (!jsonResponse.contains("token") ||
          !jsonResponse["token"].is_string() ||
          jsonResponse["token"].get<std::string>().empty()) {
        std::cerr << "Login response missing or invalid token" << std::endl;
        return 1;
      }
      token = jsonResponse["token"];
      std::cout << "Token extracted successfully (length: " << token.length()
                << ")" << std::endl;
    ...
    std::cout << "Profile Response Status: " << profileResponse.result_int()
              << std::endl;
    std::cout << "Profile Response Body: " << profileResponse.body()
              << std::endl;

    // Assert profile response + rate-limit headers
    if (profileResponse.result() != http::status::ok) {
      std::cerr << "Profile access failed with status: "
                << profileResponse.result_int() << std::endl;
      return 1;
    }

    // Check for rate-limit headers (custom headers need to be checked
    // differently)
    ...
    for (auto const &field : profileResponse.base()) {
      ...
      if (fieldName == "X-RateLimit-Limit")
        hasRateLimitLimit = true;
      else if (fieldName == "X-RateLimit-Remaining")
        hasRateLimitRemaining = true;
      else if (fieldName == "X-RateLimit-Reset")
        hasRateLimitReset = true;
    }
    ...
    std::cout << "No Auth Response Status: " << noAuthResponse.result_int()
              << std::endl;
    std::cout << "No Auth Response Body: " << noAuthResponse.body()
              << std::endl;

    // Assert unauthorized access
    if (noAuthResponse.result() != http::status::unauthorized) {
      std::cerr << "Expected unauthorized status, got: "
                << noAuthResponse.result_int() << std::endl;
      return 1;
    }
    ...
    if (badTokResp.result() != http::status::unauthorized) {
      std::cerr << "Expected 401 for tampered token, got: "
                << badTokResp.result_int() << std::endl;
      return 1;
    }

    std::cout << "\n=== JWT Authentication Test Complete - All tests passed ==="
              << std::endl;
