...
TEST_F(IntegrationTest, CompleteRequestWorkflow) {
    ...
    // Simulate a complete request workflow with proper locking
    {
        ...
        try {
            ...
            throw etl::ValidationException(etl::ErrorCode::INVALID_INPUT, "Invalid request data", "email", "invalid@email");
        } catch (const etl::ETLException& e) {
            // Verify the exception details
            EXPECT_TRUE(std::string(e.what()).find("Invalid request data") != std::string::npos);
...
TEST_F(IntegrationTest, CrossComponentExceptionHandling) {
    ...
    // Test that exceptions from different components are handled consistently
    {
        ...
        try {
            throw etl::ValidationException(etl::ErrorCode::INVALID_INPUT, "Validation failed", "field", "value");
...

    {
        ...
        try {
            throw etl::SystemException(etl::ErrorCode::DATABASE_ERROR, "System error occurred");
...
TEST_F(IntegrationTest, StrongIdConcurrentUsage) {
    ...
    for (int i = 0; i < 5; ++i) {
        threads.emplace_back([&]() {
            ...
            connectionData[conn1] = "Connection data from thread " + std::to_string(i);
        });
    }
    ...
    EXPECT_TRUE(connectionData[conn1].find("Connection data") != std::string::npos);
}
...
TEST_F(IntegrationTest, ErrorRecoveryAndLogging) {
...

    {
        ...
        try {
            ...
            throw etl::SystemException(etl::ErrorCode::NETWORK_ERROR, "Temporary failure");
        } catch (const etl::ETLException& e) {
            errorOccurred = true;
            errorMessage = e.what();
        }
...
TEST_F(IntegrationTest, TimeoutHandling) {
    ...
    auto future = std::async(std::launch::async, [&]() {
        try {
            ScopedTimedLock<ConfigMutex> lock(configMutex, std::chrono::milliseconds(100));
...
TEST_F(IntegrationTest, MemorySafetyRAII) {
    ...
    class TestResource {
    public:
        TestResource(std::atomic<int>& counter) : counter_(counter) {
            counter_++;
        }
        ~TestResource() {
            counter_--;
