...
protected:
  void SetUp() override { handler_ = std::make_unique<MockLogHandler>(); }
...
TEST(ComponentLoggerTest, MultipleComponents) {
  ...
  // Test different component types - these use the predefined ComponentTrait
  // specializations
  ...
  for (int i = 0; i < numThreads; ++i) {
    threads.emplace_back([i, messagesPerThread]() {
      ...
      for (int j = 0; j < messagesPerThread; ++j) {
        logger.info("HttpServer", "Thread " + std::to_string(i) + " message " +
                                      std::to_string(j));
      }
    });
  }
