...
protected:
  void SetUp() override { rateLimiter = std::make_unique<RateLimiter>(); }
...
TEST_F(RateLimiterTest, BasicRateLimiting) {
  ...
  std::string endpoint =
      "/api/auth/login"; // Use endpoint with low limit (5 per minute)
  ...
  EXPECT_FALSE(rateLimiter->isAllowed(clientId, endpoint))
