...
public:
   ...
   * and new JobStatus values, and a timestamp
   * (std::chrono::system_clock::now()), then appends it to the mock's internal
   * statusChanges list.
   *
  ...
  /**
   * @brief Checks whether a recorded status-change event matches a specific
   * transition for a job.
   *
   * Searches the captured statusChanges for an event with the given job ID
   * whose previous status equals `from` and whose new status equals `to`.
   *
   ...
   *
   * Searches the stored progress update events for an entry that exactly
   * matches the given job ID, progress percentage, and step string.
   *
   * @param jobId Identifier of the job to look up.
   * @param progress Progress percentage to match (exact integer equality).
   * @param step Exact step description to match (string equality,
   * case-sensitive).
...
public:
   ...
   *
   * Ensures proper polymorphic destruction of derived monitor service
   * implementations.
   */
...
public:
  /**
   * @brief Constructs an adapter that delegates monitoring callbacks to the
   * given mock service.
   *
   * The adapter retains a shared reference to the provided
   * MockJobMonitorService and forwards onJobStatusChanged and
   * onJobProgressUpdated calls to it.
   */
  MockJobMonitorServiceAdapter(std::shared_ptr<MockJobMonitorService> mock)
      : mock_(mock) {}

  /**
   * @brief Forwarding adapter that notifies the underlying mock of a job status
   * change.
   *
   * Delegates the status-change callback to the wrapped MockJobMonitorService
   * instance.
   *
   ...
   *
   * This override delegates the progress notification to the wrapped
   * MockJobMonitorService instance.
   *
 ...
 * (PENDING→RUNNING and RUNNING→COMPLETED) and at least two status-change
 * events.
 */
...
/**
 * @brief Tests that ETLJobManager publishes job progress events to an attached
 * monitor.
 *
 * Creates a MockJobMonitorService and adapter, attaches it to an ETLJobManager,
 * publishes a sequence of progress updates (0, 25, 50, 75, 100) for a test job,
 * and asserts that the mock captured all five progress events with the expected
 * progress values and step descriptions.
 *
 * @note This is a unit test: it uses assertions to validate behavior and will
 * abort on failure.
 */
...
/**
 * @brief Runs an integration-style test that verifies ETL job execution emits
 * monitoring events.
 *
 * This test configures a mock JobMonitorService, connects a test database,
 * starts an ETLJobManager, schedules both an EXTRACT job and a FULL_ETL job,
 * and asserts that the mock monitor receives the expected status transitions
 * and progress updates for each job. The manager is stopped at the end of the
 * test.
 *
 * Side effects:
...
/**
 * @brief Runs an integration-style test verifying ETL job execution when no
 * monitoring service is attached.
 *
 * This test sets up a DatabaseManager and DataTransformer, connects to a test
 * database, starts an ETLJobManager without attaching any JobMonitorService,
 * schedules a simple EXTRACT job, waits for completion, and asserts that the
 * job exists and reached JobStatus::COMPLETED. The test starts and stops the
 * ETLJobManager as part of its lifecycle.
 *
 * Note: the test uses assertions for verification and will terminate the
 * process if a check fails.
 */
...
/**
 * @brief Tests integration between ETLJobManager and a JobMonitorService
 * adapter.
 *
 * Schedules a TRANSFORM job on a manager that has been attached to a
 * MockJobMonitorService (via MockJobMonitorServiceAdapter), then publishes a
 * RUNNING status, a 50% progress update with the step message "Halfway through
 * transformation", and a COMPLETED status. Asserts that the mock monitor
 * observed at least two status changes and at least one progress update.
 *
 * This function has side effects: it mutates the provided ETLJobManager by
 * setting its monitor service, scheduling a job, and publishing status/progress
 * events. It uses assertions to validate that events were received by the mock
 * monitor.
 */
