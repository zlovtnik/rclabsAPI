...
TEST_F(ExceptionTest, ETLExceptionConstruction) {
  ...
  auto timeDiff =
      std::chrono::duration_cast<std::chrono::seconds>(now - ex.getTimestamp());
...
TEST_F(ExceptionTest, SystemExceptionConstruction) {
  ...
  SystemException ex(ErrorCode::DATABASE_ERROR, "Connection timeout", component,
                     context);
...
TEST_F(ExceptionTest, ExceptionHierarchy) {
  ValidationException validationEx(ErrorCode::INVALID_RANGE,
                                   "Value out of range");
...
TEST_F(ExceptionTest, ErrorCodeCoverage) {
  ...
  std::vector<ErrorCode> testCodes = {
      ErrorCode::INVALID_INPUT,    ErrorCode::MISSING_FIELD,
      ErrorCode::UNAUTHORIZED,     ErrorCode::DATABASE_ERROR,
      ErrorCode::NETWORK_ERROR,    ErrorCode::JOB_NOT_FOUND,
      ...
      try {
        for (int j = 0; j < exceptionsPerThread; ++j) {
          ETLException ex(ErrorCode::INTERNAL_ERROR,
                          "Thread " + std::to_string(i) + " exception " +
                              std::to_string(j));
...
TEST_F(ExceptionTest, ComplexContext) {
  ...
  ETLException ex(ErrorCode::ACCESS_DENIED, "Access denied for user",
                  complexContext);
