...
/**
 * Check if string starts/ends with prefix/suffix (case-sensitive and insensitive)
 */
...
public:
    ...
    template<typename T>
    StringBuilder& operator<<(const T& value) { return append(value); }
...
std::string format(std::string_view format_str, Args&&... args) {
    ...
    while (pos < format_str.size()) {
        ...
        if (arg_index < sizeof...(args)) {
            ((arg_index == 0 ? (append_arg(args), ++arg_index) : 
...
std::vector<std::string_view> split_view(std::string_view str, std::string_view delimiter);
 ...
 */
std::vector<std::string> split(std::string_view str, char delimiter, std::size_t max_parts);
