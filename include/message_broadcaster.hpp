...
struct MessageBroadcasterConfig {
  ...
  std::chrono::milliseconds processingInterval =
      std::chrono::milliseconds(10); // Processing interval
  ...
  size_t maxConcurrentBroadcasts =
      10; // Maximum concurrent broadcast operations
};
...
class MessageBroadcaster
    : public std::enable_shared_from_this<MessageBroadcaster> {
public:
  explicit MessageBroadcaster(std::shared_ptr<ConnectionPool> connectionPool);
  MessageBroadcaster(std::shared_ptr<ConnectionPool> connectionPool,
...
private:
  ...
  bool
  shouldProcessMessage(const std::shared_ptr<WebSocketConnection> &connection,
