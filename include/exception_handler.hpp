...
namespace ETLPlus {
    namespace ExceptionHandling {
                    ...
                    try {
                        ...
                        if (attempt == config.maxAttempts || !config.shouldRetry(ex)) {
                            LOG_ERROR("ExceptionHandler", 
                                ...
                                std::to_string(attempt) + " attempts: " + ex.toLogString());
                            throw;
                        }
                        ...
                        LOG_WARN("ExceptionHandler", 
                            ...
                            std::to_string(attempt) + "/" + std::to_string(config.maxAttempts) + 
                            "), retrying in " + std::to_string(delay.count()) + "ms: " + ex.getMessage());
                        ...
                        if (attempt == config.maxAttempts) {
                            LOG_ERROR("ExceptionHandler", 
                                "Operation '" + operationName + "' failed after " + 
                                std::to_string(attempt) + " attempts: " + std::string(ex.what()));
                            throw *baseEx;
                        }
                        ...
                        LOG_WARN("ExceptionHandler", 
                            "Operation '" + operationName + "' failed (attempt " + 
                            std::to_string(attempt) + "/" + std::to_string(config.maxAttempts) + 
                            "), retrying in " + std::to_string(delay.count()) + "ms: " + std::string(ex.what()));
        ...
        private:
            ...
            static void handleStandardException(
                const std::exception& ex,
                ExceptionPolicy policy,
                const std::string& operationName,
                const Exceptions::ErrorContext& context);
        ...
        #define TRY_CATCH_LOG(operation, func) \
            try { \
                ...
                auto baseEx = ETLPlus::ExceptionHandling::ExceptionHandler::convertException(ex, operation); \
                ETLPlus::ExceptionHandling::ExceptionHandler::logException(*baseEx, operation); \
                throw *baseEx; \
            }
        ...
        #define WITH_DATABASE_TRANSACTION(dbManager, operation, code) \
            do { \
                ETLPlus::ExceptionHandling::TransactionScope transaction(dbManager, operation); \
                try { \
                    code; \
                    transaction.commit(); \
                } catch (...) { \
