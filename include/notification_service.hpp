...
struct NotificationMessage {
    ...
    std::chrono::system_clock::time_point timestamp;   // When notification was created
    std::chrono::system_clock::time_point scheduledFor; // When to deliver (for retries)
    ...
    std::unordered_map<std::string, std::string> metadata; // Additional context data
...
struct NotificationConfig {
    ...
    int timeoutWarningThresholdMinutes = 25;  // Warn when job runs longer than this
    ...
    std::unordered_map<NotificationPriority, std::vector<NotificationMethod>> priorityMethods;
...
// Base NotificationService interface (matches existing interface in job_monitor_service.hpp)
...
/**
 * NotificationServiceImpl provides comprehensive notification capabilities for the ETL Plus system.
 * It handles job failure alerts, timeout warnings, resource alerts, and other critical system events.
 * 
 * Features:
...
public:
    ...
    void sendSystemErrorAlert(const std::string& component, const std::string& error);
    ...
    std::vector<NotificationMessage> getRecentNotifications(size_t limit = 100) const;
    void clearQueue();
...
private:
    ...
    std::unordered_map<ResourceAlertType, std::chrono::system_clock::time_point> lastAlertTime_;
    ...
    NotificationMessage createTimeoutWarningNotification(const std::string& jobId, int executionTimeMinutes);
    NotificationMessage createResourceAlertNotification(const ResourceAlert& alert);
    NotificationMessage createSystemErrorNotification(const std::string& component, const std::string& error);

    // Delivery method setup
    void setupDeliveryMethods();
    std::vector<NotificationMethod> getMethodsForPriority(NotificationPriority priority);
