...
template <typename T>
struct has_try_lock_for<T,
                        std::void_t<decltype(std::declval<T &>().try_lock_for(
                            std::chrono::milliseconds(1)))>> : std::true_type {
};
...
struct has_try_lock_shared_for<
    T, std::void_t<decltype(std::declval<T &>().try_lock_shared_for(
           std::chrono::milliseconds(1)))>> : std::true_type {};
 ...
 */
template <LockLevel Level> class OrderedMutex : public std::timed_mutex {
public:
 ...
 */
template <typename Mutex> class ScopedTimedLock {
public:
  ...
  explicit ScopedTimedLock(
      ...
      : mutex_(mutex), locked_(false),
        lockName_(lockName.empty() ? generateLockName() : lockName) {
    ...
    static_assert(
        has_try_lock_for<Mutex>::value,
        "Mutex must support try_lock_for for efficient timeout handling");
...
private:
  ...
  template <typename T> static constexpr bool hasLockLevel() {
  ...
  void recordLockAcquisition(const std::string &lockName,
                             std::chrono::microseconds waitTime);
  void recordLockFailure(const std::string &lockName,
 ...
 */
template <typename SharedMutex> class ScopedTimedSharedLock {
public:
  ...
  explicit ScopedTimedSharedLock(
    ...
    static_assert(has_try_lock_shared_for<SharedMutex>::value,
                  "SharedMutex must support try_lock_shared_for for efficient "
                  "timeout handling");
...
private:
  ...
  std::string generateLockName() const {
    return "shared_lock_" +
           std::to_string(reinterpret_cast<uintptr_t>(&mutex_));
  }

  void checkLockOrdering(LockLevel level);
  void unregisterSharedLockAcquisition(LockLevel level,
...
public:
  ...
  void recordAcquisition(const std::string &lockName,
                         std::chrono::microseconds waitTime);
  void recordFailure(const std::string &lockName,
                     std::chrono::milliseconds timeout,
                     std::chrono::microseconds waitTime);
...
public:
  ...
  void registerLockAcquisition(std::thread::id threadId, LockLevel level,
                               const std::string &mutexId);
  void unregisterLockAcquisition(std::thread::id threadId, LockLevel level,
                                 const std::string &mutexId);

  void registerSharedLockAcquisition(std::thread::id threadId, LockLevel level,
                                     const std::string &mutexId);
  void unregisterSharedLockAcquisition(std::thread::id threadId,
                                       LockLevel level,
                                       const std::string &mutexId);
...
#define SCOPED_LOCK_TIMEOUT(mutex, timeout_ms)                                 \
  etl_plus::ScopedTimedLock lock(mutex, std::chrono::milliseconds(timeout_ms), \
                                 #mutex)

#define SCOPED_SHARED_LOCK(mutex)                                              \
  etl_plus::ScopedTimedSharedLock lock(mutex, std::chrono::milliseconds(5000), \
                                       #mutex)

#define SCOPED_SHARED_LOCK_TIMEOUT(mutex, timeout_ms)                          \
  etl_plus::ScopedTimedSharedLock lock(                                        \
      mutex, std::chrono::milliseconds(timeout_ms), #mutex)
...
template <typename Mutex>
void ScopedTimedLock<Mutex>::unregisterLockAcquisition(
    LockLevel level, const std::string &mutexId) {
  DeadlockDetector::getInstance().unregisterLockAcquisition(
      std::this_thread::get_id(), level, mutexId);
}

template <typename Mutex>
void ScopedTimedLock<Mutex>::recordLockAcquisition(
    const std::string &lockName, std::chrono::microseconds waitTime) {
  LockMonitor::getInstance().recordAcquisition(lockName, waitTime);
}

template <typename Mutex>
void ScopedTimedLock<Mutex>::recordLockFailure(
    const std::string &lockName, std::chrono::milliseconds timeout,
    std::chrono::microseconds waitTime) {
...
template <typename SharedMutex>
void ScopedTimedSharedLock<SharedMutex>::unregisterSharedLockAcquisition(
    LockLevel level, const std::string &mutexId) {
...
template <typename SharedMutex>
void ScopedTimedSharedLock<SharedMutex>::recordLockAcquisition(
    const std::string &lockName, std::chrono::microseconds waitTime) {
  LockMonitor::getInstance().recordAcquisition(lockName, waitTime);
}

template <typename SharedMutex>
void ScopedTimedSharedLock<SharedMutex>::recordLockFailure(
    const std::string &lockName, std::chrono::milliseconds timeout,
    std::chrono::microseconds waitTime) {
  LockMonitor::getInstance().recordFailure(lockName, timeout, waitTime);
}
...
template <typename SharedMutex>
void ScopedTimedSharedLock<SharedMutex>::recordLockRelease(
    const std::string &lockName) {
  LockMonitor::getInstance().recordRelease(lockName);
}
