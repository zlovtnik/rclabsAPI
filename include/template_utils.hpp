...
template <typename T>
struct has_component_trait<T, std::void_t<decltype(ComponentTrait<T>::name)>>
    : std::true_type {};
 ...
 */
template <typename T> struct is_strong_id : std::false_type {};

template <typename Tag> struct is_strong_id<StrongId<Tag>> : std::true_type {};

template <typename T> constexpr bool is_strong_id_v = is_strong_id<T>::value;
 ...
 */
template <typename T> struct strong_id_value_type {
  using type = void;
};

template <> struct strong_id_value_type<JobId> {
  using type = std::string;
};

template <> struct strong_id_value_type<ConnectionId> {
  using type = std::string;
};

template <> struct strong_id_value_type<UserId> {
  using type = std::string;
};
...
template <std::size_t N> struct string_literal {
  ...
  constexpr std::size_t hash() const noexcept { return fnv1a_hash(view()); }
};

/**
 * Compile-time string hash wrapper
 */
template <std::size_t N> struct compile_time_hash {
 ...
 */
template <typename KeyEnum> class TypedConfigMap {
public:
  template <KeyEnum Key> void set(std::string value) {
    config_[static_cast<std::size_t>(Key)] = std::move(value);
  }

  template <KeyEnum Key> std::string get() const {
  ...
  template <KeyEnum Key> bool has(KeyEnum key) const {
 ...
 */
template <typename Component> class ScopedTimer {
public:
 ...
 */
template <typename T> class RangeValidator : public Validator<T> {
public:
  RangeValidator(T min_val, T max_val) : min_val_(min_val), max_val_(max_val) {
    static_assert(std::is_arithmetic_v<T>,
                  "RangeValidator requires arithmetic type");
  }
 ...
 */
template <typename BaseType, typename KeyType = std::string> class Factory {
public:
  using CreateFunc = std::function<std::unique_ptr<BaseType>()>;

  template <typename DerivedType> void register_type(const KeyType &key) {
...
private:
  std::unordered_map<
      ...
      std::conditional_t<std::is_same_v<KeyType, std::string>,
                         TransparentStringHash, std::hash<KeyType>>,
      std::conditional_t<std::is_same_v<KeyType, std::string>, std::equal_to<>,
 ...
 */
template <typename EventType> class EventDispatcher {
public:
 ...
 */
template <typename T>
std::enable_if_t<!is_strong_id_v<T> && !std::is_same_v<T, std::string>,
                 std::string>
 ...
 */
template <bool Condition> struct conditional_compile {
  template <typename T> static constexpr T value(T true_val, T false_val) {
