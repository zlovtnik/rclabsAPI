...
using ExceptionHandlerFunc =
    std::function<HttpResponse(const etl::ETLException &, const std::string &)>;
...
struct ExceptionMappingConfig {
  ...
  boost::beast::http::status defaultStatus =
...
public:
  ...
  ErrorResponseFormat
  createErrorFormat(const etl::ETLException &exception) const;
...
private:
  ...
  template <typename ExceptionType> bool hasCustomTypeHandler() const {
    std::lock_guard<std::mutex> lock(typeHandlersMutex_);
    return typeHandlers_.find(std::type_index(typeid(ExceptionType))) !=
           typeHandlers_.end();
  }

  // Apply custom handler if available
  std::optional<HttpResponse>
  tryCustomHandler(const etl::ETLException &exception,
                   const std::string &operationName) const;
...
HttpResponse
createRateLimitResponse(const std::string &message = "Rate limit exceeded",
                        const std::string &retryAfter = "60");

// Create maintenance mode response
HttpResponse createMaintenanceResponse(
    const std::string &message = "Service temporarily unavailable");

// Global exception mapper instance (thread-safe)
// The function-local static ensures thread-safe construction and that internal
// operations are protected by the class's locks.
ExceptionMapper &getGlobalExceptionMapper();

// Convenience functions for exception mapping
inline HttpResponse mapExceptionToResponse(const etl::ETLException &exception,
