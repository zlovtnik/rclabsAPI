...
struct ServerConfig {
  ...
  std::chrono::seconds idleTimeout{
      300}; // 5 minutes - timeout for idle connections

  // Timeout Settings
  std::chrono::seconds connectionTimeout{
      30};                                 // Connection establishment timeout
  ...
  size_t maxQueueSize =
      100; // Maximum requests to queue when pool is at capacity
  std::chrono::seconds maxQueueWaitTime{
      30}; // Maximum time a request can wait in queue
  ...
  ValidationResult validate() const {
    ...
    if (maxConnections > 1000) {
      result.addWarning("maxConnections is very high (" +
    ...
    if (maxQueueSize > 1000) {
      result.addWarning("maxQueueSize is very large (" +
   ...
   */
  static ServerConfig create(size_t minConn = 10, size_t maxConn = 100,
                             int idleTimeoutSec = 300, int connTimeoutSec = 30,
                             int reqTimeoutSec = 60,
                             size_t maxBodySize = 10 * 1024 * 1024,
                             bool metricsEnabled = true, size_t queueSize = 100,
