...
public:
    ...
    static void debug(const std::string& message, Args&&... args) {
        if constexpr (sizeof...(args) > 0) {
            getLogger().debug(component_name, format_message(message, std::forward<Args>(args)...));
        } else {
    ...
    static void info(const std::string& message, Args&&... args) {
        if constexpr (sizeof...(args) > 0) {
            getLogger().info(component_name, format_message(message, std::forward<Args>(args)...));
        } else {
    ...
    static void warn(const std::string& message, Args&&... args) {
        if constexpr (sizeof...(args) > 0) {
            getLogger().warn(component_name, format_message(message, std::forward<Args>(args)...));
        } else {
    ...
    static void error(const std::string& message, Args&&... args) {
        if constexpr (sizeof...(args) > 0) {
            getLogger().error(component_name, format_message(message, std::forward<Args>(args)...));
        } else {
    ...
    static void fatal(const std::string& message, Args&&... args) {
        if constexpr (sizeof...(args) > 0) {
            getLogger().fatal(component_name, format_message(message, std::forward<Args>(args)...));
        } else {
    ...
    template<typename... Args>
    static void debugJob(const std::string& message, const std::string& jobId, Args&&... args) {
        if constexpr (sizeof...(args) > 0) {
            getLogger().debugForJob(component_name, format_message(message, std::forward<Args>(args)...), jobId);
        } else {
    ...
    template<typename... Args>
    static void infoJob(const std::string& message, const std::string& jobId, Args&&... args) {
        if constexpr (sizeof...(args) > 0) {
            getLogger().infoForJob(component_name, format_message(message, std::forward<Args>(args)...), jobId);
        } else {
    ...
    template<typename... Args>
    static void warnJob(const std::string& message, const std::string& jobId, Args&&... args) {
        if constexpr (sizeof...(args) > 0) {
            getLogger().warnForJob(component_name, format_message(message, std::forward<Args>(args)...), jobId);
        } else {
    ...
    template<typename... Args>
    static void errorJob(const std::string& message, const std::string& jobId, Args&&... args) {
        if constexpr (sizeof...(args) > 0) {
            getLogger().errorForJob(component_name, format_message(message, std::forward<Args>(args)...), jobId);
        } else {
    ...
    template<typename... Args>
    static void fatalJob(const std::string& message, const std::string& jobId, Args&&... args) {
        if constexpr (sizeof...(args) > 0) {
            getLogger().fatalForJob(component_name, format_message(message, std::forward<Args>(args)...), jobId);
        } else {
    ...
    static void logMetric(const std::string& name, double value, const std::string& unit = "") {
    ...
    static void log_info_job(const char* component, const std::string& message, const std::string& jobId) {
        getLogger().infoForJob(component, message, jobId);
    }

    static void log_error_job(const char* component, const std::string& message, const std::string& jobId) {
        getLogger().errorForJob(component, message, jobId);
    }

    static void log_debug_job(const char* component, const std::string& message, const std::string& jobId) {
        getLogger().debugForJob(component, message, jobId);
    }

    static void log_warn_job(const char* component, const std::string& message, const std::string& jobId) {
        getLogger().warnForJob(component, message, jobId);
    }

    static void log_fatal_job(const char* component, const std::string& message, const std::string& jobId) {
