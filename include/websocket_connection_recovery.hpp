...
struct ConnectionRecoveryState {
  ...
  bool shouldAttemptReconnect(const ConnectionRecoveryConfig &config) const {
    if (!config.enableAutoReconnect)
      return false;
    if (reconnectAttempts.load() >= config.maxReconnectAttempts)
      return false;
  ...
  calculateBackoffDelay(const ConnectionRecoveryConfig &config) const {
    int attempts = reconnectAttempts.load();
    if (attempts <= 0)
      return config.baseReconnectDelay;

    auto delay = static_cast<long long>(
        config.baseReconnectDelay.count() *
        std::pow(config.backoffMultiplier, attempts - 1));
    delay = std::min(delay,
                     static_cast<long long>(config.maxReconnectDelay.count()));

    return std::chrono::milliseconds(delay);
  }

  void addPendingMessage(const std::string &message,
                         const ConnectionRecoveryConfig &config) {
    std::lock_guard<std::mutex> lock(pendingMessagesMutex);
