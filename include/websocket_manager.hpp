...
/**
 * @brief WebSocketManager coordinates WebSocket connections and message broadcasting
 *
 * This class acts as a coordinator between ConnectionPool and MessageBroadcaster,
 * maintaining backward compatibility with existing APIs while providing enhanced
 * functionality through component separation.
 */
...
public:
    ...
    std::shared_ptr<ConnectionPool> getConnectionPool() const { return connectionPool_; }
    std::shared_ptr<MessageBroadcaster> getMessageBroadcaster() const { return messageBroadcaster_; }
    ...
    void updateMessageBroadcasterConfig(const MessageBroadcasterConfig& newConfig);
