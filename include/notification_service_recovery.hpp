...
struct FailedNotification {
    ...
    FailedNotification(const std::string& id, const std::string& recip, const std::string& cont,
                      std::chrono::system_clock::time_point retryTime,
                      const std::string& reason, int retries, int methodIdx)
        : notificationId(id), recipient(recip), content(cont), nextRetryTime(retryTime), 
          failureReason(reason), retryCount(retries), failedMethodIndex(methodIdx) {}
...
public:
    ...
    void addFailedNotification(const std::string& notificationId,
        ...
        retryQueue_.emplace(notificationId, recipient, content, nextRetryTime, reason, 0, failedMethodIndex);
    }
...
private:
    std::chrono::milliseconds calculateRetryDelay(int retryCount) const {
        if (retryCount <= 0) return config_.baseRetryDelay;
        
        auto delay = static_cast<long long>(config_.baseRetryDelay.count() * 
                                          std::pow(config_.backoffMultiplier, retryCount));
        delay = std::min(delay, static_cast<long long>(config_.maxRetryDelay.count()));
