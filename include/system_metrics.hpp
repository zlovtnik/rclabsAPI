...
public:
    ...
    // Alert callbacks
    using MemoryAlertCallback = std::function<void(size_t currentUsage, size_t threshold)>;
    using CpuAlertCallback = std::function<void(double currentUsage, double threshold)>;
...
public:
    ...
    using MetricsUpdateCallback = std::function<void(const std::string& jobId, const MetricsSnapshot& metrics)>;
