...
struct ConnectionPoolConfig {
    ...
    std::chrono::seconds connectionTimeout = std::chrono::seconds(30);  // Connection timeout
    std::chrono::seconds healthCheckInterval = std::chrono::seconds(60); // Health check interval
    bool enableHealthMonitoring = true;             // Enable automatic health monitoring
    bool enableConnectionCleanup = true;            // Enable automatic cleanup of stale connections
...
/**
 * @brief Manages a pool of WebSocket connections with lifecycle and health monitoring
 *
...
public:
    ...
    std::shared_ptr<WebSocketConnection> getConnection(const std::string& connectionId) const;
    bool hasConnection(const std::string& connectionId) const;

    // Bulk operations
    std::vector<std::shared_ptr<WebSocketConnection>> getActiveConnections() const;
...
private:
    ...
    bool isConnectionStale(const std::shared_ptr<WebSocketConnection>& connection) const;
