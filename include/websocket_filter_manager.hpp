...
public:
  ...
  http::response<http::string_body> handleGetFilterStatistics();

  // Advanced filter management
  void saveConnectionPreferences(const std::string &connectionId,
                                 const ConnectionFilters &filters);
  bool loadConnectionPreferences(const std::string &connectionId,
                                 ConnectionFilters &filters);
  void clearStoredPreferences(const std::string &connectionId);

  // Batch operations
  void applyFiltersToMultipleConnections(
      const std::vector<std::string> &connectionIds,
      const ConnectionFilters &filters);
  void clearFiltersFromMultipleConnections(
      const std::vector<std::string> &connectionIds);
  ...
  void saveFilterTemplate(const std::string &templateName,
                          const ConnectionFilters &filters);
  bool loadFilterTemplate(const std::string &templateName,
                          ConnectionFilters &filters);
  void applyFilterTemplate(const std::string &connectionId,
...
private:
  ...
  std::unordered_map<std::string, ConnectionFilters, TransparentStringHash,
                     std::equal_to<>>
      storedPreferences_;
  ...
  bool validateFilterData(const ConnectionFilters &filters,
