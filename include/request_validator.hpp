 ...
 * 
 * This class extracts and centralizes all request validation logic from RequestHandler,
 * providing a clean separation of concerns and reusable validation utilities.
 * It handles input validation, security checks, parameter extraction, and request routing validation.
 */
...
public:
    ...
    ValidationResult validateRequestBasics(const http::request<http::string_body>& req);
    SecurityValidationResult validateSecurity(const http::request<http::string_body>& req);
    
    // HTTP method validation
    bool isValidMethod(const std::string& method, const std::string& endpoint);
    ValidationResult validateMethodForEndpoint(const std::string& method, const std::string& path);
    
    // Path and routing validation
    ValidationResult validatePath(std::string_view path);
    ValidationResult validateEndpoint(const std::string& method, const std::string& path);
    ...
    // Body validation
    ValidationResult validateBody(const std::string& body, const std::string& contentType);
    ...
    // Content validation
    ValidationResult validateContentType(const std::string& contentType, const std::string& endpoint);
    ValidationResult validateContentLength(size_t contentLength);
    
    // Endpoint-specific validation
    ValidationResult validateAuthEndpoint(const http::request<http::string_body>& req);
    ValidationResult validateJobsEndpoint(const http::request<http::string_body>& req);
    ValidationResult validateLogsEndpoint(const http::request<http::string_body>& req);
    ValidationResult validateMonitoringEndpoint(const http::request<http::string_body>& req);
    ValidationResult validateHealthEndpoint(const http::request<http::string_body>& req);
    
    // Security validation methods
    bool checkRateLimit(const std::string& clientIp);
    bool validateCsrfToken(const std::string& token, const std::string& sessionId);
    ...
    std::string extractJobIdFromPath(std::string_view target, std::string_view prefix, std::string_view suffix);
...
private:
    ...
    mutable std::unordered_map<std::string, std::vector<std::chrono::system_clock::time_point>> rateLimitMap_;
    mutable std::mutex rateLimitMutex_;
    
    // Known endpoints for validation
    std::unordered_set<std::string> knownEndpoints_;
    std::unordered_map<std::string, std::unordered_set<std::string>> allowedMethodsPerEndpoint_;
    ...
    ValidationResult validateParameterizedPath(const std::string& path, const std::string& pattern);
