 ...
 * 
 * This class extracts and centralizes all response building logic from RequestHandler,
 * providing a clean fluent interface for response construction, content negotiation,
 * serialization support, and standardized error handling.
 */
...
public:
    ...
    ResponseBuilder& setHeaders(const std::unordered_map<std::string, std::string>& headers);
    ...
    http::response<http::string_body> success(const std::string& data, ContentType type);
    http::response<http::string_body> successJson(const std::string& jsonData);
    http::response<http::string_body> successWithMessage(const std::string& message, const std::string& data = "");
    
    // Error response methods
    http::response<http::string_body> error(http::status status, const std::string& message);
    http::response<http::string_body> badRequest(const std::string& message);
    http::response<http::string_body> unauthorized(const std::string& message = "Unauthorized");
    http::response<http::string_body> forbidden(const std::string& message = "Forbidden");
    http::response<http::string_body> notFound(const std::string& resource = "Resource");
    http::response<http::string_body> methodNotAllowed(const std::string& method, const std::string& endpoint);
    http::response<http::string_body> conflict(const std::string& message);
    http::response<http::string_body> tooManyRequests(const std::string& message = "Rate limit exceeded");
    http::response<http::string_body> internalServerError(const std::string& message = "Internal server error");
    http::response<http::string_body> serviceUnavailable(const std::string& message = "Service unavailable");
    
    // Exception-based response methods
    http::response<http::string_body> fromException(const etl::ETLException& ex);
    http::response<http::string_body> fromValidationResult(const InputValidator::ValidationResult& result);
    http::response<http::string_body> fromStandardException(const std::exception& ex, const std::string& context = "");
    
    // Specialized response methods
    http::response<http::string_body> validationError(const InputValidator::ValidationResult& result);
    http::response<http::string_body> authenticationRequired(const std::string& realm = "ETL Plus API");
    http::response<http::string_body> corsPreflightResponse();
    http::response<http::string_body> healthCheck(bool healthy, const std::string& details = "");
    
    // Content negotiation and serialization
    http::response<http::string_body> negotiate(const std::string& acceptHeader, 
                                              const std::unordered_map<ContentType, std::string>& content);
    
    // Streaming and chunked responses
    http::response<http::string_body> streamingResponse(const std::string& contentType = "text/plain");
    http::response<http::string_body> chunkedResponse(const std::vector<std::string>& chunks, 
                                                    ContentType type = ContentType::JSON);
    
    // File and binary responses
    http::response<http::string_body> fileResponse(const std::string& filePath, 
                                                 const std::string& filename = "");
    http::response<http::string_body> binaryResponse(const std::vector<uint8_t>& data, 
                                                   const std::string& mimeType);
    
    // Redirect responses
    http::response<http::string_body> redirect(const std::string& location, 
                                             http::status status = http::status::found);
    http::response<http::string_body> permanentRedirect(const std::string& location);
    http::response<http::string_body> temporaryRedirect(const std::string& location);
    
    // Caching responses
    http::response<http::string_body> cached(const std::string& data, 
...
private:
    ...
    void applyCorsHeaders(http::response<http::string_body>& response, const CorsConfig& cors);
    ...
    std::string createErrorJson(const std::string& message, const std::string& code = "ERROR");
    std::string createSuccessJson(const std::string& data, const std::string& message = "");
    std::string createValidationErrorJson(const InputValidator::ValidationResult& result);
