...
struct CacheConfig {
  ...
  std::chrono::seconds sessionDataTTL =
  ...
  std::chrono::seconds warmupBatchTimeout =
      std::chrono::seconds(5); // Timeout per batch
  std::chrono::seconds warmupTotalTimeout =
      std::chrono::seconds(60); // Total warmup timeout
};
...
public:
  ...
  bool cacheSessionData(const std::string &sessionId,
...
private:
  ...
  std::chrono::seconds
  getTTLForTags(const std::vector<std::string> &tags) const;
