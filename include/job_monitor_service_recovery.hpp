...
struct ServiceRecoveryState {
    ...
    bool shouldAttemptRecovery(const ServiceRecoveryConfig& config) const {
        ...
        if (recoveryAttempts.load() >= config.maxRecoveryAttempts) return false;
    ...
    std::chrono::milliseconds calculateBackoffDelay(const ServiceRecoveryConfig& config) const {
        int attempts = recoveryAttempts.load();
        if (attempts <= 0) return config.baseRecoveryDelay;
        
        auto delay = static_cast<long long>(config.baseRecoveryDelay.count() * 
                                          std::pow(config.backoffMultiplier, attempts - 1));
        delay = std::min(delay, static_cast<long long>(config.maxRecoveryDelay.count()));
