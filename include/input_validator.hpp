...
public:
    ...
    struct ValidationError {
        ...
        ValidationError(const std::string& f, const std::string& m, const std::string& c = "INVALID_INPUT")
    ...
    struct ValidationResult {
        ...
        void addError(const std::string& field, const std::string& message, const std::string& code = "INVALID_INPUT") {
    ...
    static ValidationResult validateJsonStructure(const std::string& json, const std::vector<std::string>& requiredFields);
    
    // Field type validation methods
    static bool isValidString(const std::string& value, size_t minLength = 0, size_t maxLength = SIZE_MAX);
    ...
    static ValidationResult validateJobQueryParams(const std::unordered_map<std::string, std::string>& params);
    
    // Monitoring endpoint validation
    static ValidationResult validateMonitoringParams(const std::unordered_map<std::string, std::string>& params);
    
    // URL and path validation
    static ValidationResult validateEndpointPath(const std::string& path);
    static ValidationResult validateQueryParameters(const std::string& queryString);
    
    // HTTP method validation
    static bool isValidHttpMethod(const std::string& method, const std::vector<std::string>& allowedMethods);
    
    // Content type validation
    static bool isValidContentType(const std::string& contentType);
    
    // Authorization header validation
    static ValidationResult validateAuthorizationHeader(const std::string& authHeader);
    
    // Rate limiting and security validation
    static bool isValidRequestSize(size_t contentLength, size_t maxSize = 1024 * 1024); // 1MB default
    static ValidationResult validateRequestHeaders(const std::unordered_map<std::string, std::string>& headers);
...
private:
    ...
    static std::optional<size_t> findJsonFieldStart(const std::string& json, const std::string& field);
    static std::string extractJsonValue(const std::string& json, size_t start, size_t end);
