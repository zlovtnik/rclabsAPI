...
struct LogEntry {
  ...
  LogEntry(
      LogLevel lvl, const std::string &comp, const std::string &msg,
      const std::string &job_id,
      const std::unordered_map<std::string, std::string, TransparentStringHash,
                               std::equal_to<>> &ctx = {})
      : timestamp(std::chrono::system_clock::now()), level(lvl),
        component(comp), message(msg), jobId(job_id), context(ctx) {}
};
...
public:
   ...
   * @param minLevel Minimum log level to handle
   */
  StreamingLogHandler(const std::string &id,
   ...
   */
  void setJobFilter(const std::unordered_set<std::string, TransparentStringHash,
                                             std::equal_to<>> &jobIds);
