...
/**
 * TimeoutManager provides centralized timeout handling for HTTP connections and requests.
 * It manages Boost.Asio steady_timer instances for connection and request timeouts,
 * with proper cleanup and callback mechanisms.
 */
class TimeoutManager {
public:
    using TimeoutCallback = std::function<void(std::shared_ptr<PooledSession>, TimeoutType)>;
     ...
     */
    TimeoutManager(net::io_context& ioc,
     ...
     * @param callback Optional callback to invoke on timeout (uses default if not provided)
     * @param timeout Optional custom timeout duration (uses default if not provided)
     */
...
private:
     ...
     * @param type The type of timeout
     */
    void defaultTimeoutHandler(std::shared_ptr<PooledSession> session, TimeoutType type);
