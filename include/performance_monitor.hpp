 ...
 * request timing, connection reuse rates, timeout tracking, and resource utilization.
 * All operations are thread-safe and designed for high-performance concurrent access.
 */
...
public:
        ...
        // Store individual response time for detailed analysis
        {
            ...
            if (responseTimes_.size() > 10000) {
                responseTimes_.erase(responseTimes_.begin(), 
                                   responseTimes_.begin() + (responseTimes_.size() - 10000));
            }
        }
    }
    ...
    Metrics getMetrics() const {
        ...
        auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(now - snapshot.startTime);
        if (elapsed.count() > 0) {
            snapshot.requestsPerSecond = snapshot.totalRequests.load() / elapsed.count();
        }
...
private:
    ...
    void updateAverageResponseTime(double newResponseTime) {
        ...
        double currentAvg = metrics_.averageResponseTime.load(std::memory_order_relaxed);
        double newAvg;
